...
by calling the configured coding-agent CLI in headless mode as a subprocess
(same auth pattern as run_eval.py — uses the session's CLI auth).
"""
...
def _call_agent_cli(prompt: str, model: str | None, timeout: int = 300) -> str:
    """Run the configured agent CLI with the prompt on stdin and return the text response.

    Prompt goes over stdin (not argv) because it embeds the full SKILL.md
    body and can easily exceed comfortable argv length.
    """
    cli = os.environ.get("SKILL_EVAL_CLI", "claude")
    cmd = [cli, "-p", "--output-format", "text"]
    if model:
        cmd.extend(["--model", model])

    # Remove CLAUDECODE env var to allow nesting claude -p inside a
    # agent CLI session. The guard is for interactive terminal conflicts;
...
def improve_description(
    ...
    """Call the configured agent CLI to improve the description based on eval results."""
    failed_triggers = [
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    text = _call_agent_cli(prompt, model)
    ...
    # SDK path did this as a true multi-turn; the headless CLI is one-shot, so we
    ...
    if len(description) > 1024:
        ...
        shorten_text = _call_agent_cli(shorten_prompt, model)
