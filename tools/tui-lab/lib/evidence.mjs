...
async function ownedAgentOutputPath(sessionDir, agentId, agentDir) {
  let state;
  try {
    state = JSON.parse(await readFile(path.join(sessionDir, 'state.json'), 'utf8'));
  } catch (error) {
    if (error?.code !== 'ENOENT') throw error;
    return path.join(agentDir, 'output.md');
  }
  const declared = state.tasks?.[agentId]?.output_path;
  if (typeof declared !== 'string') return path.join(agentDir, 'output.md');
  const relative = path.relative(await realpath(agentDir), await realpath(declared));
...
async function collectSubagentEvidence(projectDir) {
  ...
  for (const sessionEntry of sessions.filter((entry) => entry.isDirectory())) {
    ...
    for (const agentEntry of agents.filter((entry) => entry.isDirectory())) {
      ...
      const output = await ownedAgentOutputPath(path.join(projectDir, sessionId), agentId, agentDir);
...
async function collectTargetedSubagentSteerEvidence(
  ...
  result.targetOutput = await ownedAgentOutputPath(sessionDir, targetAgentId, targetDir);
