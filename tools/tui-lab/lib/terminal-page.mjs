...
<body>
  ...
  <script>
    ...
    window.tuiLab = {
      beginResponseProbe(token) { responseProbe = { token, started: performance.now(), tail: "", rawMs: null, parsedMs: null }; },
      responseProbe() { return responseProbe ? { rawMs: responseProbe.rawMs, parsedMs: responseProbe.parsedMs } : {}; },
      ready: false,
      ...
      resize(cols, rows) {
        if (!Number.isInteger(cols) || !Number.isInteger(rows) || cols < 20 || cols > 300 || rows < 10 || rows > 120) throw new Error('invalid owned terminal size');
      ...
      internalScrollbar() {
        ...
        for (let row = 0; row < term.rows; row += 1) {
          ...
          // A welcome/tool card border beside the gutter is content, not a second
          // scrollbar column. Compare cell style as well as rail-shaped glyphs.
          const sameRailStyle = other => other && other.getFgColorMode() === cell.getFgColorMode() &&
            other.getFgColor() === cell.getFgColor() && other.getBgColorMode() === cell.getBgColorMode() &&
            other.getBgColor() === cell.getBgColor() && other.isInverse() === cell.isInverse();
          if ((railCell(left) && sameRailStyle(left)) || (railCell(right) && sameRailStyle(right))) adjacentRailRows.push(row);
        }
    ...
    socket.addEventListener('message', (event) => {
      ...
      let matched = false;
      if (probe && probe.rawMs === null) {
        probe.tail = (probe.tail + event.data).slice(-32768);
        if (probe.tail.includes(probe.token)) { probe.rawMs = performance.now() - probe.started; matched = true; }
      }
      term.write(event.data, () => {
        if (matched && probe === responseProbe) probe.parsedMs = performance.now() - probe.started;
        updateScrollbar();
      });
    });
