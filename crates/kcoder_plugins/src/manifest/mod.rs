...
pub fn load_plugin_manifest(
    ...
    for (relative, parser) in external_candidates {
        ...
        if regular_file(&path)? {
            ...
            let manifest = parser(root, &contents)?;
            let shadowed = shadowed_manifests(root, &path, None)?;
            let mut loaded = LoadedPluginManifest::new(manifest, path, shadowed);
            let raw: serde_json::Value = serde_json::from_str(&contents).unwrap_or_default();
            for (field, capability, conventional) in [
                ("agents", "agents", "agents"),
                ("lspServers", "lsp_servers", ".lsp.json"),
                ("outputStyles", "output_styles", "output-styles"),
            ] {
                let declared = raw.get(field).is_some_and(|value| !value.is_null());
                let present = std::fs::symlink_metadata(root.join(conventional))
                    .is_ok_and(|metadata| !metadata.file_type().is_symlink());
                if declared || present {
                    loaded.compatibility.defer_capability(
                        capability,
                        "capability_deferred",
                        format!(
                            "plugin capability `{capability}` is not activated by this runtime"
                        ),
                    );
                }
            }
