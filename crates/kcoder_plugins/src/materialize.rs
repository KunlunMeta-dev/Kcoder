...
pub(crate) fn materialize_git(request: GitMaterializeRequest<'_>) -> Result<MaterializedPlugin> {
    ...
    // HEAD imports need only the current snapshot. Pinned revisions keep full
    // history so older exact SHAs and explicit refs remain resolvable.
    if request.sha.is_none() && request.ref_name.is_none() {
    ...
    run_isolated_command("git", &clone_args, &[], "clone", &command_context)
        .context("failed to clone plugin Git source")?;

    let selector = request.sha.or(request.ref_name).unwrap_or("HEAD");
    let checkout_args = [
        OsString::from("-C"),
        git_checkout.clone(),
        OsString::from("-c"),
...
fn configure_git_proxy(command: &mut Command, proxy: Option<&OsStr>) -> Result<()> {
    ...
    // Only the explicit plugin Git proxy crosses this otherwise cleared environment.
    command
...
fn run_isolated_command(
    ...
    if program == "git" {
        configure_git_proxy(
            &mut command,
            std::env::var_os("KCODER_PLUGIN_GIT_PROXY").as_deref(),
        )?;
    }
    #[cfg(windows)]
