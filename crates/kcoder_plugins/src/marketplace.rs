...
pub fn load_marketplace_manifest(path: &Path) -> Result<MarketplaceManifest> {
    ...
    for raw_plugin in raw.plugins {
        ...
        if !seen.insert(plugin_id.clone()) {
            bail!("marketplace contains duplicate plugin entry {plugin_id}");
        }
        ...
        let fallback = (!raw_plugin.manifest_fields.is_empty())
            .then(|| Value::Object(raw_plugin.manifest_fields.clone()));
