...
#[tokio::test]
async fn queued_model_refresh_failure_preserves_input_without_repeated_notices() {
    struct RejectedSource;
    impl kcoder_engine::ClientModelConfiguration for RejectedSource {
        fn settings(&self, _: &str) -> anyhow::Result<kcoder_config::Settings> {
            anyhow::bail!("selected model removed")
        }
        fn provider(&self, _: &kcoder_config::Settings) -> anyhow::Result<std::sync::Arc<dyn kcoder_api::Provider>> {
            panic!("invalid model must not construct provider")
        }
    }
    let temp = tempfile::tempdir().unwrap();
    let engine = test_engine(temp.path()).with_client_model_configuration(std::sync::Arc::new(RejectedSource));
    let mut app = ReplApp::default();
    app.enqueue_user_message_for_turn(Message::user_text("keep my queued input"));
    let (raw_tx, mut rx) = mpsc::channel(APP_EVENT_CHANNEL_CAPACITY);
    let tx = AppEventSender::new(raw_tx);
    let prompt = TuiPermissionPrompt::new(tx.clone());
    let before = engine.state.messages().len();
    for _ in 0..2 {
        assert!(!try_start_next_turn(&engine, &mut app, &tx, &prompt).await.unwrap().started());
        assert_eq!(app.queued_user_message_count(), 1);
        assert_eq!(engine.state.messages().len(), before);
    }
    assert!(matches!(rx.try_recv().unwrap(), AppEvent::SystemNotice(message) if message.contains("queued input was kept")));
    assert!(rx.try_recv().is_err());
    assert!(app.restore_latest_queued_message_for_edit());
    assert_eq!(app.input, "keep my queued input");
}
