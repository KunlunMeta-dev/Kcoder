...
async fn slash_plugins_lists_discovered_plugins() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    // PluginManager reads the user layer even with explicit effective settings.
    let _config = EnvVarGuard::set("KCODER_CONFIG_DIR", &tmp.path().join("config"));
    ...
    let plugins = messages_as_pairs(&app)
        ...
        .map(|(_, text)| text)
        .unwrap_or_else(|| panic!("/plugins should add a plugin status message: {:?}", messages_as_pairs(&app)));
...
async fn slash_plugins_surfaces_incompatible_manifest_diagnostics() {
    ...
    let status = messages_as_pairs(&app)
        ...
        .unwrap_or_else(|| panic!("/plugins should surface invalid plugin diagnostics: {:?}", messages_as_pairs(&app)));
