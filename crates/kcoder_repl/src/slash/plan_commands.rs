...
// Storage owns commit/rollback. The UI must never convert an Err into a started notice.
fn report_goal_creation(
    app: &mut ReplApp,
    result: anyhow::Result<Goal>,
    label: &str,
) -> Option<Goal> {
    match result {
        Ok(goal) => Some(goal),
...
impl SlashCommand for GoalCommand {
    ...
    async fn run(&self, args: &str, app: &mut ReplApp, engine: &QueryEngine) -> SlashResult {
        ...
        let Some(goal) = report_goal_creation(
            app,
            engine.state.set_goal_prepared_with_mode_and_verification(
                prepared.objective.clone(),
                prepared.objective_file.clone(),
                token_budget,
                GoalMode::Standard,
...
mod tests {
    ...
    #[tokio::test]
    async fn goal_command_reports_real_persistence_failure_and_recovers_after_repair() {
        let temp = tempfile::tempdir().unwrap();
        let engine = test_engine(temp.path());
        engine.state.with_history_path(temp.path().join("history.jsonl"));
        let sidecar = engine.state.session_state_path().unwrap();
        if sidecar.is_file() { std::fs::remove_file(&sidecar).unwrap(); }
        std::fs::create_dir_all(&sidecar).unwrap();
        let mut app = ReplApp::default();
        assert_eq!(super::GoalCommand.run("persist this objective", &mut app, &engine).await, SlashResult::Handled);
        assert!(engine.state.goal().is_none());
        assert!(app.messages.iter().any(|message| message.text.contains("Failed to create goal")));
        assert!(!app.messages.iter().any(|message| message.text.contains("Goal started")));
        std::fs::remove_dir(&sidecar).unwrap();
        let mut recovered = ReplApp::default();
        super::GoalCommand.run("persist this objective", &mut recovered, &engine).await;
    ...
    #[test]
    fn goal_creation_error_does_not_report_started() {
        let mut app = ReplApp::default();
        let result = super::report_goal_creation(
            &mut app,
            Err(anyhow::anyhow!("owned persistence failure")),
            "goal",
        );
        assert!(result.is_none());
        assert_eq!(app.messages.len(), 1);
        assert!(app.messages.last().unwrap().text.contains("Failed to create goal"));
        assert!(!app.messages.last().unwrap().text.contains("Goal started"));
    }
