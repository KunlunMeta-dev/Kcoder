...
impl PendingBackgroundFollowup {
    ...
    fn retain_followup_tasks(&mut self, engine: &QueryEngine) {
        ...
        for (id, event) in self.ids.drain(..).zip(self.events.drain(..)) {
            if id.is_empty() || engine.background_job_triggers_followup(&id) {
                ids.push(id);
                events.push(event);
            }
        }
...
impl ReplApp {
    ...
    fn adjust_reasoning_effort(
        ...
        engine.settings.write().unwrap().model_reasoning_effort = Some(next_effort.clone());
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let followup_events: Vec<EngineEvent> = drained
                    .iter()
                    .filter(|event| {
                        background_event_id(event)
                            .is_some_and(|id| injection_engine.background_job_triggers_followup(id))
                    })
                    .cloned()
                    .collect();
                let active_goal_id =
...
async fn try_start_next_turn(
    ...
    while let Some(queued_input) = app.pop_user_message_for_turn() {
        match queued_input {
