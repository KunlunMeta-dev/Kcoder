...
impl JournalFence {
    ...
    pub fn try_acquire(root: &Path, domain: JournalDomain) -> Result<Self> {
        Self::acquire_with_timeout(root, domain, Duration::ZERO)
    }

    /// Authority writers tolerate short contention from another resident thread/process.
    /// Readers and index builders retain nonblocking `try_acquire` semantics. The lock
    /// remains bounded, pinned to the private directory, and reverified after waiting.
    pub fn acquire_for_write(root: &Path, domain: JournalDomain) -> Result<Self> {
        Self::acquire_with_timeout(root, domain, Duration::from_secs(2))
    }

    fn acquire_with_timeout(root: &Path, domain: JournalDomain, timeout: Duration) -> Result<Self> {
        ...
        let lock = match directory.open_read_write_file(name, false) {
            Ok(file) => file,
            Err(error) if missing(&error) => match directory.open_read_write_file(name, true) {
                Ok(file) => file,
                Err(error)
                    if error
                ...
                {
                    directory.open_read_write_file(name, false)?
                }
        ...
        loop {
            match FileExt::try_lock_exclusive(&lock) {
                ...
                Err(error) => return Err(error).context("journal mutation fence is busy"),
            }
        }
        let fence = Self {
            root: directory,
...
mod tests {
    ...
    #[test]
    fn authority_fence_wait_is_bounded_and_reader_remains_nonblocking() {
        let root = tempfile::tempdir().unwrap();
        let held = JournalFence::try_acquire(root.path(), JournalDomain::ClientMetadata).unwrap();
        assert!(JournalFence::try_acquire(root.path(), JournalDomain::ClientMetadata).is_err());
        let started = std::time::Instant::now();
        assert!(
            JournalFence::acquire_with_timeout(
                root.path(),
        ...
        drop(held);
        JournalFence::acquire_for_write(root.path(), JournalDomain::ClientMetadata).unwrap();
    }
