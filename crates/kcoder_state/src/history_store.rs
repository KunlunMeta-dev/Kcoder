...
impl HistorySource {
    ...
    /// Materialize an empty accepted session using the same source/commit fence as appends.
    /// Existing history remains untouched and no transcript entry is fabricated.
    pub(super) fn materialize(&self) -> Result<()> {
        self.mutate_when(None, false, || Ok(Mutation::Materialize))
    }
    ...
    /// Inspect UUIDs and append while holding the same cross-process mutation lock.
    pub(super) fn append_unique(&self, bytes: &[u8], uuid: &str) -> Result<bool> {
        self.append_unique_when(bytes, uuid, || Ok(true))
    }

    pub(super) fn append_unique_when(
        &self,
        bytes: &[u8],
        uuid: &str,
        allowed: impl FnOnce() -> Result<bool>,
    ) -> Result<bool> {
        use std::io::BufRead;
        validate_append(bytes)?;
        let mut appended = true;
        self.mutate_when_optional(None, false, || {
            if !allowed()? {
                appended = false;
                return Ok(None);
            }
            if self.path.exists() {
                for line in std::io::BufReader::new(File::open(&self.path)?).lines() {
                    let value: serde_json::Value = serde_json::from_str(&line?)?;
                    if value.get("uuid").and_then(serde_json::Value::as_str) == Some(uuid) {
                        appended = false;
                        break;
                    }
                }
            }
            Ok(appended.then_some(Mutation::Append(bytes)))
        })?;
        Ok(appended)
    }
    ...
    fn mutate_when<'a>(
        ...
        self.mutate_when_optional(expected, recover, || prepare().map(Some))
    }

    fn mutate_when_optional<'a>(
        &self,
        expected: Option<&SourceStamp>,
        recover: bool,
        prepare: impl FnOnce() -> Result<Option<Mutation<'a>>>,
    ) -> Result<()> {
        ...
        let Some(mutation) = prepare()? else {
    ...
    fn source_commit_cold_resume_confirms_completed_pending_without_rewriting_body() {
        for kind in ["append", "replace", "metadata"] {
            ...
            match kind {
                ...
                "metadata" => {
                    assert!(old.set_goal_prepared_with_mode_and_verification("pending goal", None, None, crate::GoalMode::Standard, crate::GoalVerificationKind::Artifact).is_err());
                }
                _ => unreachable!(),
            }
