    ...
    #[test]
    fn model_selection_mode_survives_history_resume_and_snapshot_import() {
        use kcoder_types::ModelSelectionMode;
        let temp = TempDir::new().unwrap();
        let state = AppState::new(temp.path());
        let history = temp.path().join(format!("{}.jsonl", state.session_id()));
        state.with_history_path(&history);
        state.add_message(Message::user_text("selection fixture"));
        state.set_model_selection(ModelSelectionMode::FollowTargetDefault, Some("vendor-a::shared-model".into())).unwrap();
        let resumed = AppState::new(temp.path());
        resumed.resume_from_history(&history).unwrap();
        assert_eq!(resumed.model_selection_mode(), ModelSelectionMode::FollowTargetDefault);
        assert_eq!(resumed.selected_model().as_deref(), Some("vendor-a::shared-model"));
        let snapshot = temp.path().join("snapshot.json");
        std::fs::write(&snapshot, serde_json::to_vec(&resumed.snapshot()).unwrap()).unwrap();
        let imported = AppState::new(temp.path());
        imported.import_snapshot(&snapshot).unwrap();
        assert_eq!(imported.model_selection_mode(), ModelSelectionMode::FollowTargetDefault);
        assert_eq!(imported.selected_model().as_deref(), Some("vendor-a::shared-model"));
        imported.start_new_session().unwrap();
        assert_eq!(imported.model_selection_mode(), ModelSelectionMode::Explicit);
        assert_eq!(imported.selected_model(), None);
    }

    #[test]
    fn model_selection_mode_write_failure_keeps_previous_intent() {
        use kcoder_types::ModelSelectionMode;
        let temp = TempDir::new().unwrap();
        let state = AppState::new(temp.path());
        // An invalid private metadata target fails before any durable mutation.
        let blocker = temp.path().join("not-a-directory");
        std::fs::write(&blocker, b"fixture").unwrap();
        state.write_inner().session_state_path = Some(blocker.join("state.json"));
        assert!(state.set_model_selection_mode(ModelSelectionMode::FollowTargetDefault).is_err());
        assert_eq!(state.model_selection_mode(), ModelSelectionMode::Explicit);
    }

    #[test]
    fn model_identity_and_mode_roll_back_together_when_sidecar_write_fails() {
        use kcoder_types::ModelSelectionMode;
        let temp = TempDir::new().unwrap();
        let state = AppState::new(temp.path());
        state.set_model_selection(ModelSelectionMode::Explicit, Some("vendor-a::shared".into())).unwrap();
        let blocker = temp.path().join("not-a-directory");
        std::fs::write(&blocker, b"fixture").unwrap();
        state.write_inner().session_state_path = Some(blocker.join("state.json"));
        assert!(state.set_model_selection(ModelSelectionMode::FollowTargetDefault, Some("vendor-b::shared".into())).is_err());
        assert_eq!(state.model_selection_mode(), ModelSelectionMode::Explicit);
        assert_eq!(state.selected_model().as_deref(), Some("vendor-a::shared"));
    }


    #[test]
    fn goal_creation_failure_preserves_previous_memory_and_durable_sidecar() {
        let root = TempDir::new().unwrap();
        let state = AppState::new(root.path());
        let history = root.path().join("goal-commit.jsonl");
        state.with_history_path(&history);
        let previous = state.set_goal("previous goal", None);
        let sidecar = state.session_state_path().unwrap();
        let before = fs::read(&sidecar).unwrap();
        let guard = install_atomic_replace_failpoint(sidecar.clone());
        let failed = state.set_goal_prepared_with_mode_and_verification(
            "must not replace", None, None, GoalMode::Standard, GoalVerificationKind::Artifact);
        assert!(failed.is_err());
        assert_eq!(state.goal().unwrap().goal_id, previous.goal_id);
        assert_eq!(fs::read(&sidecar).unwrap(), before);
        assert!(state.goal_history().is_empty());
        drop(guard);
        let committed = state.set_goal_prepared_with_mode_and_verification(
            "replacement", None, None, GoalMode::Standard, GoalVerificationKind::Artifact).unwrap();
        assert_eq!(load_session_state(&sidecar).unwrap().goal.unwrap().goal_id, committed.goal_id);
    }
