...
impl AppState {
    ...
    pub fn upsert_task(&self, mut task: Task) {
        let _persist = self.lock_session_state_persistence();
        let (state_path, tasks) = {
            let mut inner = self.write_inner();
            if let Some(existing) = inner.tasks.get(&task.id) {
                // Run receipts are owned exclusively by the reliable State APIs.
    ...
    pub fn dead_letter_subagent_deliveries_on_close(
        ...
        if let Some(task) = &task {
            for record in &task.background_runs {
                self.suppress_background_run(&record.key)?;
            }
        }
    ...
    pub fn remove_task(&self, id: &str) -> bool {
        match self.remove_task_with_background_tombstone(id) {
            Ok(removed) => removed,
            Err(error) => {
                warn!("failed to remove background task: {error:#}");
                false
            }
        }
    }
