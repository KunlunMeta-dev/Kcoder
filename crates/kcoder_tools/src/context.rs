                    ...
                    for block in content {
                        ...
                        let process_metadata = trusted.and_then(|output| {
                            output
                                ...
                                .find_map(|metadata| match metadata {
                                    ...
                                    | ToolExecutionMetadata::ArtifactValidation(_)
                                    | ToolExecutionMetadata::BackgroundResultDelivery { .. } => {
                                        None
                                    }
                                })
                        });
                        ...
                        let artifacts = trusted
                            ...
                            .filter_map(|metadata| match metadata {
                                ...
                                ToolExecutionMetadata::Process { .. }
                                | ToolExecutionMetadata::ArtifactValidation(_)
                                | ToolExecutionMetadata::BackgroundResultDelivery { .. } => None,
                            })
                            .collect();
...
#[derive(Debug, Error)]
pub enum SpawnError {
    #[error("background run persistence failed: {message}")]
...
impl ToolContext {
    ...
    pub fn reload_skill_registry(&self) -> Result<bool, ToolError> {
        ...
        let refreshed = registry.read().unwrap().reload_with_external_dirs(
            &self.state.cwd(),
