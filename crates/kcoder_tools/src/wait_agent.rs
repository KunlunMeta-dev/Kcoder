...
struct WaitAgentResult {
    ...
    delivery_diagnostics: Option<Value>,
...
impl Tool for WaitAgentTool {
    ...
    async fn call(&self, input: Value, ctx: &ToolContext) -> Result<ToolOutput, ToolError> {
        ...
        let binding =
            crate::task::BackgroundReadBinding::capture(ctx, &id, input.run_id.as_deref())?;
        let task = binding.peek(ctx)?;
        ...
        if crate::background::is_tool_background_task_description(&task.description) {
            return Ok(ToolOutput::error(format_result(&WaitAgentResult {
                delivery_diagnostics: crate::task::background_delivery_diagnostics(&task),
                run_id: task.background_run.as_ref().map(|run| run.run_id.clone()),
                agent_id: id.clone(),
        ...
        if let Some(task) = binding.read(ctx).ok()
            ...
            match received {
                Ok(Ok(event)) => {
                    if event.id() == id && event.is_final() {
                        ...
                        return Ok(ToolOutput::text(format_result(&WaitAgentResult {
                            delivery_diagnostics: crate::task::background_delivery_diagnostics(
                                &task,
                            ),
                            run_id: task.background_run.as_ref().map(|run| run.run_id.clone()),
                            agent_id: id.clone(),
...
mod tests {
    ...
    #[tokio::test]
    async fn wait_agent_preserves_legacy_delivery_marker() {
        let state = AppState::new("/tmp");
        let mut task = kcoder_state::Task::new("job-1", "subagent");
        task.status = TaskStatus::Completed;
        ...
        assert!(
            state
                ...
                .is_some(),
            "legacy histories retain their existing notification marker semantics"
        );
