...
async fn spawn_agent_with_kind(
    ...
    if run_in_background {
        let next_action = format!(
         ...
         Its final output will be written to `{output_file_text}`. \
    ...
    let artifact_task = ctx.state.task(&id);
