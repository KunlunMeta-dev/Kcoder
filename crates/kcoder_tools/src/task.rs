...
impl Tool for TaskOutputTool {
    ...
    async fn call(&self, input: Value, ctx: &ToolContext) -> Result<ToolOutput, ToolError> {
        ...
        let Some(task) = ctx.state.task_for_output_delivery(&input.task_id) else {
        ...
        loop {
            ...
            let current = ctx
                .state
                .task_for_output_delivery(&input.task_id)
                .ok_or_else(|| {
                    ToolError::Execution("task disappeared while waiting".to_string())
                })?;
...
impl Tool for TaskStopTool {
    ...
    async fn call(&self, input: Value, ctx: &ToolContext) -> Result<ToolOutput, ToolError> {
        ...
        // Claim delivery before abort emits its terminal event so a concurrent
        // engine receiver cannot inject a redundant completion notification.
        ctx.state.mark_task_notification_injected(&id);
...
mod tests {
    ...
    #[tokio::test]
    async fn task_output_claims_terminal_notification_after_delivery() {
        let state = kcoder_state::AppState::new("/tmp");
