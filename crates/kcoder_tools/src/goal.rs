...
impl Tool for CreateGoalTool {
    ...
    async fn call(&self, input: Value, ctx: &ToolContext) -> Result<ToolOutput, ToolError> {
        ...
        let goal = ctx.state.set_goal_prepared_with_mode_and_verification(
            prepared.objective,
            prepared.objective_file,
            input.token_budget,
            kcoder_state::GoalMode::Standard,
            GoalVerificationKind::Artifact,
        ).map_err(|_| ToolError::Execution("Goal persistence failed; reload the session before retrying".into()))?;
        respond(GoalToolResponse {
            success: true,
...
mod tests {
    ...
    async fn create_goal_reports_storage_failure_without_activating_memory_only_goal() {
        ...
        let error = CreateGoalTool.call(serde_json::json!({"objective":"must be durable"}), &ctx).await.unwrap_err();
