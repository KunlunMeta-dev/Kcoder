...
impl<'a> ProviderFactory<'a> {
    ...
    pub fn build_named_isolated(
        ...
        if let Some(profile_name) = matching_profile {
            let mut settings = self.settings.clone();
            settings.apply_discovered_model(&profile_name, model)?;
            return ProviderFactory::new(&settings).build_named(provider_name, model);
        }
    ...
    pub fn local_base_url(&self) -> String {
        first_non_empty([
            self.overrides.local_base_url.clone(),
            std::env::var("VLLM_BASE_URL").ok(),
            std::env::var("SGLANG_BASE_URL").ok(),
            self.overrides.openai_base_url.clone(),
            self.configured_endpoint(first_non_empty([
                self.settings.local_base_url.clone(),
    ...
    fn build_anthropic(&self) -> Result<Arc<dyn Provider>> {
        ...
        if let Some(url) = first_non_empty([
            self.overrides.base_url.clone(),
            std::env::var("ANTHROPIC_BASE_URL").ok(),
            self.configured_endpoint(self.settings.anthropic_base_url.clone()),
        ]) {
    ...
    fn build_openai(&self, model: &str) -> Result<Arc<dyn Provider>> {
        ...
        if api_format == ApiFormat::OpenaiChatCompletions {
        ...
        if let Some(url) = first_non_empty([
            self.overrides.openai_base_url.clone(),
            self.configured_endpoint(self.settings.openai_base_url.clone()),
        ]) {
            provider = provider.with_base_url(url);
        }
    ...
    fn build_gemini(&self, model: &str) -> Result<Arc<dyn Provider>> {
        ...
        if let Some(url) =
            first_non_empty([self.configured_endpoint(self.settings.gemini_base_url.clone())])
        {
            provider = provider.with_base_url(url);
        }
    ...
    fn build_grok(&self, model: &str) -> Result<Arc<dyn Provider>> {
        ...
        if let Some(url) =
            first_non_empty([self.configured_endpoint(self.settings.grok_base_url.clone())])
        {
            provider = provider.with_base_url(url);
        }
        provider = provider.with_extra_body(self.settings.provider_extra_body.clone());
    ...
    fn build_kunlunmeta(&self) -> Result<Arc<dyn Provider>> {
        ...
        let base_url = first_non_empty([
            self.overrides.base_url.clone(),
            std::env::var("KUNLUNMETA_BASE_URL").ok(),
            self.configured_endpoint(self.settings.kunlunmeta_base_url.clone()),
        ])
