            ...
            "response.output_item.added" => {
                ...
                if item.get("type").and_then(Value::as_str) == Some("function_call") {
                    ...
                    let index = value
                        ...
                        .unwrap_or(self.tool_indices.len() as u64)
                        as usize
                        + 2;
...
impl OpenAiStreamState {
    fn process_chunk(&mut self, chunk: OpenAiChunk) {
        // Usage chunk: choices is empty and usage is present.
...
mod tests {
    ...
    async fn native_transport_observes_completion_and_incomplete_streams() {
        ...
        for (completed, anthropic, failed) in [
            (true, false, false),
            (false, false, false),
            (true, true, false),
            (false, true, false),
            ...
            assert_eq!(
                observed.protocol,
                if anthropic { "anthropic" } else { "chat" }
            );
