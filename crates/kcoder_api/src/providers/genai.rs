...
impl Provider for GenAiProvider {
    ...
    fn discover_models(&self, options: ModelDiscoveryOptions) -> ProviderModelDiscovery {
        ...
        Box::pin(async move {
            ...
            let body = super::bounded_body::read_genai(response, super::bounded_body::MODEL_LIST_LIMIT).await?;
...
fn into_genai_request(
    ...
    if let Value::Object(body) = &mut extra_body {
        ...
        if let Some(ReasoningEffort::Custom(value)) = &reasoning_effort {
            body.entry("reasoning_effort".to_string()).or_insert_with(|| Value::String(value.clone()));
        }
...
mod tests {
    ...
    fn oversized_sdk_error_is_not_copied_into_kcoder_diagnostics() {
        let mapped = super::map_genai_error(genai::Error::HttpError {
            ...
            body: serde_json::json!({"error":{"type":"SYNTHETIC_PRIVATE".repeat(100_000)}}).to_string(),
        });
        let crate::ApiErrorKind::Http { message, metadata, .. } = mapped else { panic!("HTTP facts lost") };
