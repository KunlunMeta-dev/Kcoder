...
impl LlmDebugRecorder {
    pub(crate) fn new(provider: &str, session_id: Option<&str>, url: &str, body: &str) -> Self {
        ...
        // Prune expired days at most once per process per day: the first request
        // recorded after the date rollover also reclaims old log directories.
        if !root.join(&date).exists() {
            let removed =
                prune_debug_log_directories(&root, now.date_naive(), DEBUG_LOG_RETENTION_DAYS);
            if removed > 0 {
                warn!(
                    removed,
                    retention_days = DEBUG_LOG_RETENTION_DAYS,
                    "pruned expired DEV_DEBUG request logs"
                );
            }
        }
        ...
        let path = unique_log_path(&dir, &timestamp);
...
fn unique_log_path(dir: &std::path::Path, timestamp: &str) -> PathBuf {
    let mut path = dir.join(format!("{timestamp}.json"));
    if !path.exists() {
        return path;
    }
    for index in 2.. {
        path = dir.join(format!("{timestamp}-{index}.json"));
        if !path.exists() {
            return path;
        }
    }
    unreachable!("infinite suffix loop should always return a path")
}
