...
const VISIBLE_PROJECTION_VERSION: &str = "final-transcript-v2";
...
pub(super) async fn read(
    ...
    if params.before_cursor.is_some() && !opaque {
        return super::thread_transcript(engine, params, running).await;
    }
    if params.thread_id == engine.session_id() {
        engine.state.flush_history().await?;
    }
    match try_indexed(engine, &params, running).await {
        Ok(Some(mut page)) => {
            if params.thread_id == engine.session_id() {
        ...
        _ => super::thread_transcript(engine, params, running).await,
    }
}
...
async fn load_artifacts(
    ...
    for kind in [
        ...
        for (index, entry) in entries.enumerate() {
            ...
            let limit = if kind == "turn-client-messages" || kind == "turn-outcomes" {
                64 * 1024
            } else {
                1024 * 1024
            };
