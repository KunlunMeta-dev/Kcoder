...
#[derive(Debug)]
pub(super) struct StorageBusyError;
impl std::fmt::Display for StorageBusyError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str("storage is busy; wait for active tasks to finish and refresh")
    }
}
impl std::error::Error for StorageBusyError {}
...
#[derive(Debug)]
pub(super) struct StorageScanCancelled;
impl std::fmt::Display for StorageScanCancelled {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str("storage scan cancelled")
    }
}
impl std::error::Error for StorageScanCancelled {}

pub(super) fn read_report(cancel: &CancellationToken) -> Result<StorageReport> {
    report_for_with_cancel(&config_root()?, cancel)
}
...
fn report_for(root: &Path) -> Result<StorageReport> {
    report_for_with_cancel(root, &CancellationToken::new())
}

fn report_for_with_cancel(root: &Path, cancel: &CancellationToken) -> Result<StorageReport> {
    let started = Instant::now();
    let mut partial = false;
    let mut skipped_entries = 0usize;
    let mut visited = 0usize;
    let mut oldest_day: Option<String> = None;
    anyhow::ensure!(!cancel.is_cancelled(), StorageScanCancelled);
    let mut buckets = vec![
        StorageBucket {
            id: "debug-logs".into(),
            bytes: 0,
    ...
    'scan: while let Some(dir) = stack.pop() {
        anyhow::ensure!(!cancel.is_cancelled(), StorageScanCancelled);
        if started.elapsed() >= Duration::from_secs(10) || visited >= 500_000 {
            partial = true;
            break;
        }
            ...
            if id == "debug-logs" {
                    ...
                    .and_then(|value| value.as_os_str().to_str())
                {
                    if day.len() == 8
                        ...
                        && oldest_day.as_deref().is_none_or(|old| day < old)
                    {
                        oldest_day = Some(day.to_owned());
                    }
                }
            }
    ...
    let logs = buckets
        .iter()
        .find(|bucket| bucket.id == "debug-logs")
        .expect("debug log bucket");
    ...
    let credentials = credentials_status(root)?;
    anyhow::ensure!(!cancel.is_cancelled(), StorageScanCancelled);
    Ok(StorageReport {
...
fn bucket_for(relative: &Path) -> &'static str {
    ...
    if joined_has("turn-file-changes")
        && parts.iter().any(|part| {
            part.starts_with("snapshot-repository-") || part.starts_with("revert-repository-")
        })
    {
...
pub(super) fn clean(target: &str, confirm: bool) -> Result<StorageCleanResult> {
    ...
    clean_at(&config_root()?, target)
}

fn clean_at(root: &Path, target: &str) -> Result<StorageCleanResult> {
    let mut errors = Vec::new();
    let (removed_bytes, removed_files) = match target {
        TARGET_DEBUG_LOGS => {
            let logs = root.join("logs").join("llm-request");
            if logs.exists() {
                let _lease = kcoder_api::providers::debug_log::try_debug_log_cleanup_at(&logs)?
                    .ok_or(StorageBusyError)?;
                remove_directory_with_outcome(&logs, &mut errors)
            } else {
                (0, 0)
            }
        }
        TARGET_TURN_SNAPSHOTS => {
            ...
            let repositories = snapshot_repositories(&root)?;
            // Acquire every session before deleting anything. A stale preview
            // must not partially clean one session and then fail on a busy one.
            let mut parents = std::collections::BTreeSet::new();
            let mut leases = Vec::new();
            for dir in &repositories {
                let parent = dir.parent().context("snapshot repository has no parent")?;
                if parents.insert(parent.to_path_buf()) {
                    leases.push(super::snapshot_leases::cleanup(parent)?.ok_or(StorageBusyError)?);
                }
            }
            for dir in repositories {
                let (dir_bytes, dir_files) = remove_directory_with_outcome(&dir, &mut errors);
                bytes += dir_bytes;
                files += dir_files;
            }
            (bytes, files)
...
// A failed recursive removal may already have reclaimed some files. Report
// the observed remaining footprint and an explicit partial failure, never imply
// that the entire previewed bucket was deleted.
fn remove_directory_with_outcome(path: &Path, errors: &mut Vec<String>) -> (u64, usize) {
    let before = directory_footprint(path);
    match remove_directory(path) {
        Ok(removed) => removed,
        Err(error) => {
            let after = directory_footprint(path);
    ...
    while let Some(dir) = stack.pop() {
        ...
        for entry in entries.flatten() {
            ...
            if name.starts_with("snapshot-repository-") || name.starts_with("revert-repository-") {
                ...
                let lease = leases
                    .entry(parent.clone())
                    .or_insert_with(|| super::snapshot_leases::cleanup(&parent).ok().flatten());
                if lease.is_none() {
                    protected_bytes += directory_footprint(&entry.path()).0;
                    continue;
                }
...
mod tests {
    ...
    #[test]
    fn cancelled_scans_stop_and_incomplete_scans_are_not_reported_as_complete() {
        let root = tempfile::tempdir().unwrap();
        let token = CancellationToken::new();
        token.cancel();
        assert!(
            report_for_with_cancel(root.path(), &token)
                .unwrap_err()
                .is::<StorageScanCancelled>()
        );
        let missing = root.path().join("missing");
        let report = report_for(&missing).unwrap();
        assert_eq!(report.partial, Some(true));
        assert_eq!(report.skipped_entries, 1);
        assert!(report.scanned_at.is_some());
    }

    #[test]
    fn removal_failure_is_retained_as_a_partial_outcome() {
        let root = tempfile::tempdir().unwrap();
        let file = root.path().join("not-a-directory");
        fs::write(&file, "retained").unwrap();
        let mut errors = Vec::new();
        assert_eq!(remove_directory_with_outcome(&file, &mut errors), (0, 0));
    ...
    fn buckets_route_config_paths_to_their_slices() {
        for file in ["turn.patch", "turn.json", ".snapshot-repositories.lock"] {
            assert_eq!(
                bucket_for(
                    &Path::new("projects/demo/client-sessions/s1/turn-file-changes").join(file)
                ),
                "session-data"
            );
        }
        assert_eq!(
            bucket_for(Path::new(
                "projects/demo/client-sessions/s1/turn-file-changes/revert-repository-x/objects/blob"
            )),
            "turn-snapshots"
        );
        assert_eq!(
...
mod lease_cleanup_tests {
    ...
    fn repository(root: &Path, session: &str, bytes: usize) -> PathBuf {
        let path = root
            .join("projects")
            .join(session)
            .join("turn-file-changes")
            .join("snapshot-repository-fixture");
        fs::create_dir_all(path.join("objects")).unwrap();
        fs::write(path.join("objects/blob"), vec![b'x'; bytes]).unwrap();
        path
    }

    #[test]
    fn cleanup_rechecks_after_preview_and_refuses_partial_deletion() {
        let root = tempfile::tempdir().unwrap();
        let idle = repository(root.path(), "idle", 10);
        let busy = repository(root.path(), "busy", 20);
        assert!(report_for(root.path()).unwrap().total_bytes >= 30);
        let lease = crate::app_server::snapshot_leases::active(busy.parent().unwrap()).unwrap();
        assert!(clean_at(root.path(), TARGET_TURN_SNAPSHOTS).is_err());
        assert!(idle.join("objects/blob").exists());
        assert!(busy.join("objects/blob").exists());
        drop(lease);
        let result = clean_at(root.path(), TARGET_TURN_SNAPSHOTS).unwrap();
    ...
    #[test]
    fn automatic_reclamation_preserves_active_repositories_even_over_budget() {
        let root = tempfile::tempdir().unwrap();
        let idle = repository(root.path(), "idle", 10);
        let busy = repository(root.path(), "busy", 20);
        let lease = crate::app_server::snapshot_leases::active(busy.parent().unwrap()).unwrap();
        ...
        let result = enforce_turn_snapshot_budget_at_with(
        ...
        assert!(busy.join("objects/blob").exists());
        assert!(
            !busy.join("HEAD").exists(),
            "GC must not repair a live repository either"
        );
