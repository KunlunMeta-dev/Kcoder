...
pub(super) async fn upsert(
    ...
    tokio::task::spawn_blocking(move || commit(&path, &baseline, input, Some(&snapshot))).await??;
    request(
        &engine,
        &Ok(state),
...
fn profile_value(
    ...
    models.insert(model.to_owned(), serde_json::from_value(model_value)?);
...
fn save(path: &Path, baseline: &Settings, input: ProviderUpsertParams) -> Result<()> {
    commit(path, baseline, input, None)
}
...
fn commit(
    ...
    kcoder_config::update_settings_file(path, |user| {
        ...
        if let Some(key) = &key {
            ...
            .context("Failed to save API credentials")?;
        }
        Ok(())
    })
    .context("Failed to save provider settings; credentials may already have been updated")?;
    Ok(())
}
