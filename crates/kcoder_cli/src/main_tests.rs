...
fn required_project_skill_preflight_fails_before_model_until_root_is_trusted() {
    ...
    .unwrap();
    // Project discovery is not authorization: the registry rechecks persisted
    // folder trust when resolving active skills. Keep that store private to
    // this fixture instead of depending on the test process's user profile.
    let config_dir = temp.path().join("config");
    let mut trust = kcoder_config::FolderTrustStore::load(&config_dir);
    trust.revoke(&project).unwrap();
    let mut untrusted = SkillRegistry::load_project_only(&project).unwrap();
    untrusted.require_folder_trust(&project.join(".kcoder/skills"), &project, Some(config_dir));
    assert!(untrusted.is_trust_denied("ci-triage"));
    ...
    assert!(trusted.get_active("ci-triage").is_some());
    ...
    .unwrap();
    trust.revoke(&project).unwrap();
    assert!(trusted.is_trust_denied("ci-triage"));
    assert!(trusted.get_active("ci-triage").is_none());
}
...
fn auth_logout_accepts_document_only_ids_and_deletes_the_store_entry() {
    ...
    let remaining: serde_json::Value =
        serde_json::from_slice(&std::fs::read(&paths.credentials).unwrap()).unwrap();
    assert_eq!(remaining["Orphan-Id"], serde_json::json!({"type":"revoked"}));
...
#[test]
fn auth_logout_revokes_legacy_fallback_without_a_stored_key_and_is_idempotent() {
    let (_temp, paths) = logout_fixture();
    let mut settings = Settings::default();
    settings.api_key = Some("legacy-fixture".into());
    let backend = RecordingCredentialBackend::default();
    logout_provider_credential(&paths, "fallback-provider", &settings, &backend).unwrap();
    logout_provider_credential(&paths, "fallback-provider", &settings, &backend).unwrap();
    let store = CredentialStore::load_from(&paths.credentials).unwrap();
    assert!(store.revoked.contains("fallback-provider"));
    assert!(!store.credentials.contains_key("fallback-provider"));
