...
struct AgentSteerResponseGate {
    ...
    deferred_applied: Vec<EngineEvent>,
}
...
impl ResidentTurnState {
    ...
    fn resolve_response(&self, response: &Value) -> bool {
        resolve_server_response(response, &self.pending_approvals, &self.pending_questions)
    }
...
fn resolve_server_response(
    ...
    if let Some(response_tx) = pending_approvals
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner())
        .remove(&request_id)
    {
        let parsed = if let Some(error) = response.get("error") {
            ...
            serde_json::from_value::<ApprovalResponse>(
    ...
    if let Some(response_tx) = pending_questions
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner())
        .remove(&request_id)
    {
        let parsed = if let Some(error) = response.get("error") {
            ...
            serde_json::from_value::<QuestionResponse>(
                response.get("result").cloned().unwrap_or(Value::Null),
            )
    ...
    fn project_with_client_message_id(
        ...
        match event {
            ...
            EngineEvent::AssistantMessageStarted => {
                let id = format!("{}-assistant-{}", self.turn_id, self.next_item);
                self.next_item += 1;
...
async fn queue_recovered_background_followups(
    ...
    for (id, summary) in recovered {
        if should_trigger(&id) {
            ...
            queue_background_followup_once(state, id, run_started_at_ms, summary, true).await;
        }
    }
}
...
async fn project_engine_background_event(
    ...
    if let EngineEvent::SubagentSteerApplied { agent_id, .. } = &event {
        let mut state = state.lock().await;
        if let Some(gate) = state.steer_response_gates.get_mut(agent_id) {
            gate.deferred_applied.push(event);
            return Ok(());
        }
    }
    ...
    let routed = {
        ...
        if let EngineEvent::BackgroundJobAssociated { tool_call_id, .. } = &event {
            ...
            let Some(context) = state.jobs.get(&id).cloned() else {
                let pending = state.pending.entry(id).or_default();
                let duplicate_start = matches!(event, EngineEvent::BackgroundJobStarted { .. })
                    && pending
                        .iter()
                        .any(|event| matches!(event, EngineEvent::BackgroundJobStarted { .. }));
                let must_deliver = matches!(event, EngineEvent::SubagentSteerApplied { .. });
                if pending.len() < 64 && !duplicate_start {
                    pending.push(event);
                } else if must_deliver {
                    if let Some(index) = pending.iter().position(|event| {
                        matches!(event, EngineEvent::BackgroundJobProgress { .. })
                    }) {
    ...
    let result = async {
        for (context, event) in routed {
            let client_message_id =
                if let EngineEvent::SubagentSteerApplied { message_id, .. } = &event {
                    state
                        .lock()
                        .await
                        .steer_correlations
    ...
    if retired {
        state
            ...
            .await
            .steer_correlations
            .retain(|_, correlation| correlation.agent_id != id);
    }
    result
}
            ...
            received = receiver.recv() => {
                match received {
                    Ok(event) => {
                        ...
                        if matches!(event, BackgroundJobEvent::Completed { .. }
                        ...
                        if let Some((id, summary)) = terminal_summary
                            && engine.background_job_triggers_followup(&id)
                            && !engine.turn_driver_active()
                        {
                            let run_started_at_ms = engine
                                .state
...
async fn run_background_followup_turn(
    ...
    if !stop_followup && !cancel.is_cancelled() {
        ...
        if let Err(error) = append_background_followup_message(&engine, &followups, nudge) {
            status = "failed";
                    ...
                    .is_ok()
            {
                ...
                if !allowed {
                    ...
                    let pending_summary = if has_pending_followup {
                        take_ready_background_followup(&followups, false, |id| {
                            engine.background_job_triggers_followup(id)
                        })
                        .await
                    } else {
                    ...
                    let summary = if let Some(summary) = pending_summary {
                        summary
                    ...
                    let turn_id = if summary.starts_with("[scheduled task ")
...
impl ConnectionState {
    ...
    fn initialize(&mut self, id: Value, params: &Value, engine: &QueryEngine) -> Value {
        ...
        let result = InitializeResult {
            ...
            capabilities: server_capabilities(engine.state.history_path().is_some()),
...
pub async fn run(
    ...
    let provider_settings_state = provider_settings::capture_state(&workspace_engine)
        .map(|state| state.with_session_reload(engine_factory.supports_session_reload()));
    ...
    loop {
        ...
        if method.is_empty() && !id.is_null() {
            for turn_state in thread_manager.turn_states() {
                if turn_state.resolve_response(&request) {
                    break;
                }
            }
            continue;
        ...
        if method == method::THREAD_READ_INDEXED {
            ...
            indexed_read_tasks.spawn(async move {
                ...
                let response = match indexed_transcript::read(&target, params, &running).await {
        ...
        match method {
            ...
            method::DIAGNOSTICS_STORAGE_READ => match storage_diagnostics::read_report() {
                Ok(result) => {
                    send(
                        &outbound_tx,
                        success_response(id, serde_json::to_value(result)?),
            ...
            method::DIAGNOSTICS_STORAGE_CLEAN => {
                let result = serde_json::from_value::<StorageCleanParams>(params.clone())
                    .map_err(anyhow::Error::from)
                    .and_then(|params| storage_diagnostics::clean(&params.target, params.confirm));
                match result {
                    Ok(result) => {
                        send(
                            &outbound_tx,
                ...
                if let Some(thread_id) = params.get("threadId").and_then(Value::as_str) {
                    match thread_manager.engine(thread_id) {
                        Some(thread) => {
                            send(
                                &outbound_tx,
                                success_response(id, configured_models_response(&thread)),
                            )
                            .await?
                        }
                        None => {
                    ...
                    match engine_factory.current_settings() {
                        Ok(settings) => {
                            send(
                                &outbound_tx,
                                success_response(
                                    ...
                                    configured_model_catalog_response(
                                        QueryEngine::configured_model_profiles_for_settings(
                                            &settings,
                                        ),
                                    ),
                                ),
                    ...
                    .context("invalid thread/read params")
                {
                    Ok(params) => {
                        let target_engine = thread_manager
                            .engine(&params.thread_id)
                            .unwrap_or_else(|| workspace_engine.clone());
                        thread_transcript(&target_engine, params, &running_thread_ids).await
                    }
                    Err(error) => Err(error),
                };
                match result {
                    ...
                    Ok(prepared_resume) => match prepared_resume {
                        ...
                        PreparedThreadResume::Persisted {
                            ...
                            let response = success_response(
                                id,
                                json!({"thread": thread_snapshot(&engine, false)}),
                            );
                            send(&outbound_tx, response).await?;
                        }
                    },
                ...
                if params.get("ephemeral").and_then(Value::as_bool) == Some(true) {
                    let result: Result<Value> = async {
                        ...
                        let mut messages = if params.last_turn_id.is_empty() {
                            ...
                            client_fork_transcript_through_turn(
                                &kcoder_state::load_transcript_history(&path)?, &params.last_turn_id,
                            )?
                        };
                        anyhow::ensure!(!messages.is_empty(), "source thread has no completed context to fork");
                        let mut turn_state = ResidentTurnState::new(
                            outbound_tx.clone(), permission_mode,
                            Arc::clone(&next_question_id), Arc::clone(&next_approval_id),
                        );
                        ...
                        if thread_manager.is_ephemeral(&params.thread_id) {
                            ...
                            target_engine.and_then(|target_engine| {
                                ...
                                .and_then(|_| {
                                    ...
                                    let mut messages = client_fork_transcript_through_turn(
                                        &transcript,
                                        &params.last_turn_id,
                                    )?;
            ...
            "turn/start" => {
                ...
                if let Some(model) = turn_params.model.as_deref()
                    && let Err(error) = engine.select_client_model(model)
                {
                    turn_running.store(false, Ordering::SeqCst);
                    background_followups.notify.notify_waiters();
                    send(&outbound_tx, error_response(id, -32602, &error.to_string())).await?;
                ...
                if let Some(reasoning_effort) = turn_params.reasoning_effort.as_deref()
                ...
                if (turn_params.proxy_url.is_some() || turn_params.service_tier.is_some())
                ...
                let Some(prompt) = prompt_from_params(&params) else {
                    turn_running.store(false, Ordering::SeqCst);
                    background_followups.notify.notify_waiters();
                    send(
                        &outbound_tx,
                ...
                let turn_id = format!("turn-{}", completed_turns.saturating_add(1));
                let thread_id = engine.session_id();
                ...
                if let Err(error) = save_turn_client_message_id(
                    &engine,
                    &thread_id,
                    &turn_id,
                ...
                send(
                    &outbound_tx,
                    success_response(id, json!({"turn": {"id": turn_id, "threadId": thread_id, "status": "running"}})),
                )
                .await?;
                ...
                *approval_context
                ...
                let handle = tokio::spawn(async move {
                    ...
                    let mut stream = turn_execution::stream(
                        task_engine.clone(),
                        model_message,
                        prompt,
...
fn configured_models_response(engine: &QueryEngine) -> Value {
    configured_model_catalog_response(engine.configured_model_profiles())
}
    ...
    let providers = groups
        ...
        .map(|(provider_id, profiles)| {
            ...
            let model = json!({
                ...
                "providerId": profile.profile_name,
                "providerName": profile.provider,
                "providerType": "provider",
                "providerCurrent": current,
                "description": Value::Null,
                "hidden": false,
                "isDefault": profile.current,
                "defaultReasoningEffort": if profile.reasoning { Value::String("medium".into()) } else { Value::Null },
...
fn validate_thread_parent(
    ...
    client_fork_transcript_through_turn(&entries, &parent.last_turn_id)
...
async fn thread_transcript(
    ...
    let thread: Thread = serde_json::from_value(thread_value)?;
...
fn client_fork_transcript_through_turn(
    entries: &[kcoder_state::HistoryEntry],
    turn_id: &str,
) -> Result<Vec<kcoder_types::Message>> {
    let requested_turn = turn_id
        .strip_prefix("turn-")
        .and_then(|value| value.parse::<usize>().ok())
    ...
    for entry in entries {
        if kcoder_engine::agent::is_real_user_message(&entry.message) {
            ...
            if completed_turns > requested_turn {
                break;
            }
        }
