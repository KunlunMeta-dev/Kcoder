    ...
    match action {
        ...
        AuthAction::Login {
            ...
            if use_keyring {
                ...
                credentials.save_to(&paths.credentials)?;
                println!(
...
fn import_dotenv_credentials(
    ...
    for (id, credential) in available {
        ...
        credentials.set_api_key(&id, api_key.to_string())?;
        imported.push(id);
    }
    if !imported.is_empty() {
        credentials.save_to(&paths.credentials)?;
...
fn logout_provider_credential(
    ...
    let mut credentials = CredentialStore::load_from(&paths.credentials)?;
    let stored = credentials.credentials.get(&credential_id).cloned();
...
fn run_auth_status(_cli: &Cli, settings: &Settings, paths: &ConfigPaths) -> Result<()> {
    println!("kcoder auth");
    println!("==============");
