...
#[test]
fn cron_diagnostics_expose_removed_one_shot_and_receipt_cleanup_preserves_schedule() {
    let root = tempfile::tempdir().unwrap();
    let workspace = root.path().join("workspace");
    std::fs::create_dir_all(&workspace).unwrap();
    let settings = root.path().join("settings.json");
    write_test_settings(&settings);
    let mut server = TestAppServer::builder(&workspace, &settings).config_dir(&root.path().join("config")).spawn();
    let initialized = server.initialize(1);
    assert_eq!(initialized["result"]["capabilities"]["experimental"]["cronDeliveryDiagnosticsV1"], true);
    server.send(json!({"id":2,"method":"cron/create","params":{
        "prompt":"keep periodic fixture", "confirmed":true, "jitterSeconds":0,
        "schedule":{"kind":"every","every_seconds":3600}
    }}));
    let created = server.response(2);
    assert!(created.get("error").is_none(), "{created}");
    let job = created["result"]["job"]["id"].clone();
    let path = workspace.join(".kcoder/cron/jobs.json");
    let mut persisted: Value = serde_json::from_slice(&std::fs::read(&path).unwrap()).unwrap();
    let jobs_before = persisted["jobs"].clone();
    let now = chrono::Utc::now().to_rfc3339();
    // Reproduce the durable post-crash evidence; actual process crash cut points
    // are exercised by cron::delivery::tests with the same v3 storage contract.
    persisted["receipts"] = json!([{
        "job_id":"removed-one-shot", "scheduled_at":now, "recorded_at":now,
        "session_id":"kcoder-project-automations", "owner_instance":"dead-fixture", "phase":"queued"
    }, {
        "job_id":job, "scheduled_at":now, "recorded_at":now,
        "session_id":"kcoder-project-automations", "owner_instance":"dead-fixture", "phase":"dispatch_pending"
    }]);
    std::fs::write(&path, serde_json::to_vec(&persisted).unwrap()).unwrap();
    server.send(json!({"id":3,"method":"cron/list","params":{}}));
    let listed = server.response(3);
    assert_eq!(listed["result"]["jobs"], jobs_before);
    assert_eq!(listed["result"]["deliveryDiagnostics"]["receipts"][0]["status"], "delivery_unknown");
    assert_eq!(listed["result"]["deliveryDiagnostics"]["receipts"][0]["jobId"], "removed-one-shot");
    assert_eq!(listed["result"]["deliveryDiagnostics"]["receipts"][0]["executionConfirmed"], false);
    server.send(json!({"id":4,"method":"cron/delete","params":{"jobId":job,"receiptsOnly":true}}));
    assert!(server.response(4).get("error").is_some());
    for (id, target) in [(5, job), (6, json!("removed-one-shot"))] {
        server.send(json!({"id":id,"method":"cron/delete","params":{"jobId":target,"receiptsOnly":true,"confirmed":true}}));
        let cleared = server.response(id);
        assert_eq!(cleared["result"]["schedulePreserved"], true, "{cleared}");
        assert_eq!(cleared["result"]["deleted"], true, "{cleared}");
    }
    server.send(json!({"id":7,"method":"cron/list","params":{}}));
    let after = server.response(7);
    assert_eq!(after["result"]["jobs"], jobs_before);
    assert!(after["result"]["deliveryDiagnostics"]["receipts"].as_array().unwrap().is_empty());
