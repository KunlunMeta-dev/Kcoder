#[test]
#[cfg(unix)]
fn new_sessions_reload_extensions_while_resident_model_catalogs_stay_live() {
    ...
    assert!(old_catalog.get("error").is_none(), "{old_catalog}");
    assert!(
        old_catalog["result"]
            .to_string()
            .contains("updated-model")
    );
    // The execution selection remains a session snapshot; the discoverable
    // catalog is current so users can select newly configured models.
    server.send(json!({"id":40,"method":"thread/read","params":{"threadId":old_thread}}));
    let unchanged = server.response(40);
    assert_eq!(unchanged["result"]["thread"]["model"], initial["result"]["thread"]["model"]);
    ...
    assert!(server.response(9).get("error").is_some(), "invalid live config must not silently fall back to a stale catalog");
    std::fs::write(&settings_path, serde_json::to_vec(&settings).unwrap()).unwrap();
...
fn silent_user_prompt_plugin_hook_allows_a_complete_turn() {
    ...
    // Discovery trust does not replace the live execution trust receipt.
    kcoder_config::FolderTrustStore::load(&temp.path().join("config"))
        .trust(temp.path())
        .unwrap();
