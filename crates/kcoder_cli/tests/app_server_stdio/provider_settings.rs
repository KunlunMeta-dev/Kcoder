...
fn stdio_provider_deletion_preserves_overlay_and_credentials_choice_across_restart() {
    ...
    reader.shutdown_successfully();
    let keys: Value = serde_json::from_slice(&std::fs::read(&credential_path).unwrap()).unwrap();
    // Explicit removal records a tombstone so environment/legacy credentials
    // cannot silently restore the deleted key on the next load.
    assert_eq!(keys["beta"], json!({"type": "revoked"}));
    let credentials = kcoder_config::CredentialStore::load_from(&credential_path).unwrap();
    assert!(!credentials.has_api_key("beta"));
    assert!(credentials.revoked.contains("beta"));
    assert!(keys.get("alpha").is_some());
