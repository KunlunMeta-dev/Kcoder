...
#[test]
fn moa_provider_retains_the_host_credential_factory() {
    struct Source(std::sync::Mutex<Vec<Settings>>);
    impl crate::ClientModelConfiguration for Source {
        fn settings(&self, _: &str) -> anyhow::Result<Settings> {
            anyhow::bail!("MoA must keep its model snapshot")
        }
        fn provider(&self, settings: &Settings) -> anyhow::Result<Arc<dyn kcoder_api::Provider>> {
    ...
    profile.default_model = "moa-fixture".into();
    profile.models.clear();
    settings.providers.insert("moa-fixture".into(), profile);
    let source = Source(std::sync::Mutex::new(Vec::new()));
    let model = MoaModelConfig::new("moa-fixture", "moa-fixture");
    crate::moa_runtime::build_moa_provider(&settings, &model, Some(&source)).unwrap();
