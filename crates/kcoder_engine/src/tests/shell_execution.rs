...
async fn shell_sandbox_denial_retries_without_sandbox_after_approval() {
    ...
    let mut engine = QueryEngine::new_with_folder_trust(
    ...
    // Discovery permission is not live Hook authorization. Bind only this
    // fixture's project Hooks to its private trust store; never alter the test
    // process's user profile or disable the production revocation guard.
    let config_dir = tmp.path().join("fixture-trust-config");
    let mut trust = kcoder_config::FolderTrustStore::load(&config_dir);
    trust.revoke(tmp.path()).unwrap();
    let document: Value = serde_json::from_slice(
        &std::fs::read(kcoder_dir.join("settings.json")).unwrap(),
    ).unwrap();
    let hooks: kcoder_hooks::HooksSettings = serde_json::from_value(document["hooks"].clone()).unwrap();
    let mut source = kcoder_hooks::HookSource::project_settings(
        &kcoder_dir.join("settings.json"), tmp.path(),
    );
    source.required_trust.as_mut().unwrap().config_dir = Some(config_dir);
    engine.hook_registry = kcoder_hooks::HookRegistry::from_matchers(
        hooks.into_matchers().into_iter().map(|(event, mut matcher)| {
            matcher.source = Some(source.clone());
            (event, matcher)
        }).collect(),
    );
    let (denied, decision, _, _) = engine.execute_tool(
        "before-trust", "bash", serde_json::json!({"command": "printf sandbox-ok"}),
        &kcoder_permissions::AutoAllowPrompt,
    ).await.unwrap();
    assert_eq!(decision, PermissionDecision::Deny);
    assert!(tool_output_text(&denied).contains("hook directory trust was revoked"));
    trust.trust(tmp.path()).unwrap();
    ...
    assert_eq!(decision, PermissionDecision::Allow, "{}", tool_output_text(&output));
