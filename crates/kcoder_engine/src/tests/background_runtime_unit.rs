...
#[tokio::test]
async fn old_run_terminal_delivers_only_its_own_receipt() {
    let root = tempfile::tempdir().unwrap();
    let engine = TestEngineBuilder::new(root.path()).build();
    let mut task = Task::new("reused", "fixture");
    task.kind = TaskKind::Subagent;
    task.notify_parent_on_completion = true;
    task.delivery = kcoder_state::TaskDelivery::Background;
    engine.state.upsert_task(task);
    let old = engine.state.begin_background_run("reused").unwrap();
    let identity = kcoder_types::BackgroundEventIdentity::terminal(old.clone());
    engine
        .state
        .commit_background_terminal(&identity, TaskStatus::Completed, None)
        .unwrap();
    let new = engine.state.begin_background_run("reused").unwrap();
    let event = BackgroundJobEvent::Scoped {
        identity,
        event: Box::new(BackgroundJobEvent::Completed {
            id: "reused".into(),
            output: ToolOutput::text("old"),
        }),
    };
    let first = engine
        .apply_background_event_with_hooks(event.clone())
        .await;
    assert!(
        first
            .iter()
            .any(|event| event.background_identity().is_some_and(|id| id.run == old))
    );
    assert!(
        engine
            .state
            .background_run_record(&old)
            .unwrap()
    ...
    assert!(
        engine
            .apply_background_event_with_hooks(event)
            .await
            .is_empty()
    );
    let identity = kcoder_types::BackgroundEventIdentity::terminal(new.clone());
    engine
        .state
        .commit_background_terminal(&identity, TaskStatus::Completed, None)
        .unwrap();
    engine
        .apply_background_event_with_hooks(BackgroundJobEvent::Scoped {
            identity,
            event: Box::new(BackgroundJobEvent::Completed {
                id: "reused".into(),
                output: ToolOutput::text("new"),
            }),
        })
        .await;
    assert!(
    ...
    assert_eq!(
        engine
            ...
            .messages()
            .iter()
            .filter(|message| message.preview(1000).contains("<subagent_notification"))
            .count(),
        2
    );
}
...
#[tokio::test]
async fn running_run_rejects_uncommitted_scoped_terminal() {
    let root = tempfile::tempdir().unwrap();
    let engine = TestEngineBuilder::new(root.path()).build();
    let mut task = Task::new("running", "fixture");
    task.kind = TaskKind::Subagent;
    engine.state.upsert_task(task);
    let key = engine.state.begin_background_run("running").unwrap();
    let events = engine
        .apply_background_event_with_hooks(BackgroundJobEvent::Scoped {
            identity: kcoder_types::BackgroundEventIdentity::terminal(key.clone()),
            event: Box::new(BackgroundJobEvent::Completed {
                id: "running".into(),
                output: ToolOutput::text("stale"),
            }),
        })
        .await;
...
#[derive(Debug)]
struct DuplicateCompletionProvider {
    calls: Arc<std::sync::atomic::AtomicUsize>,
    sender: tokio::sync::broadcast::Sender<BackgroundJobEvent>,
    duplicate: Arc<Mutex<Option<BackgroundJobEvent>>>,
}
impl Provider for DuplicateCompletionProvider {
    fn name(&self) -> &'static str {
    ...
    fn stream_messages(
        ...
        let duplicate = self.duplicate.lock().unwrap().clone();
        Ok(Box::pin(async_stream::stream! {
            yield Ok(StreamEvent::ContentBlockStart { index: 0, content_block: ContentBlock::Text { text: String::new() } });
            yield Ok(StreamEvent::ContentBlockDelta { index: 0, delta: kcoder_types::ContentDelta::TextDelta { text: "done".into() } });
            if call == 0 {
                if let Some(duplicate) = duplicate { let _ = sender.send(duplicate); }
                tokio::time::sleep(Duration::from_millis(10)).await;
            }
...
#[tokio::test]
async fn duplicate_deferred_completion_does_not_sample_the_parent_again() {
    ...
    let mut engine = test_engine(
        Arc::new(DuplicateCompletionProvider {
            calls: calls.clone(),
            sender,
    ...
    engine.background_job_rx = Arc::new(tokio::sync::Mutex::new(receiver));
    let mut task = Task::new("already-delivered", "fixture");
    task.kind = TaskKind::Subagent;
    task.delivery = kcoder_state::TaskDelivery::Background;
    task.notify_parent_on_completion = true;
    engine.state.upsert_task(task);
    let key = engine
        .state
        .begin_background_run("already-delivered")
        .unwrap();
    let identity = kcoder_types::BackgroundEventIdentity::terminal(key);
    engine
        .state
        .commit_background_terminal(&identity, TaskStatus::Completed, None)
        .unwrap();
    ...
    engine
        .apply_background_event_with_hooks(event.clone())
        .await;
    *duplicate.lock().unwrap() = Some(event);
    engine
        .state
        .add_message(Message::user_text("Summarize the completed result."));
    let prompt = kcoder_permissions::AutoAllowPrompt;
    let _ = tokio::time::timeout(
        Duration::from_secs(5),
        engine.run_turn_stream(&prompt).collect::<Vec<_>>(),
    )
    .await
    .unwrap();
...
#[test]
fn modern_run_rejects_legacy_terminal_without_run_identity() {
    let root = tempfile::tempdir().unwrap();
    let engine = TestEngineBuilder::new(root.path()).build();
    let mut task = Task::new("scoped-only", "fixture");
    task.kind = TaskKind::Subagent;
    engine.state.upsert_task(task);
    let run = engine.state.begin_background_run("scoped-only").unwrap();
    assert!(
        QueryEngine::apply_background_event(
            &engine.state,
            BackgroundJobEvent::Completed {
                id: "scoped-only".into(),
                output: ToolOutput::text("old unscoped event")
            }
        )
        .is_none()
    );
