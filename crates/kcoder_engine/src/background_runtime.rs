#[path = "background_delivery_runtime.rs"]
mod delivery;
use super::*;
...
impl QueryEngine {
    ...
    pub(super) fn drain_background_events(&self) -> Vec<BackgroundJobEvent> {
        self.background_jobs.retry_terminal_publications();
        ...
        events.retain(|event| {
            if matches!(
                ...
                if let Some(identity) = event.background_identity() {
                    return announced.insert(identity.run.clone());
                }
            }
            true
        });
    ...
    /// Synchronous lifecycle projection for legacy callers. Scoped events are
    /// projected once but never claim a durable parent result through this path.
    /// Use `flush_background_jobs_with_hooks().await` to commit run-scoped
    /// notifications and receive their persisted delivery acknowledgement.
    ...
    pub(super) fn apply_background_event(
        ...
        if let BackgroundJobEvent::Scoped { identity, event } = event {
            let record = state.background_run_record(&identity.run)?;
            if event.is_final() && record.terminal.as_ref() != Some(&identity) {
                return None;
            }
            return Some(EngineEvent::BackgroundScoped {
                identity,
        ...
        if state
            .task(event.id())
            .is_some_and(|task| task.background_run.is_some())
        {
            tracing::warn!(
                agent_id = event.id(),
                "discarded unscoped event for a run-scoped task"
            );
            return None;
        }
        match &event {
            BackgroundJobEvent::Scoped { .. } => unreachable!("handled above"),
    ...
    fn recover_final_background_events(&self) -> Vec<BackgroundJobEvent> {
        // A crash may leave the terminal task snapshot committed before its outbox.
        // Freeze that same run before replay; never borrow a newer run's result.
        for task in self.state.tasks().into_values() {
            if let Some(run) = &task.background_run {
                if matches!(
                    task.status,
                ...
                {
                    let identity = kcoder_types::BackgroundEventIdentity::terminal(run.clone());
                    if let Err(error) = self.state.commit_background_terminal(
                        &identity,
                        task.status,
                        task.output_path.clone(),
                    ) {
                        tracing::debug!(run_id = %run.run_id, %error, "terminal outbox recovery remains pending");
                    }
                }
            }
        }
        let mut records = self.state.pending_background_terminals();
        let mut included: HashSet<_> = records.iter().map(|record| record.key.clone()).collect();
        for task in self
            .state
            .tasks()
            .into_values()
        ...
        {
            for record in task.background_runs {
                if record.terminal.is_some()
                    && !record.suppressed
                    && record.delivery == kcoder_state::TaskDelivery::Background
                    && (record.has_pending_hooks()
                        || (!record.followup_handled && !record.followup_started))
        ...
        let mut scoped: Vec<_> = records
            .into_iter()
            .filter_map(|record| {
                let identity = record.terminal?;
                let id = record.key.agent_id;
                let output = record.output_snapshot.unwrap_or_default();
                let event = match record.status {
                    TaskStatus::Completed => BackgroundJobEvent::Completed {
                        id,
                        output: ToolOutput::text(output),
                    },
                    TaskStatus::Failed => BackgroundJobEvent::Failed { id, error: output },
                    TaskStatus::Cancelled => BackgroundJobEvent::Cancelled { id, reason: output },
                    TaskStatus::Halted => BackgroundJobEvent::Halted { id, reason: output },
                    _ => return None,
                };
                Some(BackgroundJobEvent::Scoped {
                    identity,
                    event: Box::new(event),
                })
            })
        ...
        scoped.extend(
            self.state
                .tasks()
                .values()
                .filter(|task| task.background_run.is_none())
                .filter(|task| task.notify_parent_on_completion)
    ...
    pub(super) async fn apply_background_event_with_hooks(
        ...
        if let BackgroundJobEvent::Scoped { identity, event } = event {
            return self
                .apply_scoped_background_event_with_hooks(identity, *event)
                .await;
        }
        match &event {
            BackgroundJobEvent::Progress {
