...
pub struct BackgroundJobManager {
    ...
    publishers: Arc<RwLock<HashMap<String, ScopedPublisher>>>,
}
...
impl BackgroundJobManager {
    pub(crate) fn retry_terminal_publications(&self) {
        let publishers: Vec<_> = {
            let mut publishers =
                crate::recover_write_lock(&self.publishers, "background_publishers");
            publishers.retain(|id, _| self.state.task(id).is_some());
    ...
    fn publish(&self, event: BackgroundJobEvent) -> anyhow::Result<()> {
        let publisher = crate::recover_read_lock(&self.publishers, "background_publishers")
            .get(event.id())
            .cloned();
        if let Some(publisher) = publisher {
            publisher.send(event)
        } else {
            // Legacy/unmanaged records have no immutable run yet.
    ...
    pub fn report_progress_with_detail(
        ...
        let _ = self.publish(BackgroundJobEvent::Progress {
            id: id.to_string(),
            message,
            detail,
            current,
    ...
    pub fn report_subagent_steer_applied(
        ...
        let _ = self.publish(BackgroundJobEvent::SubagentSteerApplied {
            id: id.to_string(),
    ...
    pub fn associate_subagent_tool_call(
        ...
        let _ = self.publish(BackgroundJobEvent::Associated {
            id: id.to_string(),
    ...
    fn spawn_with_id_and_kind(
        ...
        // Keep Paused visible to the durable run transition until it is accepted.
        task.output = None;
        ...
        if matches!(kind, TaskKind::Subagent) {
            ...
            // Attribute the sub-agent to the session that spawned it. Both the
            // app-server's agent control surface and the live snapshot scope
            // themselves by this field, so without it a running background agent
            // is reported by its association event yet cannot be listed, steered
            // or snapshotted.
            task.parent_session_id = Some(self.state.session_id());
        ...
        let run = self
            .state
            .start_background_task_run(task)
            .map_err(|error| SpawnError::Persistence {
                message: error.to_string(),
            })?;
        let publisher = ScopedPublisher::new(self.tx.clone(), self.state.clone(), run.clone());
        crate::recover_write_lock(&self.publishers, "background_publishers")
            .insert(id.clone(), publisher.clone());
        let _ = self.publish(BackgroundJobEvent::Started {
            id: id.clone(),
        ...
        let handle = tokio::spawn(async move {
            ...
            let mut output = match AssertUnwindSafe(work).catch_unwind().await {
                Ok(output) => output,
            ...
            let mut text = tool_output_to_text(&output);

            let Some(mut task) = state.task(&job_id) else {
                crate::recover_write_lock(&handles, "background_job_handles").remove(&job_id);
                return;
            };
            if task.background_run.as_ref() != Some(&run) {
                return;
            }
            if matches!(task.kind, TaskKind::Subagent | TaskKind::Generic) {
                ...
                let output_path = if task.kind == TaskKind::Subagent {
                    output_path
                        .parent()
                        .unwrap_or(Path::new("."))
                        .join("runs")
                ...
                match write_managed_output_file(&output_path, &text).await {
                    ...
                    Err(error) => {
                        ...
                        output = ToolOutput::error(format!(
                            "failed to persist background run output: {error}"
                        ));
                        is_error = true;
                ...
                .unwrap_or_else(|poisoned| poisoned.into_inner());
            {
                    ...
                    && matches!(control_status, TaskStatus::Paused | TaskStatus::Halted)
                {
                    state.update_task(&job_id, |current| {
                        if current.background_run.as_ref() != Some(&run) {
                            return;
                        }
    ...
    pub fn abort(&self, id: &str) -> bool {
        ...
        };
        {
            ...
            if handle.generation == self.generation.load(Ordering::SeqCst) {
                let _ = self.publish(BackgroundJobEvent::Cancelled {
                    id: id.to_string(),
    ...
    pub async fn abort_and_wait(&self, id: &str) -> bool {
        ...
        if let Some(task) = self.state.task(id) {
            if task.kind == TaskKind::Subagent {
                if let (Some(key), Some(path)) = (task.background_run, task.output_path) {
                    let snapshot = path
                        .parent()
                        .unwrap_or(Path::new("."))
                        .join("runs")
                    ...
                    let text = tokio::fs::read_to_string(&path)
                        .await
                        .unwrap_or_else(|_| "cancelled by user".to_string());
                    let saved = write_managed_output_file(&snapshot, &text).await;
                    self.state.update_task(id, |task| {
                        if task.background_run.as_ref() == Some(&key) {
                            task.output_path = saved.as_ref().ok().map(|_| snapshot);
                        }
                    });
                }
            }
    ...
    pub fn promote_to_background_delivery(&self, id: &str) -> Result<(), SpawnError> {
        let terminal_event = {
            ...
            if task.delivery == TaskDelivery::Background && task.notify_parent_on_completion {
                return Ok(());
            }
            if let Some(key) = &task.background_run {
                self.state.promote_background_run(key).map_err(|error| {
                    SpawnError::Persistence {
                        message: error.to_string(),
                    }
                })?;
            }
        ...
        let _ = self.publish(BackgroundJobEvent::Promoted { id: id.to_string() });
    ...
    pub fn cancel_for_goal_stop(&self, id: &str, reason: &str) -> bool {
        ...
        if let Some(handle) = handle {
            ...
            if let Some(publisher) =
                crate::recover_read_lock(&self.publishers, "background_publishers")
                    .get(id)
                    .cloned()
            {
                publisher.send_retired(event);
            } else {
                let _ = self.tx.send(event);
            }
            true
