...
impl EngineEvent {
    pub fn background_payload(&self) -> &Self {
        match self {
            Self::BackgroundScoped { event, .. } => event.background_payload(),
            event => event,
        }
    }
    pub fn background_identity(&self) -> Option<&kcoder_types::BackgroundEventIdentity> {
        match self {
            Self::BackgroundScoped { identity, .. } => Some(identity),
            _ => None,
        }
    }
}
...
impl From<BackgroundJobEvent> for EngineEvent {
    fn from(event: BackgroundJobEvent) -> Self {
        match event {
            BackgroundJobEvent::Scoped { identity, event } => Self::BackgroundScoped {
                identity,
