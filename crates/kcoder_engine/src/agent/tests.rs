...
mod artifact_validation_tests {
    ...
    async fn artifact_validation_spawn_and_task_output_publish_state_not_model_text() {
        ...
        assert_eq!(output.execution_metadata.len(), 2);
        assert!(output.execution_metadata.iter().any(|metadata| matches!(metadata, kcoder_tools::ToolExecutionMetadata::ArtifactValidation(observed) if observed == &report)));
        assert!(output.execution_metadata.iter().any(|metadata| matches!(metadata, kcoder_tools::ToolExecutionMetadata::BackgroundResultDelivery { run } if engine.state.task(agent_id).unwrap().background_run.as_ref() == Some(run))));
...
#[test]
fn agent_model_selection_preserves_the_host_credential_factory() {
    struct Source(std::sync::Mutex<Vec<kcoder_config::Settings>>);
    impl crate::ClientModelConfiguration for Source {
        fn settings(&self, _: &str) -> anyhow::Result<kcoder_config::Settings> {
            anyhow::bail!("agent must not reload parent semantic configuration")
        }
        fn provider(&self, settings: &kcoder_config::Settings) -> anyhow::Result<Arc<dyn kcoder_api::Provider>> {
            self.0.lock().unwrap().push(settings.clone());
            Ok(Arc::new(crate::test_support::providers::EmptyProvider))
        }
    }
    let temp = tempfile::tempdir().unwrap();
    for use_profile in [true, false] {
        ...
        let engine = crate::test_support::engine_builder::TestEngineBuilder::new(temp.path())
            .settings(settings).build().with_client_model_configuration(source.clone());
        let runner = QueryEngineAgentRunner::new(engine);
        let runtime = runner.build_agent_runtime(&kcoder_tools::AgentRuntimeSelection {
            profile: use_profile.then(|| "agent-fixture".into()),
            provider: (!use_profile).then(|| "agent-fixture".into()),
            model: Some("agent-fixture".into()),
        }).unwrap();
        let seen = source.0.lock().unwrap();
        assert_eq!(seen.len(), 1);
        assert_eq!(seen[0].active_provider.as_deref(), Some("agent-fixture"));
        assert_eq!(seen[0].provider_extra_body["temperature"], 0.3);
        assert_eq!(runtime.settings.provider_extra_body, seen[0].provider_extra_body);
    }
}
