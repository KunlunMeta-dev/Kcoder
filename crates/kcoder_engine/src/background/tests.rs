...
async fn idle_reservation_blocks_spawn_rolls_back_and_commits() {
    ...
    finish.send(()).unwrap();
    while !matches!(
        events.recv().await.unwrap().into_payload(),
...
async fn spawn_runs_work_and_sends_completion() {
    ...
    let event = rx.recv().await.expect("started event").into_payload();
    assert!(matches!(event, BackgroundJobEvent::Started { id: got, .. } if got == id));

    let event = rx.recv().await.expect("completed event").into_payload();
    match event {
...
async fn running_job_can_publish_bounded_progress() {
    ...
    let id = manager
        ...
        .expect("spawn succeeds");
    assert!(matches!(
        rx.recv().await.expect("started event").into_payload(),
        BackgroundJobEvent::Started { .. }
    ));

    assert!(manager.report_progress(&id, "Running read", Some(2), Some(60)));
    assert!(matches!(
        rx.recv().await.expect("progress event").into_payload(),
...
async fn running_subagent_can_publish_steer_applied_without_becoming_terminal() {
    ...
    let id = manager
        ...
        .unwrap();
    assert!(matches!(
        rx.recv().await.unwrap().into_payload(),
        BackgroundJobEvent::Started { .. }
    ));
...
async fn generic_foreground_task_promotes_in_place_and_persists_output() {
    ...
    assert!(
        matches!(rx.recv().await.unwrap().into_payload(), BackgroundJobEvent::Started { id: got, .. } if got == id)
    );
    ...
    release.notify_one();
    assert!(
        matches!(rx.recv().await.unwrap().into_payload(), BackgroundJobEvent::Completed { id: got, .. } if got == id)
    );
...
async fn foreground_subagent_disables_parent_notification_before_start() {
    ...
    let started = rx.recv().await.expect("started event").into_payload();
    ...
    let _ = rx.recv().await.expect("completed event").into_payload();
}
...
async fn same_id_continuation_never_overtakes_previous_terminal_event() {
    ...
    assert!(rx.recv().await.unwrap().into_payload().is_final());
    assert!(matches!(
        rx.recv().await.unwrap().into_payload(),
        BackgroundJobEvent::Started { .. }
    ));
...
async fn abort_stops_running_job() {
    ...
    assert!(manager.abort(&id));
    let event = rx.recv().await.expect("cancelled event").into_payload();
...
async fn foreground_subagent_can_be_promoted_without_restart() {
    ...
    let event = rx.recv().await.unwrap().into_payload();
    assert!(matches!(event, BackgroundJobEvent::Completed { id: got, .. } if got == id));
    assert_eq!(state.task(id).unwrap().status, TaskStatus::Completed);
}
...
#[tokio::test]
async fn repeated_promotion_preserves_delivery_and_does_not_republish() {
    let state = AppState::new("/tmp");
    let (manager, mut rx) = BackgroundJobManager::new(state.clone());
    let mut task = Task::new("repeat-promotion", "fixture");
    task.kind = TaskKind::Subagent;
    task.status = TaskStatus::Completed;
    task.delivery = kcoder_state::TaskDelivery::Background;
    task.notify_parent_on_completion = true;
    task.notification_injected_at_ms = Some(42);
    state.upsert_task(task);
    manager
        .promote_to_background_delivery("repeat-promotion")
        .unwrap();
    assert_eq!(
        state
            .task("repeat-promotion")
            .unwrap()
