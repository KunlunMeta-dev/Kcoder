    ...
    async fn recv(&mut self) -> Result<Option<String>> {
        ...
        let line = loop {
            ...
            if self.stdout_buffer.len().saturating_add(count) > crate::sse::MAX_MCP_FRAME_BYTES {
                self.process_tree.terminate();
                anyhow::bail!("MCP stdio frame exceeds 16 MiB limit");
            }
            ...
            if complete {
                ...
                break Some(String::from_utf8(frame).context("invalid UTF-8 MCP frame")?);
            }
        };
        #[cfg(windows)]
        ...
        let handshake = async {
            while let Some(chunk) = stream.next().await {
                ...
                if buffer.len().saturating_add(chunk.len()) > crate::sse::MAX_MCP_FRAME_BYTES {
                    anyhow::bail!("MCP SSE handshake frame exceeds 16 MiB limit");
                }
                    ...
                    if let Some(event) = parser.push_line(&line) {
                        match event {
                            ...
                            SseEvent::LimitExceeded => {
                                anyhow::bail!("MCP SSE event exceeds 16 MiB limit")
                            }
