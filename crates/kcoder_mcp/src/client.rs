...
impl McpClient {
    ...
    pub async fn list_tools(&mut self) -> Result<Vec<crate::model::McpToolDefinition>> {
        ...
        tokio::time::timeout(REQUEST_TIMEOUT, self.list_tools_pages())
            .await
            .map_err(|_| anyhow!("MCP tools/list discovery timed out after 60s"))?
    }
    ...
    async fn list_tools_pages(&mut self) -> Result<Vec<crate::model::McpToolDefinition>> {
        ...
        for _ in 0..MAX_PAGES {
            ...
            if bytes > MAX_BYTES {
                anyhow::bail!("MCP tools/list exceeds 16 MiB discovery limit");
            }
            let page: Page = serde_json::from_value(raw).context("invalid MCP tools/list page")?;
            if tools.len().saturating_add(page.tools.len()) > MAX_TOOLS {
                anyhow::bail!("MCP tools/list exceeds 4096 tool discovery limit");
            }
        ...
        anyhow::bail!("MCP tools/list exceeds 128 page discovery limit")
    }
