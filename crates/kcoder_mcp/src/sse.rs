...
/// Capacity of the MCP transport's internal message queue; drop and warn when full, preserving established SSE behavior.
...
pub(crate) fn drain_sse_buffer(
    ...
    while let Some(line) = pop_line(buffer) {
        if let Some(SseEvent::Message(data)) = parser.push_line(&line) {
