...
/// Reads only a declared plugin icon, never an arbitrary target filesystem path.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct PluginIconParams {
    pub plugin_id: String,
    #[serde(default)]
    pub dark: bool,
}
...
/// Cancellation requests target one active installation. The install response
/// remains authoritative if cancellation races with publication.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct PluginCancelInstallParams {
    pub install_attempt_id: String,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct PluginCancelInstallResult {
...
pub struct MarketplaceAddParams {
    /// Explicit consent to trust the local source directory and its descendants.
    /// Only a marketplace root directory is accepted, never a URL or manifest file.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub trust_source_directory: Option<bool>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub replace_existing: Option<bool>,
    /// Optional target-owned plugin download proxy. Empty restores automatic proxy selection.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Target-user trust decisions; no client-local filesystem semantics are implied.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct PluginTrustSetParams {
    pub path: PathBuf,
    pub action: PluginTrustAction,
}
...
pub struct PluginTrustEntry {
    pub path: PathBuf,
    pub decision: PluginTrustAction,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct PluginTrustListResult {
    pub entries: Vec<PluginTrustEntry>,
...
mod trust_contract_tests {
    ...
    #[test]
    fn trust_mutations_require_an_explicit_known_action() {
        assert!(serde_json::from_value::<PluginTrustSetParams>(serde_json::json!({"path":"/project"})).is_err());
        assert!(serde_json::from_value::<PluginTrustSetParams>(serde_json::json!({"path":"/project","action":"default"})).is_err());
        let params: PluginTrustSetParams = serde_json::from_value(serde_json::json!({"path":"/project","action":"revoke"})).unwrap();
        assert_eq!(params.action, PluginTrustAction::Revoke);
        assert_eq!(serde_json::to_value(params).unwrap(), serde_json::json!({"path":"/project","action":"revoke"}));
    }
}
