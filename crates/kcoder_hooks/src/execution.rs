...
fn hook_matches_if_rule(input: &HookInput, hook: &HookCommand) -> bool {
    match hook {
...
fn hook_description(hook: &HookCommand) -> String {
    match hook {
...
fn source_label(source: Option<&HookSource>) -> String {
    source
        .map(|source| {
            format!(
                " from {} '{}'",
