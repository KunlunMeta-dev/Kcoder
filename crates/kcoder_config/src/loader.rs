...
impl<'de> Deserialize<'de> for CredentialStore {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        let mut credentials = match stored {
            StoredCredentialDocument::Legacy(legacy) => legacy.api_keys,
            StoredCredentialDocument::Current(stored) => {
                ...
                for (provider, credential) in stored {
                    ...
                    credentials.insert(provider, credential.key);
                }
                credentials
            }
        };
...
impl CredentialStore {
    ...
    pub fn remove_api_key(&mut self, provider: &str) -> Result<bool> {
        let provider = normalize_provider(provider)?;
        Ok(self.credentials.remove(&provider).is_some())
    }
...
pub(crate) fn write_bytes_atomic(
    ...
    let temp_id = ATOMIC_WRITE_TEMP_COUNTER.fetch_add(1, Ordering::Relaxed);
    let tmp_path = path.with_extension(format!("json.{}.{}.tmp", std::process::id(), temp_id));
    fs::write(&tmp_path, content)
        .with_context(|| format!("failed to write temporary file {}", tmp_path.display()))?;
    if require_user_only || cfg!(unix) {
