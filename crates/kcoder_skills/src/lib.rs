...
pub fn parse_skill_file(path: &Path) -> Result<Skill> {
    ...
    let directory = kcoder_config::PrivateDirectory::open_existing(parent)
        .with_context(|| format!("failed to open skill directory {:?}", parent))?;
    let name = path
        .file_name()
        .ok_or_else(|| anyhow::anyhow!("skill path has no file name: {:?}", path))?;
    let file = directory
        .open_regular_file(name)
        .with_context(|| format!("skill entry must be a regular non-symlink file: {:?}", path))?;
    ...
    if content.len() as u64 > MAX_SKILL_BYTES {
        anyhow::bail!("skill entry exceeds 1 MiB limit: {:?}", path);
    }
...
#[cfg(test)]
mod discovery_bounds_tests {
    #[test]
    fn rejects_oversized_skill_entry_without_reading_entire_file() {
    ...
    #[cfg(unix)]
    #[test]
    fn rejects_symlinked_skill_file_outside_package() {
        let root = tempfile::tempdir().unwrap();
...
#[cfg(all(test, unix))]
mod readonly_discovery_tests {
    #[test]
    fn read_only_shared_package_is_not_chmodded_or_required_to_be_private() {
        use std::os::unix::fs::PermissionsExt;
        let temp = tempfile::tempdir().unwrap();
        let root = temp.path().join("shared");
        std::fs::create_dir(&root).unwrap();
        let path = root.join("SKILL.md");
        std::fs::write(&path, "---\nname: shared\n---\nRead-only skill").unwrap();
        std::fs::set_permissions(&path, std::fs::Permissions::from_mode(0o444)).unwrap();
        std::fs::set_permissions(&root, std::fs::Permissions::from_mode(0o555)).unwrap();
        assert_eq!(super::parse_skill_file(&path).unwrap().name, "shared");
        assert_eq!(
