...
usage() {
  echo "Usage: scripts/audit/audit_release_check.sh [--current-source-only]"
}
...
if ((current_source_only)); then
  echo "== Committed source secret scan =="
  source_scan_output="$(mktemp)"
  trap 'rm -f "$source_scan_output"' EXIT
  if git grep -I -l -E -- "(^|[^[:alnum:]_-])($secret_regex)" HEAD -- . >"$source_scan_output"; then
    echo "FAIL: committed source contains possible credential material (contents withheld)."
    cat "$source_scan_output"
    exit 1
  else
    source_scan_status=$?
    if ((source_scan_status != 1)); then
      echo "FAIL: committed source scan could not complete."
      exit 1
    fi
  fi
  echo "OK: committed source has no matching credential material."
  exit 0
fi
