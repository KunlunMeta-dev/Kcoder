...
try {
    ...
    if ($ripgrepBinary) {
        ...
        $archiveItems += (Join-Path $stage "lib")
    }
    & node (Join-Path $PSScriptRoot "artifact-manifest.mjs") --stage $stage cli $Target
    if ($LASTEXITCODE -ne 0) { throw "Release resource audit failed" }
    $archiveItems += (Join-Path $stage "kcoder-release-manifest.json")
