...
archive_stage="$stage/archive.zip"
"$zip_bin" -9 -j -q "$archive_stage" \
  "$stage/kcoder.exe" \
  "$stage/kcoder-process-supervisor.exe" \
  "$stage/install-kcoder.ps1" \
  "$stage/kcoder-release-manifest.json"
if [[ -f "$stage/lib/kcoder/rg.exe" ]]; then
  (cd "$stage" && "$zip_bin" -9 -q -r "$archive_stage" lib)
fi
mv -f "$archive_stage" "$archive"
