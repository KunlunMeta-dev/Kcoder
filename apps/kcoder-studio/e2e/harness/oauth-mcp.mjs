...
export async function startOAuthMcpFixture(context, { manualApproval = false, manualClient = null, resourceScopes = [], tokenResponseFields = {}, beforeTokenResponse = null, tokenFailure = false, toolsListFailure = false, isTokenRevoked = () => false } = {}) {
  ...
  const server = createServer((request, response) => {
    void (async () => {
      ...
      if (url.pathname === '/token') {
        ...
        if (beforeTokenResponse) await beforeTokenResponse();
        if (tokenFailure) return json(503, { error: 'temporarily_unavailable' });
      ...
      if (isTokenRevoked() || request.headers.authorization !== 'Bearer ' + token) {
      ...
      if (rpc.method === 'tools/list' && toolsListFailure) return json(503, { error: 'fixture_discovery_unavailable' });
      if (rpc.id === undefined) {
