...
if ((historyProbe || workflowProbe || pluginsProbe) && !studioDirectory) throw new Error('UNMET_PREREQUISITE: project history validation requires a Windows Studio installation');
...
await runE2E(import.meta.url, {
  ...
  if (studioDirectory) {
    ...
    const ui = JSON.parse((await ps(`$result=& node '${directory}\\studio-ui-probe.cjs' '${directory}\\kcoder.exe' '${studioDirectory}' '${directory}\\overlay' '${directory}' ${pluginsProbe ? '--plugins' : workflowProbe ? '--workflows' : historyProbe ? '--project-history' : ''} ${process.env.KCODER_E2E_WINDOWS_INSTALLED_RESOURCES === '1' ? '--installed-resources' : ''}; Write-Output $result`, historyProbe || workflowProbe || process.env.KCODER_E2E_WINDOWS_INSTALLED_RESOURCES === '1' ? 240000 : 180000)).trim());
    await context.writeArtifactJson('windows-studio-ui.json', ui);
    for (const name of ['windows-skill-management.png', 'windows-mcp-management.png', 'windows-plugin-manager.png', 'windows-appearance.png', 'windows-avatar.png', 'windows-usage.png', 'windows-project-history.png', 'windows-workflow-colors.png', 'windows-workflows.png', 'windows-failure.png', 'windows-history-dom.json']) {
    ...
    if (pluginsProbe) assert.equal(ui.result?.pluginsValidated, true);
