...
await runE2E(import.meta.url, {
  ...
  try {
    ...
    phase = 'preserve-node-edits-across-panels';
    await page.getByTestId('workflow-conversation-canvas').getByTestId('workflow-node-A').click();
    const titleEditor=page.getByTestId('workflow-node-title');
    const originalTitle=await titleEditor.inputValue();
    await titleEditor.fill('Unsaved local title');
    await waitFor(async()=>await page.getByTestId('workflow-conversation-canvas').getByTestId('workflow-canvas-close').isDisabled(),5000,'dirty editor guard');
    await page.getByTestId('toggle-right-workspace-panel-button').click();
    await page.getByTestId('workflow-conversation-canvas').waitFor({state:'hidden'});
    await page.getByTestId('toggle-right-workspace-panel-button').click();
    await titleEditor.waitFor({state:'visible'});
    assert.equal(await titleEditor.inputValue(),'Unsaved local title');
    await page.getByTestId('workflow-node-discard').click();
    assert.equal(await titleEditor.inputValue(),originalTitle);
    await waitFor(async()=>await page.getByTestId('workflow-conversation-publish').isEnabled(),5000,'node edits restored');
    phase = 'publish';
    ...
    phase = 'reuse-from-conversation-card';
    await page.getByTestId('workflow-card-use').click();
    ...
    phase = 'execution-canvas';
    const executionCanvas = page.getByTestId('workflow-execution-canvas');
    await executionCanvas.waitFor();
    await executionCanvas.getByTestId('workflow-node-A').click();
    await executionCanvas.getByTestId('workflow-node-output').click();
    await executionCanvas.locator('pre').filter({hasText:'WF_NODE_A_RESULT'}).waitFor();
    assert.equal(await executionCanvas.getByTestId('workflow-conversation-publish').count(),0);
    await executionCanvas.getByRole('status').filter({hasText:'已完成'}).waitFor({timeout:10000});
    await page.screenshot({path:context.pathInArtifacts('conversation-run-canvas.png')});
    phase = 'narrow-conversation-canvas';
    await page.setViewportSize({width:820,height:900});
    await page.getByTestId('chat-message-input').click();
    await page.keyboard.insertText('Keep narrow draft.');
    await page.getByTestId('workflow-card-open').click();
    await executionCanvas.waitFor({state:'visible'});
    await page.screenshot({path:context.pathInArtifacts('narrow-run-canvas.png')});
    await executionCanvas.getByTestId('workflow-canvas-close').click();
    await page.getByTestId('chat-message-input').waitFor({state:'visible'});
    await waitFor(()=>page.getByTestId('workflow-card-open').evaluate(element=>document.activeElement===element),5000,'canvas return focus');
    assert.ok((await page.getByTestId('chat-message-input').innerText()).includes('Keep narrow draft.'));
    ...
    await page.getByTestId('workflow-workspace').getByTestId('workflow-node-A').waitFor();
    assert.equal(await page.getByTestId('workflow-add-node').isDisabled(), true);
    await page.getByTestId('workflow-workspace').getByTestId('workflow-node-A').click();
    assert.equal(await page.getByTestId('workflow-workspace').getByTestId('workflow-node-editor').count(), 0);
    await actions.getByRole('button', {name:'导出', exact:true}).waitFor({state:'visible'});
    const [download] = await Promise.all([page.waitForEvent('download'), actions.getByRole('button', {name:'导出', exact:true}).click()]);
    ...
    await page.getByTestId('workflow-workspace').getByTestId('workflow-node-output').waitFor();
    await page.getByTestId('workflow-workspace').getByTestId('workflow-fit').click();
    await page.getByTestId('workflow-workspace').getByTestId('workflow-canvas').screenshot({path:context.pathInArtifacts('seven-node-types-dark.png')});
    await page.goto(`${gateway.baseUrl}/settings`);
    await page.getByTestId('settings-nav-appearance').click();
    ...
    await page.getByTestId('workflow-workspace').getByTestId('workflow-node-output').waitFor();
    await page.getByTestId('workflow-workspace').getByTestId('workflow-fit').click();
    await page.getByTestId('workflow-workspace').getByTestId('workflow-canvas').screenshot({path:context.pathInArtifacts('seven-node-types-light.png')});
