...
describe('localModelCatalog', () => {
  it('uses a complete KCoder default capability profile', () => {
    ...
    expect(DEFAULT_CUSTOM_MODEL_BASE_INSTRUCTIONS).toContain('You are KCoder')
    expect(DEFAULT_CUSTOM_MODEL_BASE_INSTRUCTIONS).not.toContain(['You are Code', 'x'].join(''))
    expect(DEFAULT_CUSTOM_MODEL_BASE_INSTRUCTIONS).not.toContain('based on GPT-5')
  })

  it('keeps reasoning summary capability compatible across catalog versions', () => {
