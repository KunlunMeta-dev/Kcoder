...
export interface CodexOfficialModel {
  ...
  activeConfiguration?: ModelConfigurationSummary
  executionScope?: ModelExecutionScope
  id: string
  displayName: string
  modelId: string
  providerId: string
...
export interface CodexOfficialModelList {
  supportsModelSelectionMode?: boolean
...
function normalizeOfficialModel(
  value: unknown,
  provider?: Pick<
    CodexOfficialModelProvider,
    'id' | 'displayName' | 'type' | 'current' | 'available' | 'error'
  >
  ...
  return {
    ...
    providerAvailable: record.available !== false && provider?.available !== false,
    configuration: readModelConfiguration(record.configuration),
    providerError: stringValue(record.error) ?? provider?.error ?? null,
  }
}
...
export function normalizeCodexOfficialModelList(value: unknown): CodexOfficialModelList {
  const record = recordValue(value)
  const executionScope = readModelExecutionScope(record.executionScope)
  const activeConfiguration = readModelConfiguration(record.activeConfiguration)
  const enrich = (model: CodexOfficialModel): CodexOfficialModel => ({
    ...model,
    executionScope,
    ...(activeConfiguration?.providerId === model.providerId &&
    activeConfiguration.modelId === model.modelId
      ? { activeConfiguration }
      : {}),
  })
  ...
  if (providers.length > 0) {
    return {
      providers,
      models: providers.flatMap(provider => provider.models).map(enrich),
      supportsModelSelectionMode: record.supportsModelSelectionMode === true,
    }
  }
  ...
  return {
    ...
    models: fallbackModels.map(enrich),
    supportsModelSelectionMode: record.supportsModelSelectionMode === true,
  }
}
...
export function codexOfficialModelName(modelOrId: CodexOfficialModel | string): string {
  ...
  if (modelOrId.providerType !== 'provider') return modelOrId.modelId
  return modelOrId.id.includes('::')
    ? modelOrId.id
    : `${modelOrId.providerId}::${modelOrId.modelId}`
}
