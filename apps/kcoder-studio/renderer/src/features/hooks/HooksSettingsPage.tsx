import { KCoderHooksSettingsPage } from '@/kcoder/KCoderHooksSettingsPage'
...
export function HooksSettingsPage() {
  return isKCoderGatewayPage() ? <KCoderHooksSettingsPage /> : <NativeHooksSettingsPage />
}

function NativeHooksSettingsPage() {
  const { t } = useTranslation('hooks')
  const settings = useHooksSettings()
  const readOnlyGateway = isKCoderGatewayPage()
  ...
  return (
    <SettingsPage data-testid="hooks-settings-page">
      ...
      />
      {readOnlyGateway && (
        <div className="mb-4 rounded-xl border border-border bg-surface/70 px-4 py-3 text-sm text-text-secondary">
          {t('gatewayReadOnly')}
        </div>
      )}
      {settings.loading ? (
        ...
        <div className="rounded-lg bg-red-500/10 p-3 text-sm text-red-500">
          {settings.error}
          <button
            data-testid="hooks-reload-button"
            aria-label={t('retry')}
            className="ml-3"
      ...
      {!readOnlyGateway && editing !== undefined && (
        <HookEditorDialog
