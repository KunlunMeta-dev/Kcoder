...
function Harness() {
  ...
  return (
    <div>
      <span data-testid="persistence-error">{String(Boolean(persistenceError))}</span>
...
describe('AppearanceProvider', () => {
  ...
  test('reports failed persistence and clears the error after storage recovers', async () => {
    ...
    try {
      ...
      await waitFor(() => expect(screen.getByTestId('persistence-error')).toHaveTextContent('true'))
      failure.mockRestore()
      await userEvent.click(screen.getByTestId('set-dark'))
      await waitFor(() =>
        expect(screen.getByTestId('persistence-error')).toHaveTextContent('false')
      )
      expect(JSON.parse(localStorage.getItem('wework.appearance') ?? '{}').mode).toBe('dark')
    } finally {
      failure.mockRestore()
    }
  })
  ...
  test('steps UI and code font sizes together within their own bounds', async () => {
    ...
    act(() => {
      window.dispatchEvent(new CustomEvent(KCODER_STUDIO_STEP_FONT_SIZE_EVENT, { detail: { delta: 1 } }))
    })
