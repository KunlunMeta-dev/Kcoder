  ...
  return (
    <div className="flex items-center gap-2">
      <input
        ...
        className="h-8 w-16 rounded-lg border border-border bg-background px-2 text-right text-sm text-text-primary shadow-sm outline-none focus:border-focus focus:ring-2 focus:ring-focus/20"
      />
...
export function AppearanceSettingsPage() {
  ...
  const { appearance, resolvedMode, setAppearance, resetAppearance, persistenceError } =
    useAppearance()
  ...
  const setSeparateBackgrounds = (separateBackgroundsByTheme: boolean) => {
    if (separateBackgroundsByTheme && !appearance.themeBackgroundsInitialized) {
      const inheritedSettings = {
        imagePath: commonBackground.imagePath,
  ...
  return (
    <SettingsPage data-testid="appearance-settings-page">
      {persistenceError && (
        <p
          role="alert"
          data-testid="appearance-storage-error"
          className="mb-4 text-sm text-red-500"
        >
          {t('workbench.appearance_storage_error')}
        </p>
      )}
      <SettingsPageHeader
        title={t('workbench.appearance_title', '外观')}
        description={t('workbench.appearance_subtitle', '调整 KCoder Studio 的主题、颜色和字体')}
        actions={
          <button
            type="button"
            ...
            const displayConfig = {
              ...
              imagePath:
                editor.slot === 'common'
                  ...
                  : appearance.themeBackgroundsInitialized
                    ? editor.config.imagePath
      ...
      <section className="mt-4 overflow-hidden rounded-lg border border-border bg-background">
        <div className="border-b border-border px-4 py-3">
          ...
          <p className="mt-1 text-sm text-text-secondary">
            {t('workbench.appearance_font_availability')}
          </p>
        </div>
        <SettingsRow
      ...
      <section className="mt-4 overflow-hidden rounded-lg border border-border bg-background">
        <div className="border-b border-border px-4 py-3">
          <h2 className="text-sm font-semibold text-text-primary">
            {t('workbench.appearance_terminal_colors')}
          </h2>
          <p className="mt-1 text-sm text-text-secondary">
            {t('workbench.appearance_terminal_colors_description')}
          </p>
        </div>
        {(['light', 'dark'] as const).map(mode => {
          const field = mode === 'light' ? 'foreground_light' : 'foreground_dark'
          const configured = appearance.terminal?.colors[field]
          const color =
            configured?.length === 4
              ? `#${configured
                  .slice(1)
                  ...
                  .join('')}`
              : configured
          const label = t(`workbench.appearance_terminal_foreground_${mode}`)
          return (
            <SettingsRow
              key={mode}
              label={label}
              control={
                <div className="flex items-center gap-2">
                  <input
                    type="color"
                    aria-label={label}
                    data-testid={`appearance-terminal-foreground-${mode}`}
                    value={color || (mode === 'light' ? '#1a1a1a' : '#f1f5f9')}
                    onChange={event =>
                      setAppearance({ terminal: { colors: { [field]: event.target.value } } })
                    }
                    className="h-8 w-11 cursor-pointer rounded-md border border-border bg-background max-md:h-11"
                  />
                  <button
                    type="button"
                    data-testid={`appearance-terminal-reset-${mode}`}
                    disabled={!configured}
                    onClick={() => setAppearance({ terminal: { colors: { [field]: null } } })}
                    className="h-8 rounded-md px-3 text-sm text-text-secondary hover:bg-surface disabled:opacity-40 max-md:h-11"
                  >
                    {t('workbench.appearance_terminal_colors_reset')}
                  </button>
                </div>
              }
            />
