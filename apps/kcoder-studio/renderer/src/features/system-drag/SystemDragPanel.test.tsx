...
describe('SystemDragPanel', () => {
  ...
  test('identifies the floating panel with the KCoder Studio application icon', () => {
    render(<SystemDragPanel />)

    expect(screen.getByTestId('system-drag-brand')).toHaveTextContent('KCoder')
    expect(screen.getByRole('img', { name: 'KCoder Studio' })).toHaveAttribute(
      'src',
      expect.stringContaining('studio-icon.png')
    )
