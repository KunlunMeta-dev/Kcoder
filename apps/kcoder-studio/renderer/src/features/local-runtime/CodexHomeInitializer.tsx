...
const SHOULD_SKIP_CODEX_HOME_INITIALIZATION =
  ...
  import.meta.env.VITE_KCODER_STUDIO_E2E_CODEX_HOME_INITIALIZATION !== 'true'
...
export function CodexHomeInitializer({ children }: { children?: ReactNode }) {
  if (isKCoderGatewayPage()) return <>{children}</>
  return <LegacyCodexHomeInitializer>{children}</LegacyCodexHomeInitializer>
}

function LegacyCodexHomeInitializer({ children }: { children?: ReactNode }) {
  ...
  const initialize = (migrateNativeHome: boolean) => {
    console.warn('[KCoder Studio Codex init] initialization requested', {
      migrateNativeHome,
    })
    ...
    localPluginApi
      ...
      .then(() => {
        console.warn('[KCoder Studio Codex init] initialization finished')
      ...
      .catch((initializationError: Error) => {
        console.warn('[KCoder Studio Codex init] initialization failed', initializationError)
