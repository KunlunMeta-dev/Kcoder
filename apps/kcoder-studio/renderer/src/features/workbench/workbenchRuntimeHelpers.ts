import { joinDevicePath, normalizeDevicePath } from '@/lib/device-workspace-path'
...
/**
 * Device workspace entries become identity keys (task addresses, RPC
 * arguments) and UI text, so a runtime work entry must never carry a Windows
 * namespace (`\\?\`) spelling past the data entry point that feeds them.
 */
export function normalizeRuntimeWorkspacePaths(
  workspaces: RuntimeDeviceWorkspace[]
): RuntimeDeviceWorkspace[] {
  return workspaces.map(workspace => ({
    ...workspace,
    workspacePath: normalizeDevicePath(workspace.workspacePath),
  }))
}
