...
describe('WorkbenchProvider runtime tasks', () => {
  ...
  test('keeps an incompatible explicit local model unavailable instead of silently choosing another model', async () => {
    const models: UnifiedModel[] = [
      {
        name: 'gpt-5.5',
        type: 'runtime',
    ...
    expect(screen.getByTestId('selected-model-availability')).toHaveTextContent('unavailable')
  ...
  test('keeps a completed response idle after a refreshed task snapshot and late create acknowledgement', async () => {
    ...
    // A terminal event refreshes authoritative run/error facts exactly once.
    await waitFor(() => expect(listRuntimeWork).toHaveBeenCalledTimes(refreshCount + 1))
    expect(screen.getByTestId('pane-busy')).toHaveTextContent('idle')
    await userEvent.click(screen.getByText('refresh work lists'))
    await waitFor(() => expect(listRuntimeWork).toHaveBeenCalledTimes(refreshCount + 2))
