...
import {
  ...
  evictRuntimeTargetConversations,
...
describe('runtimeConversationCache', () => {
  ...
  test('keeps answered request ids across a canonical device alias and evicts them with the task', () => {
    cacheRuntimeAnsweredRequestUserInputIds(address, new Set(['item:question-1']))

    expect(
      getRuntimeAnsweredRequestUserInputIds({ ...address, deviceId: 'canonical-device' })
    ).toEqual(new Set(['item:question-1']))

    evictRuntimeConversation({ ...address, deviceId: 'canonical-device' })
    expect(getRuntimeAnsweredRequestUserInputIds(address)).toEqual(new Set())
  })
...
test('account invalidation removes only its own transcript and queue', () => {
  const a = { deviceId: 'account', taskId: 'kcoder:account:a', workspacePath: '/same' }
  const b = { deviceId: 'other', taskId: 'kcoder:other:b', workspacePath: '/same' }
  const message = { id: 'private', role: 'user' as const, content: 'private', status: 'done' as const, createdAt: '2026-09-23' }
  cacheRuntimeConversationMessages(a, [message]); cacheRuntimeConversationMessages(b, [message])
  cacheRuntimeConversationQueuePaused(a, true); cacheRuntimeConversationQueuePaused(b, true)
  cacheRuntimeAnsweredRequestUserInputIds(a, new Set(['approval-a']))
  cacheRuntimeAnsweredRequestUserInputIds(b, new Set(['approval-b']))
  evictRuntimeTargetConversations('account')
  expect(getRuntimeConversationMessages(a)).toEqual([])
  expect(getRuntimeConversationQueuePaused(a)).toBe(false)
  expect(getRuntimeAnsweredRequestUserInputIds(a).size).toBe(0)
  expect(getRuntimeConversationMessages(b)).toEqual([message])
  expect(getRuntimeConversationQueuePaused(b)).toBe(true)
  expect(getRuntimeAnsweredRequestUserInputIds(b).has('approval-b')).toBe(true)
})
