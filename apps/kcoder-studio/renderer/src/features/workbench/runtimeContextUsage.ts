...
export function findModelForSelection(
  ...
  const provider = selection.options?.codexProviderId ?? selection.options?.codex_model_provider
  ...
  return (
    models.find(
      ...
      !selection.modelName.includes('::') && model.modelId === selection.modelName &&
      (!provider || model.config?.codexProviderId === provider) &&
      (!selection.modelType || model.type === selection.modelType)
    ) ?? (models.filter(model => model.config?.codexProviderId === selection.modelName).length === 1
      ? models.find(model => model.config?.codexProviderId === selection.modelName) : null) ?? null
  )
}
...
export function modelSelectionFromRuntimeHandle(
  ...
  const rawModelName = selection.modelName ?? selection.model_name
  // An explicit empty name means follow the target default; an absent or
  // malformed name means no persisted selection. Do not conflate the two.
  if (typeof rawModelName !== 'string') return null
  ...
  return {
    modelName,
    modelType: modelName ? modelType || null : null,
