...
export interface RuntimeTaskStreamHandlers {
  ...
  onAssistantActivity?: (turnId: string) => void
...
export function createRuntimeTaskStreamHandlers(
  ...
  const reconnectMessages = new Set<string>()
  const settleReconnectMessage = (subtaskId: string, status: string | undefined) => {
    if (!reconnectMessages.has(subtaskId) || (status !== 'done' && status !== 'error')) return
    reconnectMessages.delete(subtaskId)
    // This is a transport-status message, not a terminal event for the task's real turn.
    handlers.onMessageAction({ type: 'assistant_done', subtaskId, content: '' })
  }
  ...
  return {
    ...
    onChatChunk: payload => {
      ...
      handlers.onAssistantActivity?.(identity.subtaskId)
    ...
    onChatError: payload => {
      ...
      if (isCancelledRuntimeError(payload)) {
        ...
        handlers.onMessageAction({
          ...
          error: payload.error,
          errorType: payload.type,
          providerFailure: decodeProviderFailure(payload.providerFailure),
        })
      }
      streamedFileChanges.delete(identity.subtaskId)
      handlers.onRefreshWorkLists?.()
    ...
    onBlockCreated: payload => {
      ...
      if (
        block.type === 'tool' &&
        isRecord(block.renderPayload) &&
        block.renderPayload.kind === 'request_user_input'
      ) {
        handlers.onRefreshWorkLists?.()
      }
      ...
      if (
        ...
        block.id === identity.subtaskId &&
        block.id.startsWith('runtime-reconnecting-')
      ) {
        reconnectMessages.add(block.id)
        settleReconnectMessage(block.id, block.status)
      }
    ...
    onBlockUpdated: payload => {
      ...
      if (
        payload.blockId.startsWith('approval-') ||
        payload.blockId.startsWith('request-user-input-') ||
        payload.blockId.startsWith('question-')
      ) {
        handlers.onRefreshWorkLists?.()
      }
      if (payload.blockId === identity.subtaskId) {
        settleReconnectMessage(identity.subtaskId, payload.status)
      }
    },
    onSubagentActivity: payload => {
      ...
      if (
        ...
        ['completed', 'failed', 'interrupted', 'cancelled'].includes(payload.status ?? '')
      )
        handlers.onRefreshWorkLists?.()
    },
    ...
    onRuntimePlanUpdated: payload => {
      ...
      if (import.meta.env.DEV) {
        console.info('[KCoder Studio] Runtime task plan scoped', {
          matched,
    ...
    onStreamGap: handlers.onStreamGap,
...
export function runtimeMessagesToWorkbenchMessages(
  ...
  return coalesceRetryUserMessages(messages.map(runtimeMessageToWorkbenchMessage))
}

function coalesceRetryUserMessages(messages: WorkbenchMessage[]): WorkbenchMessage[] {
  const collapsed: WorkbenchMessage[] = []
  let latestUserId: string | null = null
  let latestAttemptStart = 0
  let retryFailures: number[] = []

  for (const message of messages) {
    if (message.role !== 'user') {
      for (const index of retryFailures) {
        collapsed[index] = { ...collapsed[index], retriedByMessageId: message.id }
      }
      retryFailures = []
    ...
    const latestAttempt = collapsed.slice(latestAttemptStart)
    ...
    if (retriesLatestFailedTurn) {
      // Coalesce the duplicate submission identity, never the failed evidence.
      retryFailures = collapsed.flatMap((candidate, index) =>
        index >= latestAttemptStart &&
        candidate.role === 'assistant' &&
        candidate.status === 'failed' &&
        !candidate.retriedByMessageId
          ? [index]
          : []
      )
      latestAttemptStart = collapsed.length
      continue
    }

    retryFailures = []
    latestUserId = message.id
    latestAttemptStart = collapsed.length + 1
    collapsed.push(message)
  }
...
export function runtimeTranscriptDebug(response: unknown): Record<string, unknown> {
  ...
  return {
    keys: Object.keys(response).filter(key => ['messages', 'success', 'error', 'runtime', 'turnNavigation', 'rangeStart', 'rangeEnd', 'hasMoreBefore', 'beforeCursor', 'hasMoreAfter', 'afterCursor'].includes(key)),
    success: typeof response.success === 'boolean' ? response.success : undefined,
    hasError: Boolean(response.error),
    runtime: typeof response.runtime === 'string' && ['local', 'kcoder', 'codex', 'claude'].includes(response.runtime) ? response.runtime : 'unknown',
    ...
    turnNavigationCount: Array.isArray(response.turnNavigation)
      ...
      : null,
    rangeStart: typeof response.rangeStart === 'number' ? response.rangeStart : undefined,
    rangeEnd: typeof response.rangeEnd === 'number' ? response.rangeEnd : undefined,
    hasMoreBefore: response.hasMoreBefore === true,
    hasBeforeCursor: typeof response.beforeCursor === 'string',
    hasMoreAfter: response.hasMoreAfter === true,
    hasAfterCursor: typeof response.afterCursor === 'string',
  }
}
...
function runtimeMessageToWorkbenchMessage(message: NormalizedRuntimeMessage): WorkbenchMessage {
  ...
  return {
    ...
    providerFailure: decodeProviderFailure(message.providerFailure ?? message.provider_failure),
    attemptId: typeof message.attemptId === 'string' ? message.attemptId : undefined,
    continuedByAttemptId:
      typeof message.continuedByAttemptId === 'string' ? message.continuedByAttemptId : undefined,
    continuationAccepted: Boolean(message.continuedByAttemptId) || undefined,
    source,
...
function warnAndDropRuntimeStreamEvent(
  ...
  console.warn('[KCoder Studio] Dropped runtime stream event without task identity', {
    event,
...
function warnAndDropMismatchedRuntimeTerminalEvent(
  ...
  console.warn('[KCoder Studio] Dropped mismatched runtime terminal event', {
    event,
...
function logAcceptedRuntimeTerminalEvent(
  ...
  console.info('[KCoder Studio] Runtime terminal event accepted', {
    event,
    currentRuntimeTask: runtimeAddressDebug(address),
    payloadTaskId: payload.taskId ?? null,
    payloadDeviceId: payload.deviceId ?? null,
...
function warnAndDropEmptyRuntimeChunk(
  ...
  console.warn('[KCoder Studio] Dropped empty runtime stream chunk', {
    event: 'chat:chunk',
    address: runtimeAddressDebug(address),
    taskId: payload.taskId,
    deviceId: payload.deviceId,
...
function warnInvalidRuntimeTranscriptIdentity(
  ...
  console.warn('[KCoder Studio] Runtime transcript message missing valid subtask identity', {
...
function isCancelledRuntimeError(payload: ChatErrorPayload): boolean {
  if (decodeProviderFailure(payload.providerFailure)) return false
  ...
  return cancellationValues.has(error) || (typeof type === 'string' && cancellationValues.has(type))
}
...
function normalizeProcessingBlock(
  ...
  if (block.type === 'tool') {
    ...
    return {
      ...
      status,
      createdAt: timestamp,
      completedAt: getOptionalBlockTimestamp(block.completedAt ?? block.completed_at),
    }
  }
  ...
  console.warn('[KCoder Studio] Dropped runtime block with unsupported type', {
    subtaskId,
    index,
...
function warnAndDropRuntimeTranscriptBlock(
  ...
  console.warn('[KCoder Studio] Dropped runtime transcript block without block identity', {
    subtaskId,
    index,
    blockType: block.type,
    blockId: block.id,
...
function getOptionalBlockTimestamp(value: unknown): number | undefined {
  const timestamp = getBlockTimestamp(value, Number.NaN)
...
type RuntimeDebugWindow = Window & { __KCODER_STUDIO_RUNTIME_DEBUG__?: boolean }

function isRuntimeWorkDebugEnabled(): boolean {
  return (
    ((window as RuntimeDebugWindow).__KCODER_STUDIO_RUNTIME_DEBUG__ ?? false) ||
    import.meta.env.VITE_KCODER_STUDIO_RUNTIME_DEBUG === '1'
  )
}
...
function debugRuntimeStreamEvent(
  ...
  console.debug(`[KCoder Studio runtime] ${label}`, {
    matched,
    currentRuntimeTask: runtimeAddressDebug(address),
    payloadDeviceId: payload.deviceId ?? null,
    payloadTaskId: payload.taskId ?? null,
