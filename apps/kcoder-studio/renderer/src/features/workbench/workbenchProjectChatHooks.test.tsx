...
describe('workbench project chat hooks', () => {
  ...
  test('restores model selection config and emits user changes', async () => {
    ...
    // reasoning/speed selections are removed from Studio; stored options are dropped
    expect(result.current.selectedModelOptions).toEqual({})
  ...
  test('uploads attachments without restricting file extensions', async () => {
    ...
    expect(upload).toHaveBeenCalledWith(file, expect.any(Function), expect.any(AbortSignal))
    expect(result.current.attachments).toEqual([attachment])
    expect(result.current.errors.size).toBe(0)
  })

  test('keeps failed uploads out of the send-ready state and clears the error on retry', async () => {
    const attachment: Attachment = {
      id: 46,
      filename: 'photo.png',
      file_size: 1200,
    ...
    const { result } = renderHook(() =>
      useWorkbenchAttachments({ uploadAttachment: upload, deleteAttachment: vi.fn() })
    )
    const file = new File(['image'], 'photo.png', { type: 'image/png' })

    await act(async () => result.current.handleFileSelect(file))
    expect(result.current.errors).toEqual(new Map([['photo.png', 'upload failed']]))
  ...
  test('shows an upload placeholder immediately and removes it when cancelled', async () => {
    const attachment: Attachment = {
      id: 47,
      filename: 'photo.png',
      file_size: 1200,
    ...
    expect(result.current.uploadingFiles.get('photo.png')).toEqual(
      expect.objectContaining({ file, progress: 0, cancel: expect.any(Function) })
    )

    act(() => result.current.cancelUpload('photo.png'))
...
test('switching model targets hides the old catalog and ignores late responses from the previous target', async () => {
  const local = { name: 'local-only', type: 'runtime' as const }
  ...
  const { result, rerender } = renderHook(
    ({ deviceId }) =>
      useWorkbenchModels({
        api,
        locked: false,
        target: { deviceId, workspacePath: '/project' },
      }),
    { initialProps: { deviceId: 'local' } }
  )
  await waitFor(() => expect(result.current.models).toEqual([local]))
  act(() => result.current.setSelectedModel(local))
