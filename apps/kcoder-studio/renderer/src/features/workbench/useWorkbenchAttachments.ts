...
export function useWorkbenchAttachments(options: UseWorkbenchAttachmentsOptions = {}) {
  ...
  const scopeGeneration = useRef(new Map<string, number>())
  const generation = scopeGeneration.current.get(scopeKey) ?? 0
  ...
  const updateScopeState = useCallback(
    (updater: (current: MultiAttachmentUploadState) => MultiAttachmentUploadState) => {
      setStateByScope(currentByScope => {
        if ((scopeGeneration.current.get(scopeKey) ?? 0) !== generation) return currentByScope
  ...
  const invalidateScopes = useCallback((matches: (key: string) => boolean) => {
    const keys = new Set([...Object.keys(stateByScope), scopeKey])
    for (const [key, controller] of uploadControllersRef.current) {
      const uploadScope = JSON.parse(key)[0] as string
      if (!matches(uploadScope)) continue
      keys.add(uploadScope)
      controller.abort()
      uploadControllersRef.current.delete(key)
    }
    for (const key of keys) if (matches(key)) {
      scopeGeneration.current.set(key, (scopeGeneration.current.get(key) ?? 0) + 1)
      for (const attachment of stateByScope[key]?.attachments ?? []) releaseAttachmentPreview(attachment)
    }
    setStateByScope(current => Object.fromEntries(Object.entries(current).filter(([key]) => !matches(key))))
  }, [scopeKey, stateByScope])
