...
export class RuntimeTaskLifecycleStore {
  ...
  turnStarted(address: RuntimeTaskAddress, turnId?: string): void {
    this.dispatch(address, { type: 'turn_started', turnId })
  }

  turnSettled(address: RuntimeTaskAddress, turnId?: string): void {
  ...
  syncTranscript(
    ...
    const currentLifecycle = this.getTask(address)
    ...
    if (transcript.running === true && !ignoreStaleRunningTranscript) {
      this.executorStarted(address)
    } else if (transcript.running === false && !ignoreStaleIdleTranscript) {
      this.executorSettled(address)
    }

    if (options.settlesActiveTurn === true) {
      this.turnSettled(address)
    } else if (hasStreamingAssistant && transcript.running !== false) {
  ...
  turnActivity(address: RuntimeTaskAddress, turnId: string): void {
    const current = this.machines.get(getRuntimeTaskLifecycleKey(address))?.getState()
    if (current?.activeTurnId && current.activeTurnId !== turnId) return
    if (
      current?.executionPhase === 'idle' &&
      current.activeTurnId === null &&
      current.expectedExecutorRunning === false
    )
      return
    this.turnStarted(address, turnId)
  }

  resetBackgroundActivity(address: RuntimeTaskAddress): void {
    this.dispatch(address, { type: 'background_reset' })
  }

  backgroundActivity(address: RuntimeTaskAddress, agentId: string, status: string): void {
    if (agentId) this.dispatch(address, { type: 'background_activity', agentId, status })
  }
  ...
  removeTarget(targetId: string): void {
    ...
    for (const key of this.machines.keys()) {
      if (key.startsWith(`${targetId}\0`)) changed = this.machines.delete(key) || changed
    }
    if (changed) this.publish()
  }
  ...
  rename(previous: RuntimeTaskAddress, next: RuntimeTaskAddress): void {
    ...
    const nextState = nextMachine.getState()
    // Events can already be routed to the canonical task while create is pending.
    // Only an unknown or purely optimistic destination may inherit send progress.
    const preserveResolvedExecution =
      nextState.expectedExecutorRunning !== null ||
      nextState.turnPhase !== 'idle' ||
      nextState.executionPhase === 'idle' ||
      (nextState.executionPhase !== 'unknown' && nextState.task?.optimistic !== true)
    if (!preserveResolvedExecution) {
    ...
    if (previousState.goalStatus !== null && nextState.goalStatus === null) {
      nextMachine.dispatch({
  ...
  private reduceMachine(address: RuntimeTaskAddress, event: RuntimeTaskLifecycleEvent): boolean {
    ...
    debugRuntimeLifecycleTransition(event, previous, next)
    if (
...
function debugRuntimeLifecycleTransition(
  event: RuntimeTaskLifecycleEvent,
  previous: RuntimeTaskLifecycleSnapshot,
  next: RuntimeTaskLifecycleSnapshot
): void {
  if (globalThis.localStorage?.getItem('wework:debug-runtime') !== '1') return
  console.debug('[KCoder Studio] Runtime lifecycle transition', {
    event: event.type,
    taskId: next.address.taskId,
    ...
    goalStatus: next.goalStatus,
    changed:
      previous.execution.phase !== next.execution.phase || previous.turn.phase !== next.turn.phase,
  })
}
