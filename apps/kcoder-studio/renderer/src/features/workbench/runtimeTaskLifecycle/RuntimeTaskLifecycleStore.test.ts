...
describe('RuntimeTaskLifecycleStore', () => {
  ...
  test('ignores a send acknowledgement arriving after the response has completed', () => {
    const store = new RuntimeTaskLifecycleStore('test')
    store.sendRequested(address)
    store.turnStarted(address)
    store.turnSettled(address)
  ...
  test('does not reopen a completed turn when an idle snapshot precedes the late send acknowledgement', () => {
    const store = new RuntimeTaskLifecycleStore('test')
    store.sendRequested(address)
    store.turnStarted(address)
    store.turnSettled(address)
  ...
  test('does not let a late optimistic task upsert replace confirmed idle execution', () => {
    const store = new RuntimeTaskLifecycleStore('test')
    store.sendRequested(address)
    store.turnStarted(address)
    store.turnSettled(address)
    store.syncRuntimeWork(runtimeWork(task({ running: false })))
    store.syncRuntimeWork(runtimeWork(task({ running: true, optimistic: true })))
    store.sendAccepted(address)
  ...
  test('settles the current turn when a stale running snapshot already contains its completed output', () => {
    const store = new RuntimeTaskLifecycleStore('test')
  ...
  test('does not resurrect a turn settled by the live stream from a late running transcript', () => {
    const store = new RuntimeTaskLifecycleStore('test')
  ...
  test('a disposed temporary send stays settled after a late canonical acknowledgment', () => {
    const store = new RuntimeTaskLifecycleStore('test')
    const resolved = { ...address, taskId: 'temporary-child' }
    store.sendRequested(address)
    store.executorSettled(address)
    store.turnSettled(address)
    store.rename(address, resolved)
    store.sendAccepted(resolved)
    expect(store.getSnapshot().runningTaskKeys.size).toBe(0)
    expect(store.getTask(resolved)?.derived.isBusy).toBe(false)
  })
  ...
  test('migrates a pending send into a canonical task known only from an optimistic list row', () => {
    const store = new RuntimeTaskLifecycleStore('test')
    const resolved = { ...address, taskId: 'resolved-task' }
    store.sendRequested(address)
    store.syncRuntimeWork(
      runtimeWork(task({ taskId: resolved.taskId, running: true, optimistic: true }))
    )
    store.rename(address, resolved)
    store.sendAccepted(resolved)

    expect(store.getTask(resolved)?.turn.phase).toBe('awaiting')
    expect(store.getTask(resolved)?.derived.isBusy).toBe(true)
  })

  test.each(['streaming', 'completed'] as const)(
    'preserves canonical %s events received before optimistic task identity resolution',
    phase => {
      const store = new RuntimeTaskLifecycleStore('test')
      const resolved = { ...address, taskId: 'resolved-task' }
      store.sendRequested(address)
      store.turnStarted(resolved)
      if (phase === 'completed') {
        store.turnSettled(resolved)
        store.syncRuntimeWork(runtimeWork(task({ taskId: resolved.taskId, running: false })))
      }
      store.syncRuntimeWork(
        runtimeWork(task({ taskId: resolved.taskId, running: true, optimistic: true }))
      )
      ...
      expect(store.getTask(address)).toBeNull()
      expect(store.getTask(resolved)?.turn.phase).toBe(phase === 'completed' ? 'idle' : 'streaming')
      expect(store.getTask(resolved)?.derived.isBusy).toBe(phase !== 'completed')
    }
  )
...
test('a late completion from another turn cannot stop the current sidebar activity', () => {
  const store = new RuntimeTaskLifecycleStore('scoped-running-test')
  store.turnStarted(address, 'new-turn')
  store.turnSettled(address, 'old-turn')
  expect(store.getTask(address)?.derived.shouldShowSidebarRunning).toBe(true)
  store.turnSettled(address, 'task-1-context-compact')
  expect(store.getTask(address)?.derived.shouldShowSidebarRunning).toBe(true)
  store.turnSettled(address, 'new-turn')
  expect(store.getTask(address)?.derived.shouldShowSidebarRunning).toBe(false)
})

test('cached done snapshots do not hide a live turn and completion still settles it', () => {
  const store = new RuntimeTaskLifecycleStore('live-snapshot-test')
  store.turnStarted(address, 'live-turn')
  store.syncRuntimeWork(runtimeWork(task({ running: false, status: 'done' })))
  expect(store.getTask(address)?.derived.shouldShowSidebarRunning).toBe(true)
  store.turnSettled(address, 'live-turn')
  expect(store.getTask(address)?.derived.shouldShowSidebarRunning).toBe(false)
})

test('sidebar activity spans background work after the parent turn ends', () => {
  const store = new RuntimeTaskLifecycleStore('background-running-test')
  store.turnStarted(address, 'parent')
  store.backgroundActivity(address, 'child-a', 'running')
  store.backgroundActivity(address, 'child-b', 'running')
  store.turnSettled(address, 'parent')
  expect(store.getTask(address)?.execution.phase).toBe('running')
  store.syncTranscript(address, { messages: [], running: false } as RuntimePaneTranscript)
  expect(store.getTask(address)?.derived.shouldShowSidebarRunning).toBe(true)
  store.backgroundActivity(address, 'child-a', 'completed')
  expect(store.getTask(address)?.derived.shouldShowSidebarRunning).toBe(true)
  store.backgroundActivity(address, 'child-b', 'paused')
  expect(store.getTask(address)?.derived.shouldShowSidebarRunning).toBe(false)
})

test('late activity cannot reopen a completed turn or replace a newer turn', () => {
  const store = new RuntimeTaskLifecycleStore('late-activity')
  store.turnStarted(address, 'new')
  store.turnActivity(address, 'old')
  store.turnSettled(address, 'new')
  store.turnActivity(address, 'new')
  expect(store.getTask(address)?.derived.shouldShowSidebarRunning).toBe(false)
})

test('replacing the runtime clears background work belonging to the old process', () => {
  const store = new RuntimeTaskLifecycleStore('replaced-background')
  store.backgroundActivity(address, 'child', 'running')
  expect(store.getTask(address)?.derived.shouldShowSidebarRunning).toBe(true)
  store.resetBackgroundActivity(address)
  expect(store.getTask(address)?.derived.shouldShowSidebarRunning).toBe(false)
})
