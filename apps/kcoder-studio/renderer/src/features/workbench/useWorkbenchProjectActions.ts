...
export function useWorkbenchProjectActions({
  ...
  const setRuntimeProjectPinned = useCallback(
    async (data: RuntimeProjectPinRequest) => {
      const response = await executorClient.runtime.setRuntimeProjectPinned(data)
      if (response?.accepted !== true)
        throw new Error(response?.error || t('workbench.pin_update_failed'))
      await refreshWorkLists()
    },
    [executorClient, refreshWorkLists, t]
  )
