...
describe('runtimeMessagesToWorkbenchMessages', () => {
  ...
  test('retains a failed attempt when the same user message is retried successfully', () => {
    const messages = runtimeMessagesToWorkbenchMessages([
      {
        id: 'provider-user-1',
        clientMessageId: 'runtime-local-pane-1',
    ...
    expect(messages[1]).toMatchObject({ status: 'failed', error: 'request failed', retriedByMessageId: 'assistant-success' })
  })

  test('keeps every failure when repeated retries continue to fail', () => {
    const messages = runtimeMessagesToWorkbenchMessages([
      {
        id: 'provider-user-1',
        clientMessageId: 'runtime-local-pane-1',
    ...
    expect(messages[1]).toMatchObject({ error: 'first failure', retriedByMessageId: 'assistant-failed-2' })
    expect(messages[2]).toMatchObject({ error: 'second failure' })
  })
...
test('restored attempt metadata preserves failure identity and accepted continuation', () => {
  const [message] = runtimeMessagesToWorkbenchMessages([{
    id: 'failed-attempt', role: 'assistant', content: 'partial', status: 'failed',
    turnId: 'turn-1', attemptId: 'turn-1-retry-prior', continuedByAttemptId: 'turn-1-retry-next',
    error: 'interrupted response',
  }])
  expect(message).toMatchObject({ status: 'failed', content: 'partial', attemptId: 'turn-1-retry-prior',
    continuedByAttemptId: 'turn-1-retry-next', continuationAccepted: true })
})
