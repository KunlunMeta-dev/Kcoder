...
function mergeRuntimeWorkspaces(
  ...
  const upsertWorkspace = (workspace: RuntimeDeviceWorkspace) => {
    ...
    const workspaceError = existing?.error ?? canonicalWorkspace.error
    ...
    workspaces.set(key, {
      workspace: {
        ...
        available:
          !workspaceError && ((existing?.available ?? false) || canonicalWorkspace.available),
        tasks,
      },
...
function canonicalizeRuntimeWorkspace(
  ...
  return {
    ...
    available:
      !workspace.error && (workspace.available || isRuntimeWorkspaceDeviceAvailable(device)),
  }
}
...
export async function timedWorkbenchBootstrapRequest<T>(
  ...
  try {
    ...
    if (elapsedMs > 5000) {
      console.warn(`[KCoder Studio] Workbench bootstrap ${label} completed slowly in ${elapsedMs}ms.`)
    }
    return { status: 'fulfilled', value }
  } catch (reason) {
    const elapsedMs = Math.round(nowMs() - startedAt)
    if (!isAbortError(reason)) console.warn(`[KCoder Studio] Workbench bootstrap ${label} failed after ${elapsedMs}ms.`, safeErrorDiagnostic(reason))
