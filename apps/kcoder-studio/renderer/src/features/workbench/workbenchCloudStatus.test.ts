...
describe('mergeRuntimeWorkLists', () => {
  test('keeps a missing workspace unavailable when its canonical device is online', () => {
    const missing = runtimeWork('local-device')
    missing.projects[0].deviceWorkspaces[0] = {
      ...missing.projects[0].deviceWorkspaces[0],
      available: false,
      error: 'workspace directory is unavailable',
    }
    const empty: RuntimeWorkListResponse = { projects: [], chats: [], totalTasks: 0 }

    const merged = mergeRuntimeWorkLists(missing, empty, {
      devices: [
        {
          id: 1,
...
test('bootstrap cancellation stays rejected without logging a product failure', async () => {
  const { timedWorkbenchBootstrapRequest } = await import('./workbenchCloudStatus')
  const warn = vi.spyOn(console, 'warn').mockImplementation(() => {})
  try {
    const error = new DOMException('navigation', 'AbortError')
    await expect(timedWorkbenchBootstrapRequest('runtimeWork', Promise.reject(error))).resolves.toEqual({ status: 'rejected', reason: error })
    expect(warn).not.toHaveBeenCalled()
    await timedWorkbenchBootstrapRequest('runtimeWork', Promise.reject(new Error('broken service')))
