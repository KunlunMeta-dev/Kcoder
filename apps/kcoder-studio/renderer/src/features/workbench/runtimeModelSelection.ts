...
function isSelectableConfiguredDefault(model: UnifiedModel): boolean {
  if (isCloudModel(model)) return true
  if (!isLocalModel(model)) return false

  const kind = modelKind(model)
  if (kind === 'codex-official') {
    return getBooleanConfigValue(model.config, 'codexAuthConfigured')
  }
  return kind === 'codex-provider' || kind === 'model-interface'
}
...
export function defaultNewChatModel(models: UnifiedModel[]): UnifiedModel | null {
  const candidates = models.filter(model => !model.compatibilityDisabled)
  const selected =
    candidates.find(
      model =>
        getBooleanConfigValue(model.config, 'codexModelDefault') &&
        isSelectableConfiguredDefault(model)
    ) ??
    candidates.find(
      model =>
        getBooleanConfigValue(model.config, 'codexProviderCurrent') &&
        isSelectableConfiguredDefault(model)
    ) ??
    candidates.find(
      model =>
...
export function defaultNewChatModelSelection(models: UnifiedModel[]): ModelSelectionConfig | null {
  const selected = defaultNewChatModel(models)
...
export function selectedModelExecutionFields(
  ...
  mode?: RuntimeSendRequest['modelSelectionMode']
): Pick<RuntimeSendRequest, 'modelId' | 'modelType' | 'modelOptions' | 'modelSelectionMode'> {
  ...
  if (mode === 'follow_target_default') {
    // Routing hints belong to the previous explicit deployment, not to the
    // user's runtime choices. Keeping them would turn following into a pinned
    // provider or a conflicting model+mode request after restoring old options.
    for (const key of ['codexProviderId', 'codex_model_provider', 'codexProviderName', 'codex_provider_name',
      CLOUD_MODEL_NAMESPACE_OPTION, CLOUD_MODEL_RESOURCE_USER_ID_OPTION,
      CLOUD_MODEL_CONTEXT_WINDOW_OPTION, CLOUD_MODEL_UPSTREAM_API_FORMAT_OPTION]) {
      delete modelOptions[key]
    }
    return { modelOptions, modelSelectionMode: mode }
  }
  if (!selectedModel) {
    return { modelOptions }
  }
