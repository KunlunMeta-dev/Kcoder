...
const answeredRequestUserInputIdsByTask = new Map<string, ReadonlySet<string>>()
...
export function getRuntimeAnsweredRequestUserInputIds(
  address: RuntimeTaskAddress
): ReadonlySet<string> {
  return new Set(touchEntry(answeredRequestUserInputIdsByTask, address.taskId) ?? [])
}

export function cacheRuntimeAnsweredRequestUserInputIds(
  address: RuntimeTaskAddress,
  ids: ReadonlySet<string>
) {
  if (ids.size === 0) {
    answeredRequestUserInputIdsByTask.delete(address.taskId)
    return
  }
  // deviceId may change after local alias resolution; runtime taskId already includes the runtime namespace inside the gateway.
  cacheBoundedEntry(answeredRequestUserInputIdsByTask, address.taskId, new Set(ids))
}
...
export function clearRuntimeConversationCacheForTests() {
  ...
  answeredRequestUserInputIdsByTask.clear()
...
/** Account changes invalidate only this target, including queued prompts and scroll snapshots. */
export function evictRuntimeTargetConversations(targetId: string): void {
  for (const cache of [messagesByConversation, queuedMessagesByConversation,
    queuedMessagesPausedByConversation, scrollSnapshotsByConversation, virtualMeasurementsByConversation]) {
    for (const key of cache.keys()) if (key.startsWith(`${targetId}:`)) cache.delete(key)
  }
  for (const key of answeredRequestUserInputIdsByTask.keys()) {
    if (key.startsWith(`kcoder:${targetId}:`)) answeredRequestUserInputIdsByTask.delete(key)
  }
}
