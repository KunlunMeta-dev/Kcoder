...
describe('workbenchReducer', () => {
  ...
  test('does not make a missing workspace available when its device reports online', () => {
    const state = workbenchReducer(initialWorkbenchState, {
      type: 'lists_refreshed',
      projects: [],
      devices: [
      ...
      runtimeWork: {
        projects: [
          {
            project: { id: 7, name: 'Missing repo' },
            deviceWorkspaces: [
              {
                deviceId: 'device-1',
                deviceStatus: 'offline',
  ...
  test('drops an optimistic runtime task when refresh returns it under a different project identity', () => {
    const state = workbenchReducer(initialWorkbenchState, {
      type: 'lists_refreshed',
      projects: [{ id: 7, name: 'Repo', tasks: [] }],
      devices: [],
    ...
    expect(refreshed.runtimeWork?.projects).toHaveLength(1)
    expect(refreshed.runtimeWork?.totalTasks).toBe(1)
  })
