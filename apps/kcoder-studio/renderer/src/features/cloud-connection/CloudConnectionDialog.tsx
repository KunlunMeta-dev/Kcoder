          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-sm leading-5 text-text-secondary">
              {t(
                ...
                '旧版云登录与凭据同步在 KCoder Studio 中不可用。请连接 Gateway，使用目标端已配置的运行时。'
              )}
            </p>
          </div>
          <button
        ...
        {isConnected ? (
          ...
          <div className="mt-5 space-y-5">
            <div>
              ...
              >
                {t('workbench.cloud_connection_backend_url', '旧版后端地址')}
              </label>
              <input
            ...
            <form data-testid="cloud-authorization-form" onSubmit={handleAuthorizationSubmit}>
              ...
              >
                {submitting ? (
                  <>
                    <Loader2 className="h-4 w-4 animate-spin" />
                    {t('workbench.cloud_connection_waiting_authorization', '旧版云授权不可用')}
                  </>
                ) : (
                  <>
                    <Cloud className="h-4 w-4" />
