import type { ModelSelectionMode } from './model-selection'
...
export interface UserPreferences {
  ...
  new_chat_model_selection?: ModelSelectionConfig | null
...
export interface ProjectWithTasks {
  /** Present only for a synthesized built-in target project. */
  nameKey?: 'currentComputer'
  id: number
  name: string
...
export type RuntimeName = 'kcoder' | 'codex' | 'claude_code' | 'claude' | string
...
export interface NormalizedRuntimeMessage {
  ...
  continuedByAttemptId?: string | null
  providerFailure?: import('@wegent/chat-core').ProviderFailureDetails | null
  provider_failure?: import('@wegent/chat-core').ProviderFailureDetails | null
  id: string
...
export interface RuntimeTaskSummary {
  ...
  running?: boolean
  /**
   * Authoritative activity published by an app-server that negotiated
   * `threadRunSummaryV1`. Distinguishes a pending approval, background work and
   * pending delivery, which `running` cannot express. Absent for older servers.
   */
...
export interface RuntimeProjectAppearance {
  ...
  marker?:
    { kind: 'icon'; icon: string } | { kind: 'emoji'; emoji: string } | Record<string, unknown>
}

export interface RuntimeDeviceWorkspace {
  /** False means omitted tasks are not evidence of deletion. */
  threadsComplete?: boolean
  /** Snapshot-wide issue total, not a sum of repeated pagination metadata. */
  threadListIssueCount?: number
  /** True when the last thread-list scan failed outright; the count is not authoritative. */
...
export interface RuntimeSessionModesRequest {
  address?: RuntimeTaskAddress
  deviceId?: string
  workspacePath?: string
}
...
export interface RuntimeSessionModesResponse {
  ...
  moaPlanError?: string | null
  /** Recorded settings-template binding of the session; absent/`null` when it uses the default. */
  settingsTemplate?: { id: string; revisionSha256: string } | null
}

export interface RuntimeSendRequest {
  /** Continue a failed logical turn without submitting another user message. */
  retryFromTurnId?: string
  retryFromAttemptId?: string
  retryModelConfiguration?: 'snapshot' | 'current'
  turnMode?: RuntimeTurnMode
  address: RuntimeTaskAddress
  message: string
  clientMessageId?: string
  /**
   * Declares a deliberate new attempt for a `clientMessageId` that was already
   * committed (regenerate, or the legacy re-send of a failed turn). Without it a
   * repeated submission is refused so a retry cannot run the turn twice.
   */
  resubmit?: boolean
  ephemeral?: boolean
  modelId?: string
  /** Explicit intent; omission preserves the existing conversation choice. */
...
export interface RuntimeGoalCreateInput {
  objective: string
  mode?: 'standard' | 'arrangement' | 'strict' | null
  verificationKind?: 'artifact' | 'answer' | null
...
export interface RuntimeWorktreeArchivePreview {
  path: string
  state: string
  revision: number
  contentToken: string
  ...
  commitsSinceCreation: number
  requiresConfirmation: boolean
  archiveAllowed: boolean
  blockingReasons: string[]
}

export interface RuntimeWorktreeArchivePreviewResponse {
  success: boolean
  deviceId: string
  preview: RuntimeWorktreeArchivePreview
}

export interface RuntimeWorktreePathRequest {
  deviceId: string
  path: string
}

export interface RuntimeWorktreeArchiveRequest extends RuntimeWorktreePathRequest {
  expectedRevision: number
  expectedContentToken: string
  riskAccepted: boolean
}

export interface RuntimeWorktreeRevisionRequest extends RuntimeWorktreePathRequest {
  expectedRevision: number
}

export interface RuntimeWorktreeForgetRequest extends RuntimeWorktreeRevisionRequest {
...
export interface RuntimeTaskShortenWaitResponse {
  accepted: boolean
  shortened: boolean
  taskId?: string
  error?: string | null
...
export interface RuntimeTaskCreateRequest extends RuntimeExecutionModes {
  projectId?: number
...
export interface DeviceCommandResponse {
  ...
  stdout:
    string | string[] | LocalDeviceSkill[] | SkillDirectorySetupResult | Record<string, unknown>
  stderr: string
  error?: string
  duration?: number
  timed_out?: boolean
...
export interface ChatErrorPayload {
  providerFailure?: import('@wegent/chat-core').ProviderFailureDetails
  taskId?: string
  subtaskId?: string
  error: string
  type?: string
...
export type SystemSkillInstallState =
  'not_installed' | 'installed' | 'update_available' | 'unavailable' | 'failed'
...
export type MCPInstallState =
  'not_installed' | 'installed' | 'update_available' | 'unavailable' | 'failed' | 'uninstalled'
...
export type ChatBlockType =
  'text' | 'tool' | 'thinking' | 'plan' | 'error' | 'guidance' | 'file_changes'
...
export interface RuntimeSubagentSteerRequest {
  address: RuntimeTaskAddress
  agentId: string
  message: string
  clientMessageId?: string
...
export interface RuntimeSubagentSteerResponse {
  accepted: boolean
  success?: boolean
  taskId?: string
  agentId: string
...
export interface RuntimeSubagentArtifactRequest {
  address: RuntimeTaskAddress
  path: string
  kind: 'output' | 'transcript'
}

export interface RuntimeSubagentArtifactResponse {
  content: string
  truncated: boolean
  name: string
  path: string
