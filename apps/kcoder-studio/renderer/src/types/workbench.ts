...
export type RuntimeSubagentStatusState = 'running' | 'paused' | 'done' | 'interrupted'
...
export interface RuntimeSubagentStatus {
  ...
  appliedSteerMessageIds?: string[]
  observedSteerMessageIds?: string[]
...
> & {
  ...
  /** A continuation was accepted for this failed attempt; retain its failure facts. */
  continuationAccepted?: boolean
  attemptId?: string
  continuedByAttemptId?: string
  /** Historical re-submission: points to the later response without erasing this failure. */
  retriedByMessageId?: string
...
export interface RuntimePaneQueuedMessage extends QueuedWorkbenchMessage {
  modelSelectionMode?: RuntimeSendRequest['modelSelectionMode']
  turnMode?: RuntimeSendRequest['turnMode']
...
export type CloudSyncTrigger =
  'bootstrap' | 'manual-refresh' | 'cloud-connection' | 'device-event' | 'runtime-event' | 'poll'
export type CloudRuntimeAvailability =
  'idle' | 'syncing' | 'ready' | 'partial' | 'stale' | 'unavailable'
