...
describe('KCoder gateway runtime task isolation', () => {
  ...
  test('projects app-server questions and returns the selected answer on the same turn', async () => {
    ...
    expect(client.responses).toContainEqual({
      ...
      result: {
        answers: { 'question-1': { answers: ['SQLite'] } },
        // The connection negotiated interactionBindingV1, so the reply must
        // name the interaction it answers.
        questionId: 'question-1000000',
        threadId: 'thread-1',
        turnId: 'turn-1',
      },
  ...
  test('closes and rolls back a client that returns a malformed thread result', async () => {
    ...
    ).rejects.toMatchObject({ name: 'ThreadCreationUnknownError' })
    expect(client.requests.filter(request => request.method === 'thread/start')).toHaveLength(1)
    expect(client.requests.filter(request => request.method === 'thread/creation/read')).toHaveLength(1)
