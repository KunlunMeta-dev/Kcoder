...
describe('KCoder gateway runtime task isolation', () => {
  test('plugin operations open only the explicitly selected remote workspace', async () => {
    ...
    expect(client.request).toHaveBeenCalledWith('plugin/uninstall', { pluginId: 'demo@market' })
  })
  ...
  test('uses runtime-neutral project keys and KCoder task identities for KCoder targets', async () => {
    ...
    expect(client.requests).toContainEqual({
      method: 'thread/start',
      params: { cwd: '/workspace', model: 'kunlunmeta' },
    })
    expect(client.requests).toContainEqual({
      method: 'turn/start',
      params: {
  ...
  test('persists context settings, applies them to prompts, and exposes an empty hook catalog', async () => {
    ...
    await expect(runtime.request('runtime.hooks.list', {})).resolves.toEqual({ plugins: [] })
  ...
  test('forwards the model catalog and keeps the selected model on follow-up turns', async () => {
    ...
    expect(client.requests).toContainEqual({
      method: 'thread/start',
      params: { cwd: '/workspace', model: 'provider-special::model-special' },
    })
