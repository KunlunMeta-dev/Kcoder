...
function executionReasoningEffort(execution: Record<string, unknown>): string | null {
...
function executionServiceTier(execution: Record<string, unknown>): string | null {
  return text(record(execution.model_config).service_tier)
}

function executionSelectedModel(
...
export class KCoderGatewayRuntime {
  ...
  private readonly backgroundJobById = new Map<string, GatewayBackgroundJob>()
  ...
  private readonly handleServersChanged = () => {
  ...
  private servers(): Promise<GatewayServer[]> {
    this.serversPromise ??= this.loadServers()
  ...
  private async connectClient(
    ...
    client.addEventListener('notification', event => {
      ...
      if (message.method) {
        ...
        const notificationQueue = (this.notificationQueueByClient.get(client) ?? Promise.resolve())
          .then(() =>
            this.forwardNotification(message.method!, message.params ?? {}, server.id, client)
          )
          .catch(error => {
            if (typeof window === 'undefined' || !('__TAURI_INTERNALS__' in window)) return
            console.error('[KCoder] Failed to forward app-server notification', error)
          })
    ...
    client.addEventListener('request', event => {
      ...
      void (this.notificationQueueByClient.get(client) ?? Promise.resolve())
        .then(() => this.forwardServerRequest(client, message, server.id))
        .catch(error => {
    ...
    await scanWorkspaces(
      ...
      async server => {
        ...
        try {
          ...
          console.warn(`[KCoder] 无法读取 ${server.label} 的工作区注册表`, error)
        }
  ...
  async request(method: string, rawParams: unknown): Promise<unknown> {
    ...
    if (method === 'runtime.diagnostics.request') {
      ...
      if (client.supportsExperimental?.('storageDiagnosticsV1') !== true)
        throw new Error('请升级目标 KCoder 以使用存储与诊断')
    ...
    if (method === 'runtime.automations.request') {
      const operation = text(params.method)
      if (!operation || !['cron/list', 'cron/create', 'cron/delete'].includes(operation))
      ...
      if (client.supportsExperimental?.('projectAutomations') !== true)
        throw new Error('The target KCoder runtime must be upgraded to support scheduled tasks')
      return client.request(operation, record(params.params))
    }
    ...
    if (method === 'runtime.plugins.request') {
      const { PLUGIN_RPC_METHODS } = await import('./gatewayPluginApi')
      ...
      const client = await this.commandClient(
        await this.serverForParams(params),
    ...
    if (isKCoderRuntimeMethod(method, 'modelsList')) {
      ...
      return client.request('runtime.models.list', {
        ...params,
      ...
      for (const task of candidates) {
        ...
        if (snippet === null) {
          try {
            ...
            console.warn(`[KCoder] 无法搜索任务 ${task.taskId} 的 transcript`, error)
            continue
          }
        }
        items.push({
  ...
  private async hydrateArchivedWorktreeTasks(servers: GatewayServer[]): Promise<void> {
    for (const server of servers) {
      ...
      try {
        ...
        if (!this.disposed) {
          console.warn(`[KCoder] 无法读取 ${server.label} 的工作树归档索引`, error)
        }
        continue
      }
    ...
    await scanWorkspaces(
      ...
      async workspace => {
        ...
        try {
          ...
          console.warn(`[KCoder] 无法同步 ${server.label} 的持久任务`, error)
        }
      },
      cancelled
    )
  ...
  private async createTask(params: Record<string, unknown>, server: GatewayServer) {
    ...
    const workspacePath = text(params.workspacePath) ?? server.workspacePath ?? '/'
    const client = await this.connectClient(server, 'runtime', workspacePath)
    let prompt: string
    ...
    try {
      ...
      if (!this.completedTurnKeys.delete(`${taskId}:${turnId}`)) {
      ...
      }).catch(emitError => console.warn('[KCoder] 无法创建首轮失败投影', emitError))
      await emit(EXECUTOR_EVENT, {
      ...
      }).catch(emitError => console.warn('[KCoder] 无法报告首轮发送失败', emitError))
    }
    return {
      accepted: true,
      deviceId: server.id,
    ...
    try {
      ...
      const deleteUnownedFork = () =>
        sourceClient
          ...
          .catch(deleteError =>
            console.warn('[KCoder] 无法清理恢复失败后的分叉 thread', deleteError)
          )
      ...
      try {
        ...
        await client
          .request('thread/delete', { threadId })
          .catch(deleteError => console.warn('[KCoder] 无法清理分叉失败后的 thread', deleteError))
        client.close()
        throw error
      }
      return {
  ...
  private async releaseRestartClient(client: GatewayClient) {
    ...
    for (const [taskId, taskClient] of this.clientByTask) {
      ...
      for (const [jobId, job] of this.backgroundJobById) {
        if (job.taskId === taskId) this.backgroundJobById.delete(jobId)
      }
    }
  ...
  private async sendTask(
    ...
    const updatedAt = Date.now()
    if (task && selectedModel && selectedModel !== task.model) {
      await this.updateTaskMetadata(
        task,
        { model: selectedModel },
        {
    ...
    const prompt = await this.promptWithAttachmentsForClient(
      rawPrompt,
      execution,
      server,
      client,
    ...
    try {
      ...
      }).catch(emitError => console.warn('[KCoder] 无法创建 turn/start 失败投影', emitError))
      await emit(EXECUTOR_EVENT, {
        event: 'response.failed',
        payload: {
          taskId,
      ...
      }).catch(emitError => console.warn('[KCoder] 无法报告 turn/start 失败', emitError))
      throw error
    }
    this.pendingTurnStartByTask.delete(taskId)
  ...
  private async resumeTaskOnce(task: GatewayTask): Promise<GatewayClient> {
    ...
    try {
      const result = await client.request<{ thread?: { id?: string } }>('thread/resume', {
        threadId: task.threadId,
      })
  ...
  private async emitSubagentSnapshot(task: GatewayTask, client: GatewayClient): Promise<void> {
    ...
    for (const rawAgent of Array.isArray(result.agents) ? result.agents : []) {
      ...
      if (!agentId || (!['pending', 'running', 'paused'].includes(status) && queueDepth === 0)) {
        continue
      }
      const agentName = text(agent.agentName) ?? 'spawn_agent'
      const existingJob = this.backgroundJobById.get(agentId)
  ...
  private recoverTaskAfterDisconnect(task: GatewayTask, disconnectProjection: Promise<void>): void {
    ...
    const recovery = Promise.resolve().then(async () => {
      ...
      }).catch(error => console.warn('[KCoder] 无法报告任务重连状态', error))
      ...
      for (const delay of delays) {
        ...
        try {
          ...
          }).catch(error => console.warn('[KCoder] 无法报告任务重连成功', error))
          return
        } catch (error) {
          lastError = error
        }
      }
      console.warn(`[KCoder] 无法自动恢复任务连接 ${task.taskId}`, lastError)
      await emit(EXECUTOR_EVENT, {
        event: 'response.block.updated',
        payload: {
          taskId: task.taskId,
      ...
      }).catch(error => console.warn('[KCoder] 无法报告任务重连失败', error))
      throw new Error(
  ...
  private async projectTaskClientDisconnect(taskId: string, client: GatewayClient) {
    ...
    await this.notificationQueueByClient.get(client)?.catch(error => {
      console.warn('[KCoder] 断线前通知投影失败', error)
    })
    ...
    for (const [key, pending] of this.pendingQuestionByKey) {
      ...
      }).catch(error => console.warn('[KCoder] 无法关闭已断线的交互请求', error))
    }

    if (this.appServerDisconnectedClients.has(client)) {
      for (const [jobId, job] of this.backgroundJobById) {
        if (job.taskId !== taskId) continue
        this.backgroundJobById.delete(jobId)
        ...
        await emit(EXECUTOR_EVENT, {
          ...
          console.warn('[KCoder] 无法结束已退出 app-server 的后台子 Agent 工具块', error)
        )
        await emit(EXECUTOR_EVENT, {
          event: 'response.subagent.activity',
          payload: {
          ...
          console.warn('[KCoder] 无法结束已退出 app-server 的后台子 Agent 状态', error)
        )
      }
    }
    ...
    if (active && !preserveInternalTurn) {
      ...
      await emit(EXECUTOR_EVENT, {
        ...
        console.warn('[KCoder] 无法报告任务连接中断', error)
      })
    }
  }
  ...
  private async forwardServerRequest(
    ...
    if (
      ...
      pendingQuestion.autoResolutionTimer = window.setTimeout(() => {
        ...
        }).catch(error => console.warn('[KCoder] 自动解决交互请求失败', error))
      }, autoResolutionMs)
    }
    ...
    if (!validAction) {
      ...
      await emit(EXECUTOR_EVENT, {
        ...
        payload: {
          ...
          data: {
            message:
              'Declined a permission request that could not be identified or displayed safely',
          },
        },
      })
      return
  ...
  private async forwardNotification(
    ...
    if (method === 'approval/resolved') {
      ...
      const explanation =
        reason === 'timeout'
          ? 'Permission request timed out and was declined automatically.'
          : reason === 'response_error'
            ? 'Permission response failed to send; the operation was declined.'
            : reason === 'cancelled'
    ...
    if (method === 'agent/steer/applied') {
      ...
      const job = this.backgroundJobById.get(agentId)
      await emit(EXECUTOR_EVENT, {
        event: 'response.subagent.activity',
        payload: {
          ...base,
    ...
    if (method === 'item/event') {
      ...
      if (eventType === 'background_job_progress' || eventType === 'background_job_promoted') {
        const jobId = text(event.id)
        const job = jobId ? this.backgroundJobById.get(jobId) : undefined
        if (!jobId || !job) return
        await this.emitBackgroundToolLifecycle(jobId, job, 'running')
        await emit(EXECUTOR_EVENT, {
          event: 'response.subagent.activity',
...
export function installGatewayPageRuntime(
  ...
  if (installation)
    void syncDesktopPreferences(readPreferences()).catch(error => {
      console.error('Failed to synchronize desktop preferences', error)
    })
  return installation
}
