...
import { DesktopMenuBar } from '@/kcoder/DesktopMenuBar'
...
import { DesktopFileMenuBridge } from '@/kcoder/DesktopFileMenuBridge'
...
function AppRoutes({ onWorkbenchStartupReadyChange, onOpenWeworkForAppshot }: AppRoutesProps = {}) {
  ...
  return (
    <WorkbenchProvider user={user} onStartupReadyChange={onWorkbenchStartupReadyChange}>
      ...
      {desktopHost()?.onMenuCommand && <DesktopFileMenuBridge />}
...
function MainApp() {
  ...
  useEffect(() => {
    ...
    getAppPreferences()
      ...
      .catch(error => {
        console.error('[KCoder Studio] Failed to initialize language preference:', error)
        if (!cancelled) {
...
function AppShell() {
  ...
  const titlebarOverlaysContent = isTauri && activeAppKey === 'studio'
  const showChromeTitlebar = isTauri && activeAppKey !== 'studio'
  ...
  useEffect(() => {
    ...
    const loadKeybindings = async () => {
      try {
        ...
        console.error('[KCoder Studio] Failed to load keybindings:', error)
      }
    }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      console.warn(
        `[KCoder Studio] Workbench startup has not completed after ${WORKBENCH_STARTUP_REVEAL_TIMEOUT_MS}ms; revealing shell while requests continue.`
      )
      ...
      >
        <div
          className={cn(
            ...
            titlebarOverlaysContent && 'relative'
          )}
        >
          <DesktopMenuBar />
          {showChromeTitlebar && (
        ...
        <div className="rounded-lg border border-border/80 bg-background/98 p-2 shadow-[0_18px_48px_rgba(0,0,0,0.18)] backdrop-blur">
          <div className="mb-1 flex items-center justify-between px-2 py-1">
            ...
            {!collapsed && (
              <button
                type="button"
                data-testid="collapse-kcoder-dev-instance-button"
