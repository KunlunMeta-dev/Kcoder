...
describe('PluginsWorkspace', () => {
  ...
  test('renders the plugin marketplace page', async () => {
    render(<PluginsWorkspace />)

    expect(screen.getByRole('heading', { name: '插件' })).toBeInTheDocument()
    expect(screen.getByText('通过插件扩展 KCoder Studio 能力')).toBeInTheDocument()
    expect(await screen.findByTestId('plugins-search-input')).toHaveAttribute(
      'placeholder',
      '搜索插件'
    )
    expect(screen.getByTestId('plugins-search-input')).toHaveClass('h-11', 'rounded-xl')
    ...
    expect(screen.getByTestId('plugins-marketplace-tab-default')).toHaveTextContent('云端插件市场')
  ...
  test('shows trial request errors in the installed plugin detail', async () => {
    mockCodexAppServerInvoke()
    render(<PluginsWorkspace />)
    await userEvent.click(await screen.findByTestId('plugin-marketplace-install-101'))
    ...
    vi.mocked(invoke).mockImplementation((command, args) => {
      const request = args as { params?: { method?: string } } | undefined
      if (request?.params?.method === 'plugin/read') {
        return Promise.reject(new Error('Fixture plugin trial failed'))
      }
    ...
    expect(await screen.findByTestId('plugin-detail-action-error')).toHaveTextContent(
      'Fixture plugin trial failed'
    )
    await userEvent.click(screen.getByTestId('plugin-detail-dismiss-error'))
  ...
  test('shows installation failure inside detail and allows dismissing and retrying', async () => {
    render(<PluginsWorkspace />)
    await screen.findByText('Documents')
    await userEvent.click(screen.getByTestId('plugin-marketplace-row-101'))
    vi.mocked(fetch).mockRejectedValueOnce(new Error('Fixture installation failed'))
    await userEvent.click(screen.getByTestId('plugin-detail-toggle-101'))
    // The raw server text stays in diagnostics; the user sees classified copy.
    expect(await screen.findByTestId('plugin-detail-action-error')).toHaveTextContent(
      zh.workbench.plugins_error_unknown
    )
    expect(screen.getByTestId('plugin-detail-toggle-101')).toBeEnabled()
    await userEvent.click(screen.getByTestId('plugin-detail-dismiss-error'))
    expect(screen.queryByTestId('plugin-detail-action-error')).not.toBeInTheDocument()
    await userEvent.click(screen.getByTestId('plugin-detail-toggle-101'))
    await waitFor(() =>
  ...
  test('Enter on the install button installs without opening the parent detail row', async () => {
    render(<PluginsWorkspace />)
    const button = await screen.findByTestId('plugin-marketplace-install-101')
    button.focus()
    await userEvent.keyboard('{Enter}')
    await waitFor(() => expect(button).toHaveTextContent('在对话中试用'))
    expect(screen.queryByTestId('plugin-detail-back-button')).not.toBeInTheDocument()
  })

  test('keeps marketplace install actions available after a row installation failure', async () => {
    const diagnostic = vi.spyOn(console, 'error').mockImplementation(() => {})
    render(<PluginsWorkspace />)
    const button = await screen.findByTestId('plugin-marketplace-install-101')
    // A structured gateway refusal must reach the user as its own message
    // rather than as the generic one.
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent(zh.workbench.plugins_error_forbidden)
    expect(diagnostic).toHaveBeenCalledWith('[KCoder Studio plugins] install failed', {
    ...
    expect(screen.getByTestId('plugin-marketplace-install-101')).toBeEnabled()
    await userEvent.click(screen.getByTestId('plugin-marketplace-dismiss-error'))
  ...
  test('browses the selected target without trusting or saving the selected folder', async () => {
    const list = vi.fn().mockResolvedValue(['plugins'])
    const getHome = vi.fn().mockResolvedValue('/remote/home')
    const create = vi.fn()
    render(
      <PluginsWorkspace
        targetDeviceId="remote-fixture"
        directoryPicker={{
          device: {
            id: 9,
            device_id: 'remote-fixture',
            name: 'Remote',
            status: 'online',
            is_default: false,
          },
    ...
    await userEvent.click(screen.getByTestId('plugins-marketplace-browse-directory'))
    await userEvent.click(await screen.findByTestId('device-folder-entry-button'))
    await userEvent.click(screen.getByTestId('confirm-device-folder-picker-button'))
    expect(screen.getByTestId('plugins-marketplace-path-input')).toHaveValue('/remote/home/plugins')
    expect(getHome).toHaveBeenCalledWith('remote-fixture')
    expect(list).toHaveBeenCalledWith('remote-fixture', '/remote/home')
    expect(create).not.toHaveBeenCalled()
    expect(screen.getByTestId('plugins-marketplace-config-dialog')).toBeInTheDocument()
    expect(screen.queryByTestId('device-folder-directory-list')).not.toBeInTheDocument()
  })
  ...
  test('explains target directory trust and clears the guidance after a successful retry', async () => {
    Object.defineProperty(window, '__TAURI_INTERNALS__', { configurable: true, value: {} })
    mockCodexAppServerInvoke({ marketplaces: [{ name: 'existing', path: '/Users/test/existing' }] })
    const original = vi.mocked(invoke).getMockImplementation()!
    let deny = true
    vi.mocked(invoke).mockImplementation((command, args) => {
      const request = args as { params?: { method?: string } } | undefined
      if (request?.params?.method === 'marketplace/add' && deny) {
        return Promise.reject(
          new Error(
            'project marketplace C:\\Users\\test\\project\\marketplace.json requires folder trust'
          )
        )
      }
    ...
    fireEvent.change(screen.getByTestId('plugins-marketplace-path-input'), {
      target: { value: '/Users/test/project' },
    })
    await userEvent.click(screen.getByTestId('plugins-marketplace-save-button'))
    expect(await screen.findByTestId('plugins-marketplace-trust-help')).toHaveTextContent(
      'kcoder trust add --path'
    )
    expect(screen.getByTestId('plugins-marketplace-path-input')).toHaveValue('/Users/test/project')
    deny = false
    await userEvent.click(screen.getByTestId('plugins-marketplace-save-button'))
    await waitFor(() =>
      expect(screen.queryByTestId('plugins-marketplace-config-dialog')).not.toBeInTheDocument()
    )
    expect(screen.queryByTestId('plugins-marketplace-trust-help')).not.toBeInTheDocument()
  })
  ...
  test('renders configured marketplaces as tabs and switches local marketplaces', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('plugins-marketplace-tab-local-team')).toHaveAttribute(
        'aria-pressed',
        'true'
      )
    )
    ...
    mockCodexAppServerInvoke({
      skills: [
        ...
        },
        {
          ...
          path: '/Users/dev/.codex/skills/disabled-skill/SKILL.md',
          scope: 'user',
          enabled: false,
        },
      ],
