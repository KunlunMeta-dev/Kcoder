...
describe('PluginsWorkspace', () => {
  ...
  test('shows installation failure inside detail and allows dismissing and retrying', async () => {
    ...
    await userEvent.click(screen.getByTestId('plugin-detail-toggle-101'))
    expect(await screen.findByTestId('plugin-detail-action-error')).toHaveTextContent('Fixture installation failed')
    expect(screen.getByTestId('plugin-detail-toggle-101')).toBeEnabled()
    await userEvent.click(screen.getByTestId('plugin-detail-dismiss-error'))
    expect(screen.queryByTestId('plugin-detail-action-error')).not.toBeInTheDocument()
    await userEvent.click(screen.getByTestId('plugin-detail-toggle-101'))
    await waitFor(() => expect(screen.getByTestId('plugin-detail-toggle-101')).toHaveTextContent('在对话中试用'))
  })
  ...
  test('keeps marketplace install actions available after a row installation failure', async () => {
    render(<PluginsWorkspace />)
    const button = await screen.findByTestId('plugin-marketplace-install-101')
    vi.mocked(fetch).mockRejectedValueOnce(new Error('Fixture row install failed'))
    await userEvent.click(button)
    expect(await screen.findByRole('alert')).toHaveTextContent('Fixture row install failed')
