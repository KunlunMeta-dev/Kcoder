...
export function EnvironmentInfoPopover({
  ...
  // The current task may run in a managed worktree outside the project root. The
  // primary path must reflect the actual execution workspace; project roots are supplementary and cannot replace the task path.
  const workspacePaths = Array.from(
    new Set(
      [info.workspacePath, ...(info.workspaceRoots ?? [])].filter(
        (workspacePath): workspacePath is string => Boolean(workspacePath?.trim())
      )
    )
  )
      ...
      {open &&
        ...
        createPortal(
          ...
          >
            ...
            {commitError && !commitFormOpen && (
              <p
                data-testid="environment-commit-error"
                className="mt-2 rounded-md bg-red-50 px-3 py-2 text-xs text-red-600"
              >
                {commitError}
              </p>
            )}
        ...
        createPortal(
          ...
          >
            ...
            {commitError && (
              <p
                data-testid="environment-commit-error"
                className="mx-4 mb-2 rounded-md bg-red-50 px-3 py-2 text-xs text-red-600"
              >
                {commitError}
              </p>
            )}
            ...
            <div className="border-t border-border p-1.5">
              ...
              <button
                ...
                className="flex h-8 w-full items-center gap-2 rounded-lg px-2 text-left text-sm leading-[18px] text-text-secondary hover:bg-hover"
              >
