...
export function DesktopWorkbenchLayout() {
  ...
  const createPermanentWorktree = useCallback(
    async ({
      ...
      const worktreeId = `permanent-${createRandomUuid()}`
      let prepared
      try {
      ...
      try {
        ...
        await runtimeWorkApi
          .previewWorktreeArchive({ deviceId, path: workspacePath })
          .then(async response => {
            if (!response.preview.archiveAllowed || response.preview.requiresConfirmation) return
            const archived = await runtimeWorkApi.archiveWorktree({
              deviceId,
              path: workspacePath,
              expectedRevision: response.preview.revision,
              expectedContentToken: response.preview.contentToken,
              riskAccepted: false,
            })
            await runtimeWorkApi.forgetWorktree({
              deviceId,
              path: workspacePath,
              expectedRevision: archived.worktree.revision,
  ...
  useEffect(
    () => subscribeDesktopFileAction('open-folder', () => openProjectFromWorkMenu('existing')),
    [openProjectFromWorkMenu]
  )
  ...
  const renderDesktopSidebar = ({
    ...
    <DesktopSidebar
      onHistoryRefreshed={refreshWorkLists}
      user={state.user}
