...
export function DeviceStatusPrompt({
  ...
  const activeDevice = activeDeviceId
    ...
    : null
  const compatibleDevices = claudeCodeDevices.filter(isStudioCompatibleDevice)
  const outdatedDevices = claudeCodeDevices.filter(isDeviceBelowStudioVersion)
  const updateCandidates = claudeCodeDevices.filter(
    device => device.status !== 'offline' && hasStudioUpdateAvailable(device)
  )
