...
describe('DesktopEmptyTaskLauncher', () => {
  ...
  test('uses the generic heading when no project is selected', () => {
    ...
    const logo = screen.getByRole('img', { name: 'KunlunMeta' })
    expect(logo).toHaveAttribute('src', expect.stringContaining('logo/logo.png'))
    expect(logo).toHaveAttribute('width', '311')
    expect(logo).toHaveAttribute('height', '64')
    expect(logo).toHaveClass('object-contain', 'max-w-full')
  })
