...
vi.mock('@xterm/addon-fit', () => ({
  FitAddon: vi.fn().mockImplementation(function FitAddonMock() {
    ...
    testState.fitAddonInstances.push(addon)
    return addon
  }),
}))
...
describe('RemoteTerminal', () => {
  ...
  beforeEach(() => {
    ...
    testState.fitAddonInstances.length = 0
  ...
  test('skips resize observer syncs while inactive', () => {
    ...
    testState.fitAddonInstances[0].fit.mockClear()
    testState.resizeObserverInstances[0].trigger()

    expect(testState.fitAddonInstances[0].fit).not.toHaveBeenCalled()
    expect(client.resize).not.toHaveBeenCalled()
  })

  test('preserves the last visible layout instead of hiding xterm at zero width', () => {
    const now = Date.now()
    ...
    const terminal = view.getByTestId('remote-terminal')
    ...
    expect(terminal).not.toHaveAttribute('hidden')
    expect(terminal).toHaveAttribute('aria-hidden', 'true')
    expect(terminal).toHaveStyle({ width: '640px', height: '480px' })
  ...
  test('ignores active size sync rejection when the old connection is released', async () => {
    const client = createClient({
      attach: vi.fn(() => new Promise(() => undefined)),
      resize: vi.fn().mockRejectedValue(new Error('KCoder app-server 连接已断开')),
    })
    createRemoteTerminalClientMock.mockReturnValue(client)
