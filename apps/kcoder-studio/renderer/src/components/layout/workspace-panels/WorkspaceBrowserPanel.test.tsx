...
const embeddedBrowserMocks = vi.hoisted(() => ({
  ...
  EMBEDDED_BROWSER_OCCLUSION_EVENT: 'kcoder:embedded-browser-occlusion-change',
}))
...
describe('WorkspaceBrowserPanel', () => {
  ...
  test('shows the target startup prerequisite error in the browser panel', async () => {
    mockBrowserHostRect()
    embeddedBrowserMocks.openEmbeddedBrowser.mockRejectedValueOnce(
      new Error('Chromium cannot run as root with its sandbox')
    )
    render(<WorkspaceBrowserPanel active />)
    const input = screen.getByTestId('workspace-browser-url-input')
    fireEvent.change(input, { target: { value: 'example.com' } })
    fireEvent.submit(input.closest('form')!)
    expect(await screen.findByText(/Chromium cannot run as root with its sandbox/)).toBeVisible()
  })

  test.each(['submit', 'reload'])(
    'retries a failed browser start using %s instead of reloading a missing session',
    async action => {
      mockBrowserHostRect()
      embeddedBrowserMocks.openEmbeddedBrowser.mockRejectedValueOnce(
        new Error('initial startup failed')
      )
      render(<WorkspaceBrowserPanel active />)
      const input = screen.getByTestId('workspace-browser-url-input')
      fireEvent.change(input, { target: { value: 'example.com' } })
      fireEvent.submit(input.closest('form')!)
      await screen.findByText(/initial startup failed/)
      if (action === 'submit') fireEvent.submit(input.closest('form')!)
      else fireEvent.click(screen.getByTestId('workspace-browser-reload-button'))
      ...
      expect(screen.queryByText(/initial startup failed/)).not.toBeInTheDocument()
    }
  )

  test('preserves the actual browser control error for diagnosis', async () => {
    mockBrowserHostRect()
    render(<WorkspaceBrowserPanel active />)
    const input = screen.getByTestId('workspace-browser-url-input')
    fireEvent.change(input, { target: { value: 'example.com' } })
    ...
    await act(async () => {})
    embeddedBrowserMocks.reloadEmbeddedBrowser.mockRejectedValueOnce(
      new Error('browser connection was lost')
    )
    fireEvent.click(screen.getByTestId('workspace-browser-reload-button'))
    expect(await screen.findByText(/browser connection was lost/)).toBeVisible()
  })

  test('does not replace or reload an in-flight browser startup', async () => {
    mockBrowserHostRect()
    let complete!: (value: { nativeLabel: string; url: string; title: null }) => void
  ...
  test('does not expose native download management on a Gateway Web page', () => {
    const tokenMeta = document.createElement('meta')
    tokenMeta.name = 'kcoder-rpc-token'
    tokenMeta.content = 'gateway-test-token'
    document.head.appendChild(tokenMeta)

    render(<WorkspaceBrowserPanel active />)

    expect(screen.queryByTestId('workspace-browser-downloads-button')).not.toBeInTheDocument()
  ...
  test('starts page-state polling after a slow native browser open exceeds the ready timeout', async () => {
    mockBrowserHostRect()
    ...
    embeddedBrowserMocks.readEmbeddedBrowserPageState.mockRejectedValueOnce(
      new Error('No existing browser')
    )
    let resolveOpen!: (state: { nativeLabel: string; title: null; url: string }) => void
    embeddedBrowserMocks.openEmbeddedBrowser.mockReturnValueOnce(
      new Promise(resolve => {
        resolveOpen = resolve
      })
    ...
    const input = screen.getByTestId('workspace-browser-url-input')
    fireEvent.change(input, { target: { value: 'https://slow.example/one' } })
    fireEvent.submit(input.closest('form')!)
    await waitFor(() => expect(embeddedBrowserMocks.openEmbeddedBrowser).toHaveBeenCalled())
    await new Promise(resolve => window.setTimeout(resolve, 900))
    expect(screen.getByTestId('workspace-browser-loading')).toBeInTheDocument()

    embeddedBrowserMocks.readEmbeddedBrowserPageState.mockClear()
    embeddedBrowserMocks.readEmbeddedBrowserPageState.mockResolvedValue({
      nativeLabel: 'workspace-browser-native-slow',
      title: 'Page two',
      url: 'https://slow.example/two',
    })
    await act(async () => {
      resolveOpen({
        nativeLabel: 'workspace-browser-native-slow',
        title: null,
        url: 'https://slow.example/one',
      })
    })

    await waitFor(() => expect(input).toHaveValue('https://slow.example/two'), { timeout: 2500 })
  ...
  test('hides the native browser while a main webview overlay occludes it', async () => {
    ...
    window.dispatchEvent(
      new CustomEvent('kcoder:embedded-browser-occlusion-change', {
