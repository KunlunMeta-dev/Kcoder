...
vi.mock('./EmbeddedLocalTerminal', () => ({
  EmbeddedLocalTerminal: ({
    ...
    <div data-testid="embedded-local-terminal" data-session-id={sessionId} aria-hidden={!active}>
      <button
        type="button"
...
describe('WorkspacePanelCards', () => {
  ...
  test('restores persisted remote terminal tabs for the workspace', async () => {
    const restoreDeviceTerminal = vi.fn(async () => ({
      session_id: 'gateway-terminal:device-1:persisted-terminal',
      url: '',
    ...
    const restoringApi: WorkspaceSessionApi = {
      ...workspaceSessionApi,
      restoreDeviceTerminal,
    }
    window.localStorage.setItem(
      'wework.workspace.remote-terminals.v1:7:device-1:/workspace/projects/project38:configured',
      JSON.stringify({
        activeSessionId: 'gateway-terminal:device-1:persisted-terminal',
        sessions: [
          {
            session_id: 'gateway-terminal:device-1:persisted-terminal',
            project_id: 7,
    ...
    render(
      <ActualWorkspacePanelCards
        currentProject={cloudProject}
        devices={cloudDevices}
        defaultOpenTool="terminal"
        workspaceSessionApi={restoringApi}
      />
    )

    await waitFor(() =>
      expect(restoreDeviceTerminal).toHaveBeenCalledWith(
        'device-1',
        '/workspace/projects/project38',
        'gateway-terminal:device-1:persisted-terminal'
      )
    )
    ...
    await userEvent.click(
      screen.getByTestId('remote-terminal-exit-gateway-terminal:device-1:persisted-terminal')
    )

    await waitFor(() => expect(screen.queryByTestId('remote-terminal')).not.toBeInTheDocument())
    expect(screen.getByTestId('workspace-tool-launcher')).toBeInTheDocument()
    expect(startProjectTerminalMock).not.toHaveBeenCalled()
  })

  test('keeps persisted remote terminals isolated between bottom panel tabs', async () => {
    const restoreDeviceTerminal = vi.fn(
      async (_deviceId: string, _path: string | undefined, sessionId: string) => ({
    ...
    const baseKey =
      'wework.workspace.remote-terminals.v1:7:device-1:/workspace/projects/project38:configured:terminal-scope:'
    for (const [scope, sessionId] of [
      ['terminal-1', 'persisted-terminal-1'],
      ['terminal-2', 'persisted-terminal-2'],
    ]) {
      window.localStorage.setItem(
        `${baseKey}${scope}`,
        JSON.stringify({
    ...
    render(
      <>
        <ActualWorkspacePanelCards
          ...
          devices={cloudDevices}
          workspaceSessionApi={restoringApi}
          terminalPersistenceScope="terminal-1"
        />
        <ActualWorkspacePanelCards
          currentProject={cloudProject}
          devices={cloudDevices}
          workspaceSessionApi={restoringApi}
          terminalPersistenceScope="terminal-2"
        />
      </>
    )

    await waitFor(() => expect(restoreDeviceTerminal).toHaveBeenCalledTimes(2))
    expect(restoreDeviceTerminal).toHaveBeenCalledWith(
      'device-1',
      '/workspace/projects/project38',
      'persisted-terminal-1'
    )
    expect(restoreDeviceTerminal).toHaveBeenCalledWith(
      'device-1',
      '/workspace/projects/project38',
      'persisted-terminal-2'
    )
  ...
  test('discards a delayed root terminal after the workspace resolves to the project path', async () => {
    const rootStart =
      createDeferred<Awaited<ReturnType<WorkspaceSessionApi['startDeviceTerminal']>>>()
    const closeStaleTerminal = vi.fn(async () => undefined)
    const createRemoteTerminalClient = vi.fn(() => ({ close: closeStaleTerminal }))
    const resolvingApi: WorkspaceSessionApi = {
      ...workspaceSessionApi,
      createRemoteTerminalClient: createRemoteTerminalClient as never,
      startDeviceTerminal: vi.fn().mockReturnValueOnce(rootStart.promise).mockResolvedValueOnce({
        session_id: 'project-terminal',
        url: '',
        transport: 'socketio',
        device_id: 'device-1',
        type: 'terminal',
    ...
    const { rerender } = render(
      <ActualWorkspacePanelCards
        ...
        workspaceTarget={{ deviceId: 'device-1', path: '/', source: 'runtime' }}
        defaultOpenTool="terminal"
        workspaceSessionApi={resolvingApi}
      />
    )

    await waitFor(() =>
      expect(resolvingApi.startDeviceTerminal).toHaveBeenCalledWith('device-1', '/')
    )
    ...
    await waitFor(() =>
      expect(resolvingApi.startDeviceTerminal).toHaveBeenCalledWith(
        'device-1',
        '/workspace/projects/project38'
      )
    )
    ...
    rootStart.resolve({
      session_id: 'stale-root-terminal',
      url: '',
      transport: 'socketio',
      device_id: 'device-1',
      type: 'terminal',
    ...
    await waitFor(() => expect(closeStaleTerminal).toHaveBeenCalledTimes(1))
    expect(createRemoteTerminalClient).toHaveBeenCalledWith('stale-root-terminal')
  ...
  test('opens the terminal add menu from the terminal plus button', async () => {
    ...
    expect(terminals[0]).toHaveAttribute('aria-hidden', 'true')
    expect(terminals[1]).toHaveAttribute('data-session-id', 'terminal-2')
    expect(terminals[1]).toHaveAttribute('aria-hidden', 'false')

    await userEvent.click(screen.getAllByTestId('workspace-terminal-close-button')[1])

    await waitFor(() => expect(screen.getAllByTestId('workspace-terminal-tab')).toHaveLength(1))
    expect(screen.getByTestId('remote-terminal')).toHaveAttribute('data-session-id', 'terminal-1')
    expect(screen.getByTestId('remote-terminal')).toHaveAttribute('aria-hidden', 'false')
  })
