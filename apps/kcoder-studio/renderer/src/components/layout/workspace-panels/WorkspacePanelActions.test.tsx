...
describe('WorkspacePanelActions', () => {
  ...
  test('shows platform panel bindings and tracks customization and clearing', () => {
    vi.spyOn(navigator, 'platform', 'get').mockReturnValue('Win32')
    setActiveKeybindings([])
    render(<WorkspacePanelActions {...baseProps} mode="panel-toggles" />)
    const bottomTooltip = screen.getByTestId('toggle-bottom-workspace-panel-button').parentElement!
    const rightTooltip = screen.getByTestId('toggle-right-workspace-panel-button').parentElement!
    expect(bottomTooltip).toHaveTextContent('CtrlJ')
    expect(rightTooltip).toHaveTextContent('CtrlAltB')
    act(() => {
      setActiveKeybindings([
        { command: OPEN_TERMINAL_COMMAND, key: 'Control+Shift+J' },
        { command: TOGGLE_SIDE_PANEL_COMMAND, key: null },
      ])
    })
    expect(bottomTooltip).toHaveTextContent('CtrlShiftJ')
    expect(rightTooltip).not.toHaveTextContent('Ctrl')
  })
  ...
  test('announces whether the right workspace panel will open or close', () => {
    const { rerender } = render(<WorkspacePanelActions {...baseProps} />)

    expect(screen.getByTestId('toggle-right-workspace-panel-button')).toHaveAccessibleName(
      'workbench.toggle_right_workspace_panel'
    )
    rerender(<WorkspacePanelActions {...baseProps} rightPanelOpen />)
    expect(screen.getByTestId('toggle-right-workspace-panel-button')).toHaveAccessibleName(
      'workbench.close_right_workspace_panel'
    )
  })
  ...
  test('hides the IDE action for a KCoder gateway without code-server support', () => {
    render(
      <WorkspacePanelActions
        {...baseProps}
        currentProject={{
          id: 7,
          name: 'KCoder project',
          config: { execution: { targetType: 'remote', deviceId: 'kcoder-remote' } },
          tasks: [],
        }}
        devices={[
          {
    ...
    expect(screen.queryByTestId('open-code-server-titlebar-button')).not.toBeInTheDocument()
  })
