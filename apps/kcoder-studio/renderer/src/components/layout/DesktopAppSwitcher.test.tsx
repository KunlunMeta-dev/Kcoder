...
describe('DesktopAppSwitcher', () => {
  ...
  test('renders 任务 as a compact product menu with a divider', () => {
    ...
    expect(screen.getByTestId('chrome-tab-studio')).toHaveTextContent('任务')
    expect(screen.getByTestId('chrome-tab-studio')).toHaveAttribute('aria-haspopup', 'menu')
  })

  test('hides Kanban while experimental features are disabled', () => {
    render(<DesktopAppSwitcher activeApp="wework" onNavigate={vi.fn()} />)

    fireEvent.click(screen.getByTestId('chrome-tab-studio'))
  ...
  test('keeps Kanban navigable while disconnected', () => {
    ...
    fireEvent.click(screen.getByTestId('chrome-tab-studio'))
    expect(screen.getByTestId('app-switcher-option-studio').parentElement).toHaveClass('pl-2')
    expect(screen.getByTestId('app-switcher-option-studio')).toHaveTextContent(
      '任务使用 AI 解决具体问题'
    )
