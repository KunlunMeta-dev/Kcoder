...
function WorkbenchSearchDialogPanel({
  ...
  const selectedOptionId = selectedItem ? `${listboxId}-option-${selectedIndex}` : undefined
  ...
  const liveStatusLabel = useMemo(() => {
    ...
    if (hasQuery && searchedQuery === trimmedQuery && items.length > 0) {
      return t('workbench.search_results_count', { count: items.length })
    }
    return ''
  }, [hasQuery, items.length, searchedQuery, statusLabel, t, trimmedQuery])
  ...
  return (
    ...
    >
      <div
        ...
        className="w-full max-w-[520px] overflow-hidden rounded-[18px] bg-white shadow-[0_24px_70px_rgba(15,23,42,0.20)] ring-1 ring-black/5"
      >
        <h2 id={titleId} className="sr-only">
          {t('workbench.search_conversations')}
        </h2>
        ...
        <div className="max-h-[420px] min-h-[120px] overflow-y-auto px-2 py-3">
          ...
          <div id={listboxId} role="listbox" aria-label={t('workbench.search_conversations')}>
            {items.map((item, index) => (
              <div
                ...
                className={cn(
                  'flex w-full min-w-0 cursor-pointer items-start gap-3 rounded-lg px-3 py-2.5 text-left',
              ...
              >
                ...
                <div className="flex max-w-[150px] shrink-0 flex-col items-end gap-1 text-xs text-text-muted">
                  <span className="max-w-full truncate">
                    {item.project?.name || item.deviceName}
                  </span>
                </div>
              </div>
            ))}
          ...
          {statusLabel && (
            <div
              className="flex min-h-[96px] items-center justify-center px-4 text-sm text-text-muted"
