...
describe('mergeRuntimeTranscriptMessages', () => {
  test('reset drops stale completed rows but keeps the live turn', () => {
    const merged = mergeRuntimeTranscriptMessages(
      [message({ id: 'fresh', turnId: 'fresh-turn' })],
      [
        message({ id: 'old', turnId: 'old-turn' }),
        message({ id: 'live-user', role: 'user', turnId: 'live-turn' }),
        message({ id: 'live-assistant', status: 'streaming', turnId: 'live-turn' }),
      ],
      true
    )
    expect(merged.map(item => item.id)).toEqual(['fresh', 'live-user', 'live-assistant'])
  })
