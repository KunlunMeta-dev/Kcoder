...
export function mergeRuntimeTranscriptMessages(
  ...
  if (resetHistory) {
    const active = (message: WorkbenchMessage) =>
    ...
    const turns = new Set(
      existingMessages.filter(active).flatMap(message => (message.turnId ? [message.turnId] : []))
    )
    existingMessages = existingMessages.filter(
      message => active(message) || (message.turnId && turns.has(message.turnId))
    )
  }
