...
export function MobileDrawer({
  ...
  const background = getWorkbenchBackground(
    appearance?.appearance ?? defaultAppearance,
    appearance?.resolvedMode ?? 'light'
  )
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-critical isolate flex h-dvh select-none flex-col overflow-hidden bg-[rgb(var(--color-mobile-drawer))] pb-[max(24px,env(safe-area-inset-bottom))] pt-[max(20px,env(safe-area-inset-top))] text-text-primary"
    >
      ...
      <header className="flex shrink-0 items-center justify-between gap-4 px-6">
        ...
        <div className="flex items-center gap-5">
          ...
          <button
            ...
            className="flex h-11 w-11 items-center justify-center rounded-full bg-surface text-sm font-semibold text-text-primary"
            aria-label={t('workbench.settings', '设置')}
          >
            <ClientAvatar fallback={getUserInitials()} />
          </button>
        </div>
      </header>
      ...
      {searchOpen && (
        <div className="relative mx-6 mt-6 shrink-0">
          ...
          <input
            ...
            className="h-12 w-full select-text rounded-2xl border border-border bg-background pl-12 pr-4 text-base leading-5 text-text-primary outline-none placeholder:text-text-secondary focus:bg-surface"
            autoFocus
          />
        </div>
      )}
                    ...
                    chatTaskItems.map(({ workspace, task }) => {
                      ...
                      return (
                        <button
                          ...
                          className={[
                            'mx-3 flex h-12 min-w-[44px] w-[calc(100%-24px)] items-center rounded-[14px] px-3 text-left text-heading-sm font-normal leading-6 disabled:cursor-not-allowed disabled:opacity-50',
                            selectedTask
                    ...
                    >
                      ...
                      {renamingProjectId === project.id ? (
                        <input
                          ...
                          className="min-w-0 flex-1 select-text rounded-lg border border-border bg-background px-2 py-1 text-heading-sm leading-6 outline-none focus:border-text-primary"
                        />
                      ) : (
                              ...
                              {visibleTaskItems.map(({ workspace, task }) => {
                                ...
                                return (
                                  <button
                                    ...
                                    className={[
                                      'flex h-12 min-w-[44px] w-full items-center rounded-[14px] px-2 text-left text-heading-sm font-normal leading-6 disabled:cursor-not-allowed disabled:opacity-50',
                                      selectedTask
                                        ? 'bg-muted text-text-primary'
                                        : 'text-text-primary hover:bg-muted',
                                    ].join(' ')}
      ...
      {projectActionTarget && (
        ...
        >
          <div
            ...
            className="absolute w-[240px] overflow-hidden rounded-2xl border border-border bg-background p-2 shadow-[0_12px_36px_rgba(0,0,0,0.18)]"
