...
export function AppshotsSettingsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([getAppshotsStatus(), getAppPreferences()])
      ...
      .catch(loadError => {
        console.error('[KCoder Studio] Failed to load Appshots settings:', loadError)
  ...
  const updatePlaySound = async (value: boolean) => {
    ...
    try {
      ...
      console.error('[KCoder Studio] Failed to save Appshots settings:', saveError)
  ...
  const openPermissionSettings = (permission: AppshotPermission) => {
    void openAppshotsPermissionSettings(permission).catch(openError => {
      console.error('[KCoder Studio] Failed to open Appshots permission settings:', openError)
  ...
  const permissionControl = (
    ...
    !status?.supported ? (
      <span className="text-xs text-text-muted">
        {t('workbench.appshots_permission_unsupported')}
      </span>
    ) : granted ? (
        ...
        <SettingsRow
          ...
          control={
            loading ? (
              ...
              <KeyboardShortcut
                value={status?.shortcut ?? 'CommandOrControl+Shift+2'}
