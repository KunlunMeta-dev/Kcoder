...
// A per-mount identifier so a tab ignores its own broadcasts; some runtimes
// deliver BroadcastChannel messages back to the posting context.
const ownChannelSource = `tab-${Math.random().toString(36).slice(2)}`

function notifyAccountContextChange(targetId: string): void {
  try {
    const channel = new BroadcastChannel(ACCOUNT_CONTEXT_CHANNEL)
    channel.postMessage({ type: 'account-changed', targetId, source: ownChannelSource })
    channel.close()
  } catch {
    // BroadcastChannel is unavailable in older runtimes; same-context tabs
    // still converge through the gateway generation checks.
  }
}

function listenAccountContextChanges(handler: (targetId: string) => void): () => void {
  try {
    const channel = new BroadcastChannel(ACCOUNT_CONTEXT_CHANNEL)
    channel.onmessage = event => {
      const data = event.data as { targetId?: string; source?: string } | null
      if (!data || data.source === ownChannelSource) return
      if (typeof data.targetId === 'string') handler(data.targetId)
    }
...
export function KCoderAccountLogin({
  ...
  const runLogout = async () => {
    ...
    try {
      ...
      setUsername('')
      notifyAccountContextChange(server.id)
      window.dispatchEvent(new CustomEvent('kcoder:servers-changed'))
      await onChanged()
