...
import { notifyRuntimeTaskUnarchived } from '@/features/workbench/runtimeArchiveEvents'
  ...
  return results
    .flatMap(result => {
      const cleanup = result.cleanup as
        { errorCount?: number; items?: Array<{ error?: string | null }> } | undefined
...
function deleteErrorsFromResults(results: Record<string, unknown>[]) {
  return results
...
export function ArchivedConversationsSettingsPage({
  ...
  const [previewItem, setPreviewItem] = useState<ArchivedConversationItem | null>(null)
  ...
  const activeProjectKey =
    projectKey === 'all' || projectGroups.some(project => project.key === projectKey)
      ? projectKey
      : 'all'
  ...
  const visibleItems = useMemo(() => {
    const projectItems =
      activeProjectKey === 'all'
        ...
        : filteredItemsBeforeProject.filter(item => itemProjectKey(item) === activeProjectKey)
    return sortItems(projectItems, sort)
  }, [activeProjectKey, filteredItemsBeforeProject, sort])

  const groupedItems = useMemo<ArchivedProjectGroup[]>(() => {
    const fallbackProjectName = t('workbench.archived_no_project', '无项目')
    if (activeProjectKey === 'all') return groupItems(visibleItems, fallbackProjectName)
    return [
      {
        key: activeProjectKey,
        name:
          projectGroups.find(project => project.key === activeProjectKey)?.name ??
          fallbackProjectName,
        items: visibleItems,
      },
    ]
  }, [activeProjectKey, projectGroups, t, visibleItems])
  ...
  const handleUnarchive = async (item: ArchivedConversationItem) => {
    ...
    try {
      ...
      const response = await api.unarchiveConversation(address)
      if (response.accepted !== true)
        throw new Error(response.error || t('workbench.archived_unarchive_failed'))
      restored = true
      ...
      setError(
        message
          ? t(restored ? 'workbench.archived_open_failed_detail' : 'workbench.archived_unarchive_failed_detail', { message })
          : t(restored ? 'workbench.archived_open_failed' : 'workbench.archived_unarchive_failed')
      )
    } finally {
      setBusyKey(null)
    }
  ...
  const confirmDelete = async () => {
    ...
    if (pendingDelete.type !== 'single') {
      ...
      try {
        ...
        const deletedTargetKeys = new Set<string>()
        let completed = 0
        const total = deleteItems.length
        let remainingTargetItems = deleteItems
        for (let round = 0; round < ARCHIVED_DELETE_MAX_VERIFY_ROUNDS; round += 1) {
          ...
          for (const batch of batches) {
            ...
            const deleteText = deleteErrorsFromResults(response.results)
            if (deleteText) deleteErrors.push(deleteText)
            ...
            batchDeletedKeys.forEach(key => deletedTargetKeys.add(key))
            completed = deletedTargetKeys.size
            setArchivedBulkDeleteProgress({
              completed,
              total,
              running: true,
          ...
          remainingTargetItems = remainingItems.filter(item => targetKeys.has(itemAddressKey(item)))
          const remainingTargetKeys = new Set(remainingTargetItems.map(itemAddressKey))
          targetKeys.forEach(key => {
            if (!remainingTargetKeys.has(key)) deletedTargetKeys.add(key)
          })
          completed = deletedTargetKeys.size
        ...
        if (remainingTargetItems.length > 0) {
          const detail = deleteErrors.slice(0, 3).join('; ')
          setOperationError(
            detail
              ? t(
                  'workbench.archived_bulk_delete_failed_detail',
                  '删除已归档任务失败：{{message}}',
                  { message: detail }
                )
              : t(
                  'workbench.archived_bulk_delete_incomplete',
                  '仍有 {{count}} 个已归档任务未能删除',
                  { count: remainingTargetItems.length }
                )
          )
        }
        setArchivedBulkDeleteProgress({
    ...
    try {
      ...
      setOperationError(
        message
          ? t('workbench.archived_delete_failed_detail', '删除已归档任务失败：{{message}}', {
              message,
            })
          : t('workbench.archived_delete_failed', '删除已归档任务失败')
      )
    } finally {
      setBusyKey(null)
    }
  ...
  return (
    <SettingsPage data-testid="archived-conversations-settings-page">
      ...
      {previewItem && <ArchivedConversationPreview key={previewItem.id} item={previewItem} api={api} onClose={() => setPreviewItem(null)} />}
      {lastUnarchived && (
        <div
      ...
      <div className="mt-3">
        {loading ? (
          ...
          <ArchivedConversationGroups
            groups={groupedItems}
            showHeaders={activeProjectKey === 'all'}
            busyKey={busyKey}
