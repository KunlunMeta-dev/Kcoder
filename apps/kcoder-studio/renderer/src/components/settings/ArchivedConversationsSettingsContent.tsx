...
interface ArchivedConversationGroupsProps {
  ...
  onOpen?: (item: ArchivedConversationItem) => void
...
export function ArchivedConversationsFilters({
  ...
  return (
    <div
      ...
      className="-mx-1 flex flex-col gap-2 bg-background px-1 py-3 md:flex-row md:items-center"
    >
                ...
                >
                  <div className="min-w-0 flex-1">
                    {onOpen ? (
                      <button
                        type="button"
                        data-testid={`archived-open-button-${suffix}`}
                        disabled={busyKey !== null}
                        onClick={() => onOpen(item)}
                        title={t('workbench.archived_preview_readonly')}
                        className="max-w-full truncate rounded text-left text-sm font-medium text-text-primary hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus disabled:opacity-45"
                      >
                        {item.title}
                        <span className="sr-only">
                          {' '}
                          — {t('workbench.archived_preview_readonly')}
                        </span>
                      </button>
                    ) : (
