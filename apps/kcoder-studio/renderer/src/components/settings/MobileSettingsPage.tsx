...
export function MobileSettingsPage({
  ...
  if (activePage === 'usage' && kcoderGatewayPage) {
    return (
      <main
        data-testid="mobile-usage-settings-page"
      ...
      >
        <header className="flex shrink-0 items-center justify-between">
          <button
            type="button"
            data-testid="mobile-usage-back-button"
          ...
          <h1 className="text-lg font-semibold">{t('workbench.settings_nav_usage')}</h1>
  ...
  if (activePage === 'personal') {
    return (
      ...
      >
        ...
        <section className="mt-8 min-h-0 flex-1 space-y-3 overflow-y-auto">
          <button
            type="button"
          ...
          {kcoderGatewayPage && (
            <button
              type="button"
              data-testid="mobile-settings-usage-button"
              onClick={() => setActivePage('usage')}
              className="flex min-h-[56px] w-full items-center gap-3 rounded-2xl bg-surface px-4 text-left text-base font-medium text-text-primary hover:bg-muted"
            >
              <ChartNoAxesColumn className="h-5 w-5 shrink-0 text-text-secondary" />
              <span className="min-w-0 flex-1 truncate">{t('workbench.settings_nav_usage')}</span>
              <ChevronRight className="h-5 w-5 shrink-0 text-text-muted" />
            </button>
          )}
          {
  ...
  if (activePage === 'model-settings') {
    return (
      ...
      >
        ...
        <div className="mt-6 min-h-0 flex-1 overflow-auto">
          {kcoderGatewayPage ? <KCoderProviderSettingsPage /> : <ModelSettingsPage />}
        </div>
      </main>
    )
  }
  ...
  return (
    ...
    >
      ...
      <section className="mt-8 space-y-3">
        {SHOW_PLUGINS_NAVIGATION && !kcoderGatewayPage && (
          <button
            type="button"
