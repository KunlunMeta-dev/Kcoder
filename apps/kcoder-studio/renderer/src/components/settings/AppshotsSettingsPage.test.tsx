...
describe('AppshotsSettingsPage', () => {
  ...
  test('does not expose permission actions when the host reports no capture support', async () => {
    getAppshotsStatusMock.mockResolvedValue({
      supported: false,
      shortcut: 'CommandOrControl+Shift+2',
    ...
    render(<AppshotsSettingsPage />)
    await screen.findByText('应用快照目前仅支持 macOS。')
    expect(
      screen.queryByTestId('appshots-open-accessibility-settings-button')
    ).not.toBeInTheDocument()
    expect(
      screen.queryByTestId('appshots-open-screen-capture-settings-button')
    ).not.toBeInTheDocument()
    expect(screen.getByTestId('appshots-play-sound-toggle')).toBeDisabled()
    expect(openAppshotsPermissionSettingsMock).not.toHaveBeenCalled()
  })
  ...
  test('shows the registered shortcut and automatic destination', async () => {
    ...
    expect(screen.getByLabelText(/Command|Control/)).toBeInTheDocument()
  })
