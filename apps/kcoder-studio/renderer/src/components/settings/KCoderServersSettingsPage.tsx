...
export function KCoderServersSettingsPage() {
  ...
  return (
    <SettingsPage>
      ...
      {loadError && (
        <div role="alert" className="mb-4 rounded-lg bg-red-500/10 px-3 py-3 text-sm text-red-500">
          ...
          <Button
            ...
            onClick={() => void load()}
            className="mt-2 focus-visible:ring-focus max-md:h-11"
          >
            {t('targets.retry')}
          </Button>
        </div>
      ...
      {healthUnavailable && !loadError && (
        ...
        >
          ...
          <Button
            ...
            onClick={() => void load()}
            className="ml-auto focus-visible:ring-focus max-md:h-11"
          >
            {t('targets.retry')}
          </Button>
        </div>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                    <h2
                      ...
                      title={gatewayServerLabel(server, t)}
                    >
                  ...
                  {server.transport === 'ssh' && (
                    <p
                      data-testid={`runtime-target-isolation-${server.id}`}
                      className="mt-1 text-xs text-text-secondary"
                    >
                      {t(server.security ? 'accounts.isolatedTarget' : 'accounts.sharedTarget')}
                    </p>
                  )}
                  ...
                  {server.id === 'local' ? (
                    ...
                    <>
                      <Button
                        ...
                        aria-label={t('targets.editAction', {
                          name: gatewayServerLabel(server, t),
                        })}
                        title={t('targets.editAction', { name: gatewayServerLabel(server, t) })}
                        ...
                        className="h-8 w-8 text-text-secondary focus-visible:ring-focus max-md:h-11 max-md:w-11"
                      >
                      ...
                      <Button
                        ...
                        aria-label={
                          deletingId === server.id
                            ? t('targets.deleting', { name: gatewayServerLabel(server, t) })
                            : t('targets.deleteAction', { name: gatewayServerLabel(server, t) })
                        }
                        title={t('targets.deleteAction', { name: gatewayServerLabel(server, t) })}
                        disabled={deletingId === server.id}
                        onClick={event =>
                          setDeleteCandidate({ server, trigger: event.currentTarget })
                        }
                        className="h-8 w-8 text-red-500 hover:bg-red-500/10 hover:text-red-500 focus-visible:ring-focus max-md:h-11 max-md:w-11"
                      >
                        {deletingId === server.id ? (
      ...
      {deleteCandidate && (
        <RuntimeTargetConfirmDialog
          title={t('targets.deleteTitle', { name: gatewayServerLabel(deleteCandidate.server, t) })}
