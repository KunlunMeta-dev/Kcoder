...
const archivedItem: ArchivedConversationItem = {
  ...
  workspacePath: '/Users/dev/dev/git/weekly-report',
  deviceId: 'device-1',
  deviceAddress: '203.0.113.10:1420',
  source: 'local',
...
describe('ArchivedConversationsSettingsPage', () => {
  ...
  beforeEach(() => {
    ...
    unarchiveConversation.mockReset().mockResolvedValue({ accepted: true })
  ...
  test('previews archived history without restoring or opening an active conversation', async () => {
    const getRuntimeTranscript = vi
      .fn()
      .mockResolvedValue({
        workspacePath: archivedItem.workspacePath,
        runtime: 'kcoder',
        messages: [{ id: 'history-1', role: 'assistant', content: 'Archived reply fixture' }],
      })
    ...
    render(<ArchivedConversationsSettingsPage api={api} onOpenRuntimeTask={onOpenRuntimeTask} />)
    await userEvent.click(await screen.findByTestId('archived-open-button-device-1-codex-1'))
    expect(await screen.findByText('Archived reply fixture')).toBeInTheDocument()
    expect(getRuntimeTranscript).toHaveBeenCalledWith(
      expect.objectContaining({
        taskId: archivedItem.taskId,
        deviceId: archivedItem.deviceId,
        includeFullContent: true,
      })
    )
    expect(unarchiveConversation).not.toHaveBeenCalled()
    expect(onOpenRuntimeTask).not.toHaveBeenCalled()
    await userEvent.click(screen.getByTestId('archived-preview-close'))
    expect(screen.queryByTestId('archived-preview')).not.toBeInTheDocument()
  })
  ...
  test('uses the compact Codex row and an in-app single delete confirmation', async () => {
    ...
    expect(screen.queryByText('/Users/dev/dev/git/weekly-report')).not.toBeInTheDocument()
  ...
  test('keeps the confirmation recoverable and renders an inline error when single delete fails', async () => {
    deleteArchivedConversation.mockRejectedValueOnce(new Error('managed worktree was not found'))
    render(<ArchivedConversationsSettingsPage />)
    ...
    expect(await screen.findByTestId('archived-operation-error')).toHaveTextContent(
      'managed worktree was not found'
    )
    expect(screen.getByTestId('archived-delete-confirm-dialog')).toBeInTheDocument()
  ...
  test('does not report unarchive success when the server rejects the operation', async () => {
    unarchiveConversation.mockResolvedValue({ accepted: false, error: 'Restore denied' })
    const onRefreshWorkLists = vi.fn()
    render(
      <ArchivedConversationsSettingsPage
        api={
          { listArchivedConversations, unarchiveConversation } as unknown as NonNullable<
    ...
    await screen.findByText('Greet user')
    await userEvent.click(screen.getByTestId('archived-unarchive-button-device-1-codex-1'))
    expect(await screen.findByText(/Restore denied/)).toBeInTheDocument()
    expect(screen.queryByTestId('archived-unarchive-success')).not.toBeInTheDocument()
  ...
  test('resets a selected project filter when that project disappears after refresh', async () => {
    const wegentItem = archivedItemAt(1, { projectName: 'Wegent', projectKey: 'Wegent' })
    const otherItem = archivedItemAt(2, { projectName: 'Other', projectKey: 'Other' })
    listArchivedConversations
      .mockReset()
      .mockResolvedValueOnce({ items: [wegentItem, otherItem], projectGroups: [], total: 2 })
      .mockResolvedValue({ items: [otherItem], projectGroups: [], total: 1 })
    ...
    await userEvent.click(screen.getByTestId('archived-project-filter'))
    await userEvent.click(screen.getByTestId('archived-project-option-Wegent'))
    expect(screen.queryByText('Archived conversation 2')).not.toBeInTheDocument()

    act(() => notifyWorkbenchCloudArchivesChanged())

    expect(await screen.findByText('Archived conversation 2')).toBeInTheDocument()
    await waitFor(() =>
      expect(screen.getByTestId('archived-project-filter')).toHaveTextContent('所有项目')
    )
  })
  ...
  test('does not report failed bulk deletions as completed', async () => {
    listArchivedConversations.mockReset().mockResolvedValue({
      items: [archivedItem],
      projectGroups: [],
      total: 1,
    })
    deleteArchivedConversationsBulk.mockReset().mockResolvedValue({
      results: [{ taskId: archivedItem.taskId, deleted: false, error: 'permission denied' }],
    })
    ...
    await waitFor(() => expect(deleteArchivedConversationsBulk).toHaveBeenCalledTimes(5))
    expect(screen.getByText('Greet user')).toBeInTheDocument()
    expect(screen.getByTestId('archived-bulk-delete-background-progress')).toHaveTextContent(
      '0 / 1'
    )
    expect(screen.getByTestId('archived-operation-error')).toHaveTextContent('permission denied')
  })
