...
export function UsageCharts({
  ...
  const onEscape = (event: KeyboardEvent<HTMLElement>) => {
    if (event.key !== 'Escape' || (!preview && !pinned)) return
    ...
    <section className="mb-8 space-y-6" onKeyDown={onEscape} data-testid="usage-charts">
      <div className="rounded-2xl bg-surface/40 p-5">
        ...
        <div
          ...
          onKeyDown={event => moveDate(event, 7)}
        >
              ...
              {calendar.map(day => {
                ...
                return (
                  <button
                    ...
                    onMouseEnter={() => setPreview(day.label)}
                    onMouseLeave={() => setPreview(null)}
                    onFocus={() => setPreview(day.label)}
                    onBlur={() => setPreview(null)}
                    onClick={() => inspect(day.label)}
                    className={`h-6 min-w-6 rounded-md transition-colors max-md:h-11 max-md:min-w-11 ${focusClass} ${HEAT[level]} ${!day.row ? 'border border-dashed border-text-muted/20' : ''} ${selected === day.label ? 'ring-2 ring-blue-500' : 'hover:ring-2 hover:ring-text-muted/40'}`}
                  />
                )
              })}
            </div>
      ...
      <div className="rounded-2xl bg-surface/40 p-5">
        ...
        <div className="overflow-x-auto pb-2" data-testid="usage-trend">
          <div className="flex min-w-max gap-2">
            ...
            <div className="flex flex-1" onKeyDown={event => moveDate(event, 1)}>
              {daily.map((day, index) => (
                <div key={day.label} className="min-w-6 flex-1 max-md:min-w-11">
                  <button
                    ...
                    onBlur={() => setPreview(null)}
                    onClick={() => inspect(day.label)}
                    className={`flex h-44 w-full flex-col-reverse overflow-hidden rounded-t-md border-b border-border transition-colors ${focusClass} ${selected === day.label ? 'bg-text-muted/15 ring-1 ring-blue-500' : 'bg-text-muted/5 hover:bg-text-muted/10'}`}
                  >
