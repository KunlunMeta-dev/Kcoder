...
export function UsageSettingsPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (current === generation.current) {
        setStats(null)
        setError(t('usageHistory.loadFailed'))
      }
    } finally {
  ...
  const rangeButton = (value: 7 | 30, labelKey: string) => (
    <button
      ...
      className={`rounded-lg px-3 py-1.5 text-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus max-md:min-h-11 aria-pressed:bg-background aria-pressed:font-medium aria-pressed:shadow-sm ${
