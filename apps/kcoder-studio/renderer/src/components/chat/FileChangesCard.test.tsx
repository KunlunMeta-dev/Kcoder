...
describe('FileChangesCard', () => {
    ...
    try {
      renderCard({
        summary: {
          ...
          files: [
            {
              path: '/Users/dev/dev/git/if/skills/wegent-dev/SKILL.md',
              change_type: 'modified',
              additions: 3,
              deletions: 0,
              binary: false,
      ...
      expect(previewHeader).toHaveTextContent(
        '/Users/dev/dev/git/if/skills/wegent-dev/SKILL.md'
      )
    } finally {
      vi.useRealTimers()
    }
  ...
  test('renders file rows instead of inline diff preview when summary contains a diff', () => {
    renderCard({
      summary: {
        ...
        files: [
          {
            path: 'renderer/src/components/layout/DesktopWorkbenchMain.tsx',
            change_type: 'modified',
            additions: 3,
            deletions: 2,
            binary: false,
          ...
          {
            path: 'renderer/src/components/layout/MobileWorkbenchLayout.tsx',
            change_type: 'modified',
            additions: 3,
            deletions: 2,
            binary: false,
        ...
        diff: [
          'diff --git a/renderer/src/components/layout/DesktopWorkbenchMain.tsx b/renderer/src/components/layout/DesktopWorkbenchMain.tsx',
          '--- a/renderer/src/components/layout/DesktopWorkbenchMain.tsx',
          '+++ b/renderer/src/components/layout/DesktopWorkbenchMain.tsx',
          '@@ -1 +1 @@',
          '-const name = "old"',
          '+const name = "new"',
          'diff --git a/renderer/src/components/layout/MobileWorkbenchLayout.tsx b/renderer/src/components/layout/MobileWorkbenchLayout.tsx',
          '--- a/renderer/src/components/layout/MobileWorkbenchLayout.tsx',
          '+++ b/renderer/src/components/layout/MobileWorkbenchLayout.tsx',
          '@@ -1 +1 @@',
          '-const mobile = "old"',
          '+const mobile = "new"',
        ].join('\n'),
    ...
    expect(
      screen.getByText('renderer/src/components/layout/DesktopWorkbenchMain.tsx')
    ).toBeInTheDocument()
    expect(
      screen.getByText('renderer/src/components/layout/MobileWorkbenchLayout.tsx')
    ).toBeInTheDocument()
    expect(
      screen.getByRole('button', {
        name: /renderer\/src\/components\/layout\/DesktopWorkbenchMain\.tsx/,
      })
    ).toBeInTheDocument()
    ...
    try {
      const { onOpenReview } = renderCard({
        ...
        summary: {
          ...
          files: [
            {
              path: 'renderer/src/components/chat/FileChangesCard.test.tsx',
              change_type: 'modified',
              additions: 2,
              deletions: 2,
              binary: false,
          ...
          diff: [
            'diff --git a/renderer/src/components/chat/FileChangesCard.test.tsx b/renderer/src/components/chat/FileChangesCard.test.tsx',
            '--- a/renderer/src/components/chat/FileChangesCard.test.tsx',
            '+++ b/renderer/src/components/chat/FileChangesCard.test.tsx',
            '@@ -1 +1 @@',
            '-old test',
            '+new test',
  ...
  test('confirms and reverts an active server artifact marked as revertible', async () => {
    const onRevert = vi.fn().mockResolvedValue({ ...summary, status: 'reverted' })
    renderCard({ summary: { ...summary, revertible: true }, onRevert })

    await userEvent.click(screen.getAllByTestId('revert-file-changes-button')[0])
    expect(screen.getByRole('dialog')).toBeInTheDocument()
    await userEvent.click(screen.getByTestId('confirm-revert-file-changes-button'))

    await waitFor(() => expect(onRevert).toHaveBeenCalledWith('21', expect.any(Object)))
