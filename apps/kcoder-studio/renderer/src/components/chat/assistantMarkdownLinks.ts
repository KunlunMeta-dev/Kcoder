...
const WORKSPACE_MARKDOWN_IMAGE_HOST = 'wework.local'
const WORKSPACE_MARKDOWN_IMAGE_PATH = '/markdown-workspace-image'
...
export function resolveDirectMarkdownImageSrc(src: string): string | null {
  if (!isLocalMarkdownImagePath(src)) return src
...
export function localPathFromMarkdownImageSrc(src: string): string {
  const workspacePath = decodeWorkspaceMarkdownImageSrc(src)
...
export function encodeWorkspaceMarkdownImageSrc(src: string): string {
  if (decodeWorkspaceMarkdownImageSrc(src) !== null) return src
  return `https://${WORKSPACE_MARKDOWN_IMAGE_HOST}${WORKSPACE_MARKDOWN_IMAGE_PATH}?path=${encodeURIComponent(src)}`
}

function decodeWorkspaceMarkdownImageSrc(src: string): string | null {
  try {
    const url = new URL(src)
    if (
      url.protocol !== 'https:' ||
      url.hostname !== WORKSPACE_MARKDOWN_IMAGE_HOST ||
      url.pathname !== WORKSPACE_MARKDOWN_IMAGE_PATH
    ) {
      return null
    }
...
function getAttachmentDownloadId(src: string): number | null {
  try {
    ...
    if (/^https?:\/\//i.test(src)) {
      const trustedOrigins = new Set([pageOrigin])
      try {
        trustedOrigins.add(new URL(getAttachmentImageUrl(0), pageOrigin).origin)
      } catch {
        // When the runtime API address is invalid, trust only same-origin addresses from the current page.
      }
...
export function isLocalMarkdownImagePath(src: string): boolean {
...
export function isWorkspaceMarkdownImagePath(src: string): boolean {
  if (decodeWorkspaceMarkdownImageSrc(src) !== null) return true
  if (isLocalMarkdownImagePath(src)) return true
  if (!src || src.startsWith('//') || /^[a-z][a-z0-9+.-]*:/i.test(src)) return false
  return !isAuthenticatedAttachmentImageSrc(src)
}
