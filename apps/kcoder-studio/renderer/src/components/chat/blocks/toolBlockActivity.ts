...
export type ToolActivityKind =
  'file' | 'search' | 'command' | 'create' | 'edit' | 'guidance' | 'tool'
...
export function buildProcessingDisplayRows(
  ...
  let consecutiveSubagents: ToolBlock[] = []
  ...
  const flushSubagents = () => {
    if (consecutiveSubagents.length === 0) return
    if (consecutiveSubagents.length === 1) {
      const block = consecutiveSubagents[0]
      rows.push({ type: 'block', id: block.id, block, sourceBlockIds: [block.id] })
    } else {
      rows.push({
        type: 'activity_group',
        id: getToolGroupId(consecutiveSubagents),
        blocks: consecutiveSubagents,
        label: i18n.t('tool_activity.subagent_group', {
          ns: 'chat',
          count: consecutiveSubagents.length,
        }),
      })
    }
    consecutiveSubagents = []
  }
  ...
  for (const block of blocks) {
    if (block.type === 'tool' && isSubagentToolName(block.toolName)) {
      flushCompletedTools()
      flushFileChanges()
      consecutiveSubagents.push(block)
      continue
    }
    ...
    if (block.type === 'tool' && block.toolName === RECONNECTING_TOOL_NAME) {
      flushCompletedTools()
      flushFileChanges()
      rows.push({ type: 'block', id: block.id, block, sourceBlockIds: [block.id] })
      continue
...
export function summarizeToolBlocks(blocks: ToolBlock[]): string {
  if (isSubagentActivityGroup(blocks)) {
    return i18n.t('tool_activity.subagent_group', { ns: 'chat', count: blocks.length })
  }
  if (isWebSearchActivityGroup(blocks)) {
    return i18n.t('toolStatus.searchedWeb', { ns: 'chat' })
  }
  ...
  if (stats.files > 0)
    parts.push(i18n.t('toolStatus.readCount', { ns: 'chat', count: stats.files }))
  if (stats.searches > 0) parts.push(i18n.t('toolStatus.searchedCode', { ns: 'chat' }))
  if (stats.creates > 0)
    parts.push(i18n.t('toolStatus.createdCount', { ns: 'chat', count: stats.creates }))
  if (stats.edits > 0)
    parts.push(i18n.t('toolStatus.editedCount', { ns: 'chat', count: stats.edits }))
  if (stats.guidance > 0) parts.push(i18n.t('toolStatus.guided', { ns: 'chat' }))
  if (stats.commands > 0)
    parts.push(i18n.t('toolStatus.ranCount', { ns: 'chat', count: stats.commands }))
  if (stats.tools > 0)
    parts.push(i18n.t('toolStatus.calledCount', { ns: 'chat', count: stats.tools }))
  if (stats.failedCommands > 0) {
    parts.push(i18n.t('toolStatus.failedCommandCount', { ns: 'chat', count: stats.failedCommands }))
  }
  if (stats.failedTools > 0)
    parts.push(i18n.t('toolStatus.failedToolCount', { ns: 'chat', count: stats.failedTools }))

  return parts.length > 0
    ? parts.join(' ')
    : i18n.t('toolStatus.calledCount', { ns: 'chat', count: blocks.length })
}

export function isSubagentActivityGroup(blocks: ToolBlock[]): boolean {
  return blocks.length > 0 && blocks.every(block => isSubagentToolName(block.toolName))
}
...
function formatSearchLabel(summary: { query: string; scope?: string }): string {
  return summary.scope
    ? i18n.t('toolStatus.searchedQueryScope', {
        ns: 'chat',
        query: summary.query,
        scope: summary.scope,
      })
    : i18n.t('toolStatus.searchedQuery', { ns: 'chat', query: summary.query })
}
