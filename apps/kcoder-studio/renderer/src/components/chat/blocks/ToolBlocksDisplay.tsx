...
import { ToolsCatalogContext, type ToolCatalogEntry } from '@/kcoder/toolsCatalogContext'
...
export function ToolBlocksDisplay({
  ...
  const activityStats = countProcessingActivityKinds(rows, catalog)
  ...
  const summaryDuration = hasToolActivity
    ...
    : duration.replace(new RegExp(`^${t('toolStatus.processed')}\\s*`), '')
    ...
    <>
      ...
      {previewRows.length > 0 ? (
        <LiveProcessingPreview
          rows={previewRows}
          showWaiting={
            isStreaming && hasToolActivity && !hasRunningToolActivity && processingPhase === 'live'
          }
          onOpenWorkspaceFile={onOpenWorkspaceFile}
          fileEditDurations={fileEditDurations}
          stateKey={stateKey}
    ...
    >
      {canToggle ? (
        <button
          ...
          aria-label={
            durationAriaLabel
              ? `${title} ${durationAriaLabel}`
              : `${title} ${i18n.t('toolStatus.processed', { ns: 'chat' })}`
          }
        >
          {titleContent}
        </button>
...
function ToolActivityStats({
  ...
  const catalog = useContext(ToolsCatalogContext)
  const stats = countProcessingActivityKinds(rows, catalog)
  const items = [
...
function LiveProcessingPreview({
  ...
  const collapsedScrollTopRef = useRef(0)
  ...
  const scrollToTail = useCallback(() => {
    ...
    scrollArea.scrollTop = followTailRef.current
      ? scrollArea.scrollHeight
      : collapsedScrollTopRef.current
  }, [hasExpandedDetail])

  useLayoutEffect(() => {
    scrollToTail()
  }, [rows, showWaiting, scrollToTail])
  ...
  return (
    <div className="ml-2 min-w-0 border-l border-border pl-3" data-testid="processing-live-preview">
      <div
        ref={scrollRef}
        className="processing-preview-scrollbar min-w-0 pr-3"
        data-testid="processing-live-preview-scroll"
        onScroll={event => {
          if (hasExpandedDetail) return
          const area = event.currentTarget
          collapsedScrollTopRef.current = area.scrollTop
          followTailRef.current = area.scrollHeight - area.clientHeight - area.scrollTop <= 2
        }}
        style={{
          maxHeight: hasExpandedDetail ? 'none' : '8rem',
        ...
        <div ref={contentRef} className="flex min-w-0 flex-col">
          ...
          {showWaiting ? (
            <div
              ...
              data-testid="tool-block-waiting"
            >
              <span className="waiting-thinking-text">{t('thinking.waiting')}</span>
            </div>
          ) : null}
        </div>
      </div>
...
function countProcessingActivityKinds(
  rows: ProcessingDisplayRow[],
  ...
  const addToolBlock = (block: ToolBlock) => {
    let kind = getToolActivityKind(block)
    if (kind === 'tool') {
      const group = catalog.get(block.toolName)?.group
      if (group === 'terminal') kind = 'command'
      else if (group === 'files') kind = 'file'
      else if (group === 'search') kind = 'search'
    }
...
function ToolActivityGroup({
  ...
  const isSubagentGroup = isSubagentActivityGroup(row.blocks)
  const icon = renderActivityGroupIcon(row.blocks)
  const label = isSubagentGroup
    ? t('tool_activity.subagent_group', { count: row.blocks.length })
    : summarizeToolBlocks(row.blocks)
  ...
  if (isGuidanceGroup) {
    return (
      ...
      >
        {icon}
        <span className="min-w-0 truncate">{summarizeToolBlocks(row.blocks)}</span>
      </div>
    )
  }
  ...
  return (
    <div
      ...
      data-testid={isSubagentGroup ? 'subagent-tool-group' : undefined}
    >
      <button
        type="button"
...
function getContextCompactionLabel(block: ToolBlock): string {
  if (block.status === 'error') return i18n.t('toolStatus.compactionError', { ns: 'chat' })
  if (block.status === 'done') return i18n.t('toolStatus.compactionDone', { ns: 'chat' })
  return i18n.t('toolStatus.compactionRunning', { ns: 'chat' })
}
...
function renderActivityGroupIcon(blocks: ToolBlock[]) {
  if (isSubagentActivityGroup(blocks)) {
    return <GitFork className="h-4 w-4 shrink-0" strokeWidth={1.7} aria-hidden="true" />
  }
