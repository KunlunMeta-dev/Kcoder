...
import { ToolCatalogIcon } from '@/kcoder/toolsCatalog'
import {
  useToolCatalogEntry,
  toolDisplayName,
  type ToolCatalogEntry,
} from '@/kcoder/toolsCatalogContext'
...
export function ToolBlockItem({
  ...
  const catalogEntry = useToolCatalogEntry(block.type === 'tool' ? block.toolName : undefined)
  ...
  if (isTodoWriteToolName(block.toolName)) {
  ...
  if (block.toolName === 'runtime_reconnecting') {
    const reconnecting = block.status !== 'done' && block.status !== 'error'
    const failed = block.status === 'error'
    const label = reconnecting
      ? t('tool_activity.reconnecting')
      : failed
        ? t('tool_activity.reconnect_failed')
        : t('tool_activity.reconnected')
    return (
      <div
        className={`flex min-w-0 items-center gap-1.5 truncate py-1 text-sm ${failed ? 'text-red-500' : 'text-text-muted'}`}
      ...
      >
        ...
        <span className={reconnecting ? 'tool-activity-shimmer' : undefined}>{label}</span>
      </div>
    )
  }

  const { icon, label } = getBlockLabel(
    block,
    {
  ...
  const labelContent = (
    <>
      ...
      {isRunning && <LoaderCircle className="h-4 w-4 shrink-0 animate-spin" aria-hidden="true" />}
    </>
  )
      ...
      >
        ...
        {hasDetail ? (
          <button
            ...
            aria-label={t(expanded ? 'toolStatus.collapse' : 'toolStatus.expand')}
            aria-expanded={expanded}
          >
            <svg
        ...
        <span
          ...
          className="ml-auto min-w-12 shrink-0 pl-2 text-right font-mono text-xs tabular-nums text-text-muted"
        >
          {duration}
        </span>
      </div>
...
function TodoWriteBlock({ block }: { block: ToolBlock }) {
  const { t } = useTranslation('chat')
  const todos = getTodoItems(block.toolInput)
  const isRunning = block.status !== 'done' && block.status !== 'error'
  const failed = block.status === 'error'
  const failureMessage = failed
    ? (getToolFailureMessage(block.toolOutput) ?? t('tool_activity.todo_error_fallback'))
    : null
  const title = failed ? t('tool_activity.todo_error') : t('tool_activity.todo_title')

  return (
    <section
      className="min-w-0 rounded-lg bg-surface px-3 py-2 text-sm"
      data-processing-block-id={block.id}
      data-testid="todo-write-block"
      aria-label={title}
      role={failed ? 'alert' : undefined}
      aria-live={failed ? 'assertive' : undefined}
    >
      <div className="flex min-h-7 items-center gap-2 text-text-secondary">
        {failed ? (
          <AlertCircle
            className="h-4 w-4 shrink-0 text-red-500"
            data-testid="todo-write-error-icon"
            aria-hidden="true"
          />
        ) : isRunning ? (
          <LoaderCircle className="h-4 w-4 shrink-0 animate-spin" aria-hidden="true" />
          ...
          <CheckCircle2
            ...
            data-testid="todo-write-success-icon"
            aria-hidden="true"
          />
        )}
        <span className={`font-medium ${failed ? 'text-red-500' : ''}`}>{title}</span>
        {todos.length > 0 ? (
          <span className="text-xs text-text-muted">
            {t('tool_activity.todo_progress', {
      ...
      {failureMessage ? (
        <p className="mt-1 break-words text-xs text-red-500" data-testid="todo-write-error">
          {failureMessage}
        </p>
      ) : null}
      {todos.length > 0 ? (
        <ul className="mt-1 flex min-w-0 flex-col gap-1" aria-label={t('tool_activity.todo_title')}>
          {todos.map((todo, index) => {
            ...
            return (
              <li
                ...
                className="flex min-w-0 items-start gap-2 py-0.5 text-text-secondary"
                data-testid="todo-write-item"
              >
                <span
                  className="mt-0.5 inline-flex h-4 w-4 shrink-0 items-center justify-center"
                  data-testid={`todo-write-item-${todo.status}`}
                  title={t(`tool_activity.todo_${todo.status}`)}
                >
                  {todo.status === 'completed' ? (
                    <CheckCircle2 className="h-4 w-4 text-green-600" aria-hidden="true" />
                  ) : todo.status === 'in_progress' ? (
                    <CircleDotDashed className="h-4 w-4 text-blue-500" aria-hidden="true" />
                  ) : (
                ...
                <span
                  className={`min-w-0 break-words ${todo.status === 'completed' ? 'text-text-muted line-through' : ''}`}
                >
                  {content}
                </span>
              </li>
        ...
        <p className="mt-1 text-xs text-text-muted">{t('tool_activity.todo_empty')}</p>
      )}
    </section>
  )
}

function getToolFailureMessage(output: unknown): string | undefined {
  if (typeof output === 'string') return output.trim() || undefined
  if (!isRecord(output)) return undefined
  return getStringField(output, 'error', 'message', 'reason', 'output')?.trim() || undefined
}

function SubagentToolBlock({ block }: { block: ToolBlock }) {
  const { t } = useTranslation('chat')
  const [expanded, setExpanded] = useState(false)
  const input = block.toolInput ?? {}
  const output = getSubagentOutput(block.toolOutput)
  const title =
    getStringField(input, 'task_name', 'name', 'title', 'description', 'message') ??
    t('tool_activity.subagent_fallback')
  const agentType = getStringField(input, 'agent_type', 'agentType', 'type')
  const lifecycle = getSubagentLifecycle(block)
  const statusLabel = t(`tool_activity.subagent_${lifecycle}`)

  return (
    <section
      className="min-w-0 rounded-lg bg-surface px-3 py-2 text-sm"
      data-processing-block-id={block.id}
      data-subagent-lifecycle={lifecycle}
      data-testid="subagent-tool-block"
    >
      <button
        type="button"
        data-testid="subagent-tool-toggle"
        disabled={!output}
        aria-expanded={output ? expanded : undefined}
        onClick={() => setExpanded(value => !value)}
        className="flex min-h-7 w-full min-w-0 items-center gap-2 text-left disabled:cursor-default"
      >
        <SubagentLifecycleIcon lifecycle={lifecycle} />
        <span className="min-w-0 flex-1 truncate text-text-secondary">{title}</span>
        {agentType ? <span className="shrink-0 text-xs text-text-muted">{agentType}</span> : null}
        <span
          className={`shrink-0 text-xs ${lifecycle === 'failed' ? 'text-red-500' : 'text-text-muted'}`}
        >
          {statusLabel}
        </span>
        {output ? (
      ...
      {expanded && output ? (
        <div
          className="ml-2 mt-2 border-l border-border pl-3 text-text-secondary"
          data-testid="subagent-tool-detail"
        >
          <AssistantMarkdown content={output} variant="process" />
        </div>
      ) : null}
    </section>
  )
...
type SubagentLifecycle = 'queued' | 'running' | 'paused' | 'completed' | 'failed' | 'interrupted'

function SubagentLifecycleIcon({ lifecycle }: { lifecycle: SubagentLifecycle }) {
  if (lifecycle === 'paused')
    return <CirclePause className="h-4 w-4 shrink-0 text-text-muted" aria-hidden="true" />
  ...
  if (lifecycle === 'interrupted') {
    return <CircleDotDashed className="h-4 w-4 shrink-0 text-text-muted" aria-hidden="true" />
  }
  return null
}

function getTodoItems(input: Record<string, unknown> | undefined): TodoItem[] {
  if (!input) return []
  const value = input.TodoList ?? input.todos ?? input.todo_list ?? input.plan
  if (!Array.isArray(value)) return []

  return value.flatMap(item => {
    if (!isRecord(item)) return []
    const content = getStringField(item, 'content', 'step', 'task', 'description')?.trim()
    if (!content) return []
    const rawStatus = getStringField(item, 'status')?.toLowerCase()
    const status: TodoItemState =
    ...
    return [
      {
        content,
        status,
        activeForm: getStringField(item, 'activeForm', 'active_form'),
      },
    ]
  })
}

function getSubagentLifecycle(block: ToolBlock): SubagentLifecycle {
  const outputStatus = isRecord(block.toolOutput)
    ? getStringField(block.toolOutput, 'status', 'state')?.toLowerCase()
    : undefined
  if (outputStatus === 'paused') return 'paused'
  if (block.status === 'error' || outputStatus === 'error' || outputStatus === 'failed') {
    return 'failed'
  }
  if (
  ...
  if (outputStatus === 'completed' || outputStatus === 'complete' || outputStatus === 'done') {
    return 'completed'
  }
  if (block.status === 'done' && outputStatus !== 'running' && outputStatus !== 'queued') {
    return 'completed'
  }
  if (outputStatus === 'queued') return 'queued'
  return 'running'
}

function getSubagentOutput(output: unknown): string | undefined {
  if (typeof output === 'string') return output.trim() || undefined
  if (!isRecord(output)) return undefined
  return getStringField(output, 'output', 'summary', 'result', 'message')?.trim() || undefined
}
...
function getBlockLabel(
  ...
  if (isFileCreateToolName(name)) {
    return { icon: <FileIcon />, label: getFileToolLabel(prefix.create, block, 'create') }
  }
  if (isFileEditToolName(name)) {
    return { icon: <EditIcon />, label: getFileToolLabel(prefix.edit, block, 'edit') }
  }
  if (isFileReadToolName(name)) {
    return { icon: <FileIcon />, label: getFileToolLabel(prefix.read, block, 'read') }
  }
  ...
  return getGenericToolLabel(block, genericLabels, entry)
}
...
function getGenericToolLabel(
  ...
  const name = entry?.displayName ?? toolDisplayName(block.toolName)
  const normalizedName = name.toLowerCase()

  if (!entry && normalizedName.includes('wait')) {
    const label =
      block.status === 'error'
        ? labels.waitError
        : block.status === 'done'
  ...
  return {
    icon: entry ? (
      <ToolCatalogIcon entry={entry} />
    ) : (
      <Wrench className="h-4 w-4" strokeWidth={1.7} />
    ),
    label,
  }
}
...
function getFileToolLabel(prefix: string, block: ToolBlock, action: string): string {
  ...
  if (filePaths.length > 1)
    return `${prefix} ${i18n.t('toolStatus.fileCount', { ns: 'chat', count: filePaths.length })}`
...
function fileToolFallbackLabel(status: ToolBlock['status'], action: string): string {
  const state = status === 'error' ? 'error' : status === 'done' ? 'done' : 'running'
  return i18n.t(`toolStatus.file_${action}_${state}`, { ns: 'chat' })
}

function getToolStatusPrefix(block: ToolBlock) {
  const state = block.status === 'error' ? 'error' : block.status === 'done' ? 'done' : 'running'
  return {
    running: i18n.t(`toolStatus.command_${state}`, { ns: 'chat' }),
    create: i18n.t(`toolStatus.create_${state}`, { ns: 'chat' }),
    edit: i18n.t(`toolStatus.edit_${state}`, { ns: 'chat' }),
    read: i18n.t(`toolStatus.read_${state}`, { ns: 'chat' }),
    webSearch: i18n.t(`toolStatus.webSearch_${state}`, { ns: 'chat' }),
    guidance: i18n.t(`toolStatus.guidance_${state}`, { ns: 'chat' }),
    generic: i18n.t(`toolStatus.generic_${state}`, { ns: 'chat' }),
  }
}
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 rounded-md border border-border bg-surface px-2 py-1 text-xs text-text-muted">
              <span>
                {typeof block.toolOutputOriginalChars === 'number'
                  ? t('toolStatus.unloadedChars', { count: block.toolOutputOriginalChars })
                  : typeof block.toolOutputOriginalBytes === 'number'
                    ? t('toolStatus.unloadedBytes', { count: block.toolOutputOriginalBytes })
                    : t('toolStatus.unloaded')}
              </span>
              {onLoadFullTranscript ? (
                <button
                  type="button"
                ...
                >
                  {t(loadingFullTranscript ? 'toolStatus.loadingFull' : 'toolStatus.loadFull')}
                </button>
              ) : null}
            </div>
          ) : null}
        ...
        <div className="mt-2 flex justify-end">
          {isDone && (
            <span className="flex items-center gap-1 text-xs text-text-muted">
              ...
              {t('toolStatus.succeeded')}
            </span>
          )}
          {isError && <span className="text-xs text-red-500">{t('toolStatus.failed')}</span>}
        </div>
      )}
    </div>
  )
