...
describe('toolBlockActivity', () => {
  ...
  test('extracts read file paths from shell read commands', () => {
    expect(
      getToolActivityFilePaths(
        tool('nl-1', 'nl -ba renderer/src/components/chat/blocks/toolBlockActivity.ts')
      )
    ).toEqual(['renderer/src/components/chat/blocks/toolBlockActivity.ts'])
    ...
    ).toEqual(['renderer/src/components/chat/blocks/toolBlockKinds.ts'])
  ...
  test('extracts code search summaries from shell search commands', () => {
    expect(
      ...
      scope: 'blocks',
      label:
        '已在 blocks 中搜索 ToolBlockItem|toolBlock|file_changes|renderPayload|read.*file|command',
    })

    expect(
      getToolActivitySearchItem(tool('rg-2', "rg -n '已编辑|edited|edited_file|edit.*file' studio"))
    ).toMatchObject({
      query: '已编辑|edited|edited_file|edit.*file',
      scope: 'studio',
      label: '已在 studio 中搜索 已编辑|edited|edited_file|edit.*file',
    })
  })
