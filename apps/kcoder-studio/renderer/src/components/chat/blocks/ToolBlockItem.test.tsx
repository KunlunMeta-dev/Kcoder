...
import { ToolsCatalogContext } from '@/kcoder/toolsCatalogContext'
...
describe('ToolBlockItem', () => {
  test('keeps Windows file actions specialized when catalog metadata is present', async () => {
    const user = userEvent.setup()
    const onOpenWorkspaceFile = vi.fn()
    const path = 'C:\\Users\\tester\\项目\\package.json'
    const entries = new Map([
      [
        'Read',
        {
          name: 'Read',
          displayName: 'Custom file metadata',
          description: '',
          icon: 'globe' as const,
          group: 'web',
        },
    ...
    render(
      <ToolsCatalogContext.Provider value={entries}>
        <ToolBlockItem
          block={{
            id: 'windows-read',
            subtaskId: 1,
    ...
    expect(screen.queryByText(/Custom file metadata/)).not.toBeInTheDocument()
  })
  test('bounds and sanitizes an unavailable historical tool label without changing identity', () => {
    const toolName = `removed__\u0001\u007f\u0085\u009b${'😀'.repeat(200)}`
    const block = {
      id: 'removed-tool',
      subtaskId: 1,
      type: 'tool' as const,
    ...
    const { container } = render(<ToolBlockItem block={block} />)
    expect(container.textContent).not.toContain('\u0001')
    expect(container.textContent).not.toContain('\u007f')
    expect(container.textContent).not.toContain('\u0085')
    expect(container.textContent).not.toContain('\u009b')
    expect(container.textContent).toContain('😀'.repeat(128))
    expect(container.textContent).not.toContain('😀'.repeat(129))
    expect(container.querySelector('.lucide-wrench')).not.toBeNull()
    expect(block.toolName).toBe(toolName)
  })
  ...
  test('renders shell command working directory in expanded details', async () => {
    ...
    expect(screen.getByText('cwd: /Users/dev/project')).toBeInTheDocument()
  })
  ...
  test('renders TodoWrite input as a structured checklist with semantic states', () => {
    render(
      <ToolBlockItem
        block={{
          id: 'todo-1',
          ...
          toolInput: {
            TodoList: [
              { content: '检查协议', activeForm: '正在检查协议', status: 'completed' },
              { content: '实现界面', activeForm: '正在实现界面', status: 'in_progress' },
    ...
    expect(screen.getByTestId('todo-write-block')).toHaveTextContent('任务清单')
    expect(screen.getAllByTestId('todo-write-item')).toHaveLength(3)
    expect(screen.getByTestId('todo-write-item-completed')).toHaveTextContent('检查协议')
    expect(screen.getByTestId('todo-write-item-in_progress')).toHaveTextContent('正在实现界面')
    expect(screen.getByTestId('todo-write-item-pending')).toHaveTextContent('运行测试')
  })

  test('renders TodoWrite failure with error semantics and no success icon', () => {
    render(
      <ToolBlockItem
        block={{
          id: 'todo-error',
          ...
          toolName: 'TodoWrite',
          toolInput: {
            TodoList: [{ content: '更新任务', status: 'pending' }],
          },
          toolOutput: { error: '任务存储不可写' },
          status: 'error',
          createdAt: 1770000000002,
        }}
      />
    ...
    const block = screen.getByTestId('todo-write-block')
    expect(block).toHaveAttribute('role', 'alert')
    expect(block).toHaveTextContent('任务清单更新失败')
    expect(screen.getByTestId('todo-write-error')).toHaveTextContent('任务存储不可写')
    expect(screen.getByTestId('todo-write-error-icon')).toBeInTheDocument()
    expect(screen.queryByTestId('todo-write-success-icon')).not.toBeInTheDocument()
  })

  test('renders a subagent tool as a structured status row with expandable result', async () => {
    const user = userEvent.setup()
    render(
      <ToolBlockItem
        block={{
    ...
    const block = screen.getByTestId('subagent-tool-block')
    expect(block).toHaveAttribute('data-subagent-lifecycle', 'completed')
    expect(block).toHaveTextContent('审查 Markdown 渲染链路')
    expect(block).toHaveTextContent('review')
    expect(block).toHaveTextContent('已完成')

    await user.click(screen.getByTestId('subagent-tool-toggle'))
    expect(screen.getByTestId('subagent-tool-detail')).toHaveTextContent('发现公式插件缺失。')
  })
  ...
  test.each([
    ...
    'mcp__agents__explore_agent',
  ])('keeps %s animated while queued or running and stops at terminal states', toolName => {
    const block: ProcessingBlock = {
      id: 'agent-motion',
      subtaskId: 1,
      type: 'tool',
      toolName,
      toolInput: { message: '等待子代理' },
      status: 'done',
      createdAt: 1770000000002,
    }
    const { rerender, container } = render(<ToolBlockItem block={block} />)
    for (const status of ['queued', 'running', 'queued', 'running']) {
      rerender(<ToolBlockItem block={{ ...block, toolOutput: { status } }} />)
      expect(container.querySelector('svg.animate-spin')).not.toBeNull()
      expect(screen.getByTestId('subagent-tool-block')).toHaveAttribute(
        'data-subagent-lifecycle',
        status
      )
    }
    for (const status of ['paused', 'completed', 'failed', 'interrupted', 'cancelled']) {
      rerender(<ToolBlockItem block={{ ...block, toolOutput: { status } }} />)
      ...
      if (status === 'paused') {
        ...
        expect(screen.getByTestId('subagent-tool-block')).toHaveTextContent('已暂停')
      }
    }
  })

  test('updates a subagent card when the same tool block receives its final result', async () => {
    const user = userEvent.setup()
    const runningBlock: ProcessingBlock = {
      ...
      type: 'tool',
      toolName: 'spawn_agent',
      toolInput: { message: '检查最终状态' },
      toolOutput: { agent_id: 'agent-linked-id', status: 'running' },
      status: 'done',
      createdAt: 1770000000002,
    }
    ...
    expect(screen.getByTestId('subagent-tool-block')).toHaveTextContent('运行中')
    ...
    expect(screen.getByTestId('subagent-tool-block')).toHaveTextContent('已完成')
    await user.click(screen.getByTestId('subagent-tool-toggle'))
    expect(screen.getByTestId('subagent-tool-detail')).toHaveTextContent('最终审查已完成。')
  })
    ...
    render(
      <ToolBlockItem
        block={{
          ...
          type: 'tool',
          toolName: 'Read',
          toolInput: { filePath: '/Users/dev/package.json' },
          status: 'done',
          createdAt: 1770000000002,
        }}
        onOpenWorkspaceFile={onOpenWorkspaceFile}
  ...
  test('renders Claude MultiEdit blocks as editable file activity', async () => {
    ...
    render(
      <ToolBlockItem
        block={{
          ...
          toolName: 'MultiEdit',
          toolInput: {
            file_path: '/Users/dev/src/config.ts',
            edits: [
              {
    ...
    expect(screen.getByText('/Users/dev/src/config.ts')).toBeInTheDocument()
  ...
  test('opens streaming process text file links with line numbers in the workspace', () => {
    ...
    render(
      <ToolBlockItem
        block={{
          ...streamingTextBlock,
          content:
            '[workbenchPaneStack.test.tsx](/Users/dev/Wegent/renderer/src/components/layout/workbenchPaneStack.test.tsx:184)',
        }}
        onOpenWorkspaceFile={onOpenWorkspaceFile}
      />
    )
    ...
    expect(onOpenWorkspaceFile).toHaveBeenCalledWith(
      '/Users/dev/Wegent/renderer/src/components/layout/workbenchPaneStack.test.tsx',
...
test('switches application tool states live without translating commands or raw output', async () => {
  ...
  const view = render(
    <ToolBlockItem
      ...
      block={{
        ...
        type: 'tool',
        toolName: 'bash',
        toolInput: { command: 'printf fixture' },
        toolOutput: '原始命令输出',
        toolOutputTruncated: true,
        toolOutputOriginalBytes: 5000,
        status: 'running',
        createdAt: Date.now(),
      }}
      onLoadFullTranscript={async () => {}}
    />
  )
  try {
    expect(screen.getByText('Running printf fixture')).toBeVisible()
    expect(screen.getByText('Load full output')).toBeVisible()
    expect(screen.getByTestId('shell-tool-output')).toHaveTextContent('原始命令输出')
    await i18n.changeLanguage('zh-CN')
    await screen.findByText('正在运行 printf fixture')
    expect(screen.getByText('加载完整输出')).toBeVisible()
    expect(screen.getByTestId('shell-tool-output')).toHaveTextContent('原始命令输出')
  } finally {
    view.unmount()
