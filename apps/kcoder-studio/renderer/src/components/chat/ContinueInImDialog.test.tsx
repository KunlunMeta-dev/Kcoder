...
describe('ContinueInImDialog', () => {
  ...
  test('explains that legacy binding is unavailable without advertising a bind command', () => {
    render(
      <ContinueInImDialog
        open
        loading={false}
    ...
    expect(screen.getByTestId('continue-im-empty-guide')).toHaveTextContent(
      '旧版云端 IM 绑定在 KCoder Studio 中不可用。'
    )
    expect(screen.getByTestId('continue-im-empty-guide')).not.toHaveTextContent('/bind')
  })
