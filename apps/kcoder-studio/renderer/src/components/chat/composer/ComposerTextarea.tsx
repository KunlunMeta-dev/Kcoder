import { ClipboardList, Cpu, Package, Plug, Shrink, Target } from 'lucide-react'
...
export function ComposerTextarea({
  ...
  const suppressEscapeAutocompleteKeyUpRef = useRef(false)
  ...
  // The localized `@project-space:keyword` scope syntax drills straight into the cloud project
  ...
  // The direct cloud-space token inserted by the localized project-space row. Selecting it
  ...
  const actionSlashCommands = useMemo<SlashCommand[]>(() => {
    ...
    if (onSetGoal) {
      commands.push({
        ...
        testId: 'goal',
        acceptsArguments: true,
        onSelect: () => onSetGoal('standard'),
      })
      commands.push({
        id: 'goal-pro',
        title: '/goal-pro',
        description: t('workbench.slash_command_goal_pro_description'),
        searchAliases: ['goal pro', 'strict goal', 'verified goal'],
        Icon: Target,
        testId: 'goal-pro',
        acceptsArguments: true,
        onSelect: () => onSetGoal('strict'),
      })
    }

    if (executionModes) {
      for (const mode of ['orchestrate', 'moa', 'moa-plan'] as const) {
        if (mode === 'orchestrate' && !executionModes.canSelectOrchestrate) continue
        commands.push({
          id: mode,
          title: `/${mode}`,
          description: t(`workbench.slash_command_${mode.replace('-', '_')}_description`),
          searchAliases: [mode],
          Icon: ClipboardList,
          testId: mode,
          acceptsArguments: mode !== 'orchestrate',
          onSelect: () => executionModes.onSelect(mode),
        })
      }
    }
    ...
    if (onCompactContext) {
      commands.push({
        id: 'compact',
        title: '/compact',
        description: t('workbench.slash_command_compact_description'),
        searchAliases: ['compact', 'context compaction'],
    ...
    (snapshot?: ComposerEditorSnapshot) => {
      ...
      const nextTrigger = chooseNearestTrigger([
        findStandaloneTrigger(
          ...
          // the localized `@project-space keyword` scope so phrases containing a
          // project-space label keep filtering instead of closing the menu.
    ...
    (row: MentionMenuRow, explicitTrigger?: ComposerTextTrigger) => {
      ...
      if (row.kind === 'files-action') {
        void openNativeWorkspacePathPicker(workspaceTarget?.path)
          ...
          .catch(error => {
            console.warn('[KCoder Studio composer] native workspace picker failed', error)
          })
      }
      editor.focus()
      return true
  ...
  const selectHighlightedSlashCommand = useCallback(() => {
    ...
    if (currentSlashTrigger && !currentSlashTrigger.query.trim()) return false
  ...
  const handleKeyUp = (event: KeyboardEvent) => {
    ...
    if (event.key === 'Escape' && suppressEscapeAutocompleteKeyUpRef.current) {
      suppressEscapeAutocompleteKeyUpRef.current = false
      return
    }
    updateAutocompleteTrigger()
  }
  ...
  const handleEditorKeyDown = useCallback(
    (event: KeyboardEvent, snapshot: ComposerEditorSnapshot) => {
      ...
      if (event.key === 'Escape') {
        if (!showSkillMenuRef.current && !showSlashMenuRef.current) return false
        event.preventDefault()
        suppressEscapeAutocompleteKeyUpRef.current = true
        closeAutocompleteMenu()
        return true
      }

      if (event.key === 'Tab' && showSlashMenuRef.current) {
        const trigger = findStandaloneTrigger(
          snapshot.value,
          snapshot.selectionOffset,
          '/',
          'slash'
        )
        const command = filteredSlashCommands[highlightedIndexRef.current]
        if (!trigger || !command || command.enabled === false) return false
        event.preventDefault()
        const replacement = `${command.title}${command.acceptsArguments ? ' ' : ''}`
        const nextValue =
          snapshot.value.slice(0, trigger.start) +
          replacement +
          snapshot.value.slice(snapshot.selectionEnd)
        commitEditorValue(nextValue, trigger.start + replacement.length)
        closeAutocompleteMenu()
        return true
      }
      ...
      if (event.key === 'Enter') {
        ...
        const slashInput = parseComposerSlashInput(snapshot.value)
        if (slashInput) {
          event.preventDefault()
          event.stopPropagation()
          if (slashInput.name === '/') {
            setSlashCommandError(
              t('workbench.slash_command_required', 'Enter a slash command name')
            )
            return true
          }
          const command = actionSlashCommands.find(
            candidate => candidate.title.toLowerCase() === slashInput.name
          )
          if (!command || command.enabled === false) {
            setSlashCommandError(
              t('workbench.slash_command_unknown', `Unknown slash command: ${slashInput.name}`, {
          ...
          if (slashInput.args) {
            if (!command.acceptsArguments) {
              setSlashCommandError(
                t(
                  'workbench.slash_command_arguments_not_supported',
                  `${command.title} does not accept arguments`,
                  { command: command.title }
                )
              )
              return true
            }
            commitEditorValue(slashInput.args, slashInput.args.length)
            closeAutocompleteMenu()
            command.onSelect?.()
            return true
          }
  ...
  return (
    <div className="relative min-w-0 flex-1 w-full">
      {slashCommandError && (
        <div
          role="alert"
          data-testid="composer-slash-command-error"
          className="absolute bottom-[calc(100%+0.5rem)] left-0 z-popover rounded-lg border border-red-500/30 bg-background px-3 py-2 text-sm text-red-500 shadow-lg"
        >
          {slashCommandError}
        </div>
      )}
