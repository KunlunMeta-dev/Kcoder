...
export function AddContextMenu({
  ...
  const handleAttachFiles = useCallback(() => {
    // Close the menu before opening the system file picker so a slow Windows picker cannot leave the menu covering attachments.
    setOpen(false)
      ...
      {open && (
        ...
        >
          ...
          {onSetGoal && (
            ...
            >
              ...
              <span className="min-w-0 truncate">
                ...
                <span className="ml-2 text-text-muted">
                  {t(
                    'workbench.pursue_goal_description',
                    '设置 KCoder Studio 将持续努力实现的目标'
                  )}
                </span>
              </span>
            </button>
          ...
          {onSetGoal && (
            <button
              type="button"
              data-testid="set-goal-pro-button"
              onClick={() => {
                setOpen(false)
                onSetGoal('strict')
              }}
              className="flex min-h-9 w-full items-center gap-2.5 rounded-lg px-3 text-left text-base text-text-primary hover:bg-muted"
            >
              <Target className="h-4 w-4 shrink-0 text-text-secondary" />
              <span>
                {t('workbench.goal_pro_label')}
                <span className="ml-2 text-text-muted">{t('workbench.goal_pro_description')}</span>
              </span>
            </button>
          )}
        </div>
