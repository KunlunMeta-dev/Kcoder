...
export function CompactChatComposer({
  ...
  const attachmentSubmissionBlocked = uploadingFiles.size > 0 || attachmentErrors.size > 0
  const canSend =
    ...
    <div className="w-full">
      ...
      >
        <div
          ...
          className={[
            'relative flex min-h-[52px] min-w-0 flex-1 flex-col rounded-[26px] border border-border bg-background pl-4 shadow-[0_12px_40px_rgba(0,0,0,0.08)]',
            'z-chrome',
        ...
        >
          <ComposerTextarea
            ...
            className="scrollbar-none max-h-32 min-h-6 w-full min-w-0 flex-1 resize-none overflow-y-auto bg-transparent py-[14px] text-chat leading-5 text-text-primary outline-none placeholder:text-text-muted"
            skillMenuClassName={[
              'left-[-1rem]',
          ...
          />
          <div
            data-testid="compact-composer-toolbar"
            className="flex min-h-11 min-w-0 items-center gap-1 pb-1"
          >
            <button
              type="button"
              ...
              disabled={disabled}
              className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-text-secondary hover:bg-muted disabled:opacity-50"
      ...
      <GatewayPermissionSelector
        ...
        onChange={
          onSelectModelOption
            ? value => onSelectModelOption('kcoderPermissionMode', value)
            : undefined
        }
      />
      {contextSheetOpen && (
            ...
            {onSetGoal && (
              <>
                ...
                >
                  ...
                  <span>{t('workbench.goal_standard_label')}</span>
                </button>
                <button
                  type="button"
                  data-testid="mobile-set-goal-pro-button"
                  onClick={() => {
                    setContextSheetOpen(false)
                    onSetGoal('strict')
                  }}
                ...
                >
                  <Target className="h-6 w-6 text-text-secondary" />
                  {t('workbench.goal_pro_label')}
                </button>
              </>
            )}
          </div>
