...
describe('ComposerTextarea', () => {
  ...
  test('executes the app-server-backed TUI slash subset instead of submitting command text', async () => {
    const textareaRef = createRef<HTMLElement>()
    const onSetGoal = vi.fn()
    ...
    expect(await screen.findByTestId('slash-command-option-goal-pro')).toHaveTextContent(
      '/goal-pro'
    )
    fireEvent.click(screen.getByTestId('slash-command-option-goal-pro'))
    expect(onSetGoal).toHaveBeenCalledWith('strict')
    ...
    fireEvent.click(await screen.findByTestId('slash-command-option-compact'))
    expect(onCompactContext).toHaveBeenCalledOnce()
    expect(onSubmit).not.toHaveBeenCalled()
  })

  test('offers goal commands without advertising the legacy ultgoal mode', async () => {
    const textareaRef = createRef<HTMLElement>()
    function Harness() {
      const [value, setValue] = useState('')
      return (
    ...
    expect(await screen.findByTestId('slash-command-option-goal')).toBeInTheDocument()
    expect(screen.getByTestId('slash-command-option-goal-pro')).toBeInTheDocument()
    expect(screen.queryByTestId('slash-command-option-ultgoal')).not.toBeInTheDocument()
  })

  test('blocks empty and unknown slash input while preserving absolute paths', async () => {
    const textareaRef = createRef<HTMLElement>()
    const onSubmit = vi.fn()
    ...
    fireEvent.keyDown(editor, { key: 'Enter', code: 'Enter' })
    expect(onSubmit).not.toHaveBeenCalled()
    expect(await screen.findByTestId('composer-slash-command-error')).toHaveTextContent(
      'Enter a slash command name'
    )
    ...
    fireEvent.keyDown(editor, { key: 'Enter', code: 'Enter' })
    expect(onSubmit).not.toHaveBeenCalled()
    expect(await screen.findByTestId('composer-slash-command-error')).toHaveTextContent('/unknown')

    act(() => {
      editor.value = '/data/project'
    })
    fireEvent.keyDown(editor, { key: 'Enter', code: 'Enter' })
    expect(onSubmit).toHaveBeenLastCalledWith('/data/project', undefined)
  })

  test('Tab completes slash without executing and goal arguments enter goal draft', async () => {
    const textareaRef = createRef<HTMLElement>()
    const onSetGoal = vi.fn()
    const onSubmit = vi.fn()
    ...
    await waitFor(() => expect(editor.value).toBe('/goal '))
    expect(onSetGoal).not.toHaveBeenCalled()
    expect(onSubmit).not.toHaveBeenCalled()

    act(() => {
      editor.value = '/goal 修复并验证'
    })
    fireEvent.keyDown(editor, { key: 'Enter', code: 'Enter' })
    await waitFor(() => expect(editor.value).toBe('修复并验证'))
    expect(onSetGoal).toHaveBeenCalledWith('standard')
    expect(onSubmit).not.toHaveBeenCalled()
  })

  test('Escape keeps the slash menu closed through the matching keyup', async () => {
    const textareaRef = createRef<HTMLElement>()
    ...
    fireEvent.keyUp(editor, { key: 'Escape', code: 'Escape' })
