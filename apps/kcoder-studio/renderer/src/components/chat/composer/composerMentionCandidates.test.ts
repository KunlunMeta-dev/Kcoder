...
describe('KCoder skill runtime compatibility', () => {
  test.each(['DeepSeek-V4.1-Flash', 'gpt-5', 'claude-sonnet'])(
    'allows discovered KCoder skills with %s regardless of provider',
    name => {
      for (const provider of ['openai', 'claude', 'custom']) {
        for (const scope of ['system', 'user', 'repo']) {
          expect(
            canSelectSkillForModel(
              { name: 'skill', description: '', path: '/skills/SKILL.md', source: 'kcoder', scope },
              { name, type: 'runtime', provider }
            )
          ).toBe(true)
        }
      }
  ...
  test('does not treat display labels as runtime authority or bypass other hosts', () => {
    const skill = {
      name: 'skill',
      description: '',
    ...
    expect(
      canSelectSkillForModel(skill, { name: 'gpt-5', type: 'runtime', provider: 'openai' })
    ).toBe(false)
    expect(
      canSelectSkillForModel(
        { ...skill, source: 'codex' },
        { name: 'claude-sonnet', type: 'runtime', provider: 'claude' }
      )
    ).toBe(false)
  })
})
