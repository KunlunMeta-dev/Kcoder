...
import { isExecutorVersionCompatible } from '@/lib/device-capabilities'
...
export function ProjectWorkBar({
  ...
  }, [getDeviceForProject, normalizedProjectQuery, sortedProjects, t])
  const selectedLocalStandaloneDeviceId = useMemo(
    () =>
      standaloneDevices
        ...
        .find(
          device =>
            isOnlineDevice(device) && isExecutorVersionCompatible(device.executor_version)
        )?.device_id ?? null,
    [standaloneDevices]
  )
  ...
  const pendingWorkspaceSelection =
    ...
    !selectedDeviceWorkspace
  const projectWorkTriggerAriaLabel = (currentProject ? runtimeProjectLabel(currentProject, t) : undefined) ?? projectWorkTriggerLabel
          ...
          >
            {isMobile &&
              renderMobileSheetHeader(
                ...
                (currentProject ? runtimeProjectLabel(currentProject, t) : undefined) ??
                  emptyLabel ??
                  t('workbench.enter_project_work', '进入项目工作'),
                closeMenu
              )}
          ...
          >
            {currentProject ? (
              <>
                ...
                <span className="max-w-[12rem] truncate">{runtimeProjectLabel(currentProject, t)}</span>
              </>
            ) : (
              <>
