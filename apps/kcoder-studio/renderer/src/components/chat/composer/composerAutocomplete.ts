...
export interface ComposerSlashInput {
  name: string
  args: string
}
...
/**
 * Parses localized `@project-space:keyword` / `@project-space keyword` scope syntax. Returns
...
export function parseComposerSlashInput(value: string): ComposerSlashInput | null {
  const trimmed = value.trim()
  if (!trimmed.startsWith('/') || trimmed.includes('\n')) return null
  const separator = trimmed.search(/\s/)
  const name = separator < 0 ? trimmed : trimmed.slice(0, separator)
  // Absolute paths such as `/data/project` are not slash commands.
  if (name.slice(1).includes('/')) return null
  return {
