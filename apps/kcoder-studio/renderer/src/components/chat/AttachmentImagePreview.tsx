...
function attachmentPreviewIdentity(attachment: Attachment): string {
  return JSON.stringify([
    ...
    attachment.runtime_device_id ?? '',
    attachment.runtime_thread_id ?? '',
...
async function loadAttachmentImageUrl(
  ...
  if (localPreviewUrl) {
    ...
    if (localPath && isKCoderGatewayPage()) {
      const result = await invoke<{
        ...
        path: localPath,
        deviceId: attachment.runtime_device_id,
        threadId: attachment.runtime_thread_id,
        workspacePath: attachment.runtime_workspace_path,
      ...
      const objectUrl = URL.createObjectURL(
        new Blob([bytes], { type: result.mimeType || attachment.mime_type })
      )
      return { url: objectUrl, objectUrl }
    }
    if (localPath && isTauriRuntime()) {
      const exists = await invoke<boolean>('local_path_exists', { path: localPath }).catch(
        () => null
      )
      if (exists === false) {
...
async function downloadAttachmentImage(attachment: Attachment, imageUrl: string) {
  ...
  if (sourcePath && isTauriRuntime() && !isKCoderGatewayPage()) {
...
export function AttachmentImagePreview({
  ...
  const failedPreviewIdentityRef = useRef<string | null>(null)
  ...
  const currentLightboxAttachmentIdentity = attachmentPreviewIdentity(currentLightboxAttachment)
  ...
  useEffect(() => {
    failedPreviewIdentityRef.current = null
  ...
  useEffect(() => {
    if (!shouldLoadPreview) return undefined
    if (failedPreviewIdentityRef.current === previewIdentity) return undefined
    ...
    async function loadPreview() {
      ...
      try {
        const loaded = await loadAttachmentImageUrl(attachmentRef.current)
        ...
        if (isMounted) {
          failedPreviewIdentityRef.current = previewIdentity
          setHasError(true)
        }
      }
    }
  ...
  useEffect(() => {
    ...
    attachment,
    currentLightboxAttachmentIdentity,
    disableLightbox,
    gallery,
    isLightboxOpen,
    lightboxIndex,
