...
describe('ScrollableMessageArea', () => {
  ...
  test.each(['unchanged', 'insert-before', 'replace'])(
    'keeps the exact reading anchor when paused content remeasures: %s',
    change => {
      const resizeCallbacks: ResizeObserverCallback[] = []
      const originalResizeObserver = globalThis.ResizeObserver
      ...
      try {
        ...
        if (change !== 'unchanged') {
          ...
          other.dataset.scrollAnchor = ''
          other.textContent = 'An unrelated newly mounted paragraph'
          mockScrollRelativeRect(other, scroller, 1100, 40)
          if (change === 'insert-before') anchor.before(other)
          else anchor.replaceWith(other)
        }
        act(() => {
          resizeCallbacks.forEach(callback => callback([], {} as ResizeObserver))
        })
        ...
        expect(scroller.scrollTo).not.toHaveBeenCalled()
        expect(scroller.scrollTop).toBe(change === 'replace' ? 300 : 540)
        if (change !== 'replace') expect(anchor.getBoundingClientRect().top).toBe(150)
      } finally {
        vi.stubGlobal('ResizeObserver', originalResizeObserver)
      }
    }
