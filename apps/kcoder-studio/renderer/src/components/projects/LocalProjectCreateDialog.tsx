...
function LocalProjectCreateDialogContent({
  ...
  const addFolders = async () => {
    if (picking) return
    if (!canUseNativeProjectPickerForDevice(device, shouldUseNativeProjectDirectoryPicker())) {
    ...
    setError(null)
    try {
      const selected = await pickProjectDirectoriesForDevice(device, roots[0])
    ...
    <div className="fixed inset-0 z-modal flex items-center justify-center bg-black/35 px-4">
      ...
      >
        ...
        <div className="mt-2 overflow-hidden rounded-xl border border-border bg-background">
          ...
          <button
            type="button"
            data-testid="local-project-create-enter-path"
            disabled={submitting || picking}
            onClick={() => setShowFolderPicker(true)}
            className="h-9 w-full border-t border-border px-3 text-left text-sm text-text-secondary hover:bg-muted disabled:opacity-50"
          >
            {t('workbench.enter_folder_path', '输入文件夹路径…')}
          </button>
          {showFolderPicker && (
            <div
