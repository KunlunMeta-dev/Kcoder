...
import {
  canPickDesktopProjectDirectories,
  canUseNativeProjectPickerForDevice,
  pickProjectDirectoriesForDevice,
} from '@/kcoder/desktopDirectoryPicker'
import {
...
function isLocalDevice(device: DeviceInfo): boolean {
  return isKCoderLocalGatewayDevice(device) || (!isCloudDevice(device) && !isRemoteDevice(device))
}
...
export function StandaloneFolderProjectDialog({
  ...
  const shouldUseNativeLocalPicker =
    ...
    canUseNativeProjectPickerForDevice(activeDevice, preferNativeLocalPicker) &&
  ...
  useEffect(() => {
    ...
    const openPickerTimer = window.setTimeout(() => {
      ...
      void (async () => {
        try {
          setNativePickerError(null)
          const selectedPaths = await pickProjectDirectoriesForDevice(activeDevice!)
          const selectedPath = selectedPaths[0]
          if (!selectedPath) {
            if (canPickDesktopProjectDirectories(activeDevice!)) setNativePickerFallback(true)
            else closeDialog()
            return
          }
          ...
          console.error('[KCoder Studio project] native picker failed', error)
