...
export function DeviceFolderPicker({
  ...
  const pathInteractionRevision = useRef(0)
  const initialResolutionKey = `${device.device_id}\0${initialPath ?? ''}\0${pickerDisabled}`
  ...
  useLayoutEffect(() => {
    initialResolution.current = {
      key: initialResolutionKey,
      interactionRevision: pathInteractionRevision.current,
    }
  ...
  useEffect(() => {
    ...
    let cancelled = false
    // Interaction may occur after the first frame but before the effect starts. Use the
    // revision captured on the first render for this device/initialPath so a late home
    // query cannot overwrite an absolute path just entered by the user.
    const startingInteractionRevision = initialResolution.current.interactionRevision
    ...
    async function resolveInitialPath() {
      try {
        ...
        if (!cancelled && pathInteractionRevision.current === startingInteractionRevision) {
  ...
  const browsePath = (path: string) => {
    pathInteractionRevision.current += 1
  ...
  const confirmPathInput = (inputValue = pathInput) => {
    const normalized = normalizePath(inputValue) || '/'
    if (normalized === currentPath) {
      browsePath(normalized)
      return
    }

    const { parentPath, query } = getPathSearchParts(inputValue)
    ...
    <div className={dark ? 'space-y-3' : 'rounded-lg border border-[#d8d8d8] bg-white'}>
      ...
      >
        ...
        >
          ...
          <input
            ...
            onFocus={() => {
              // Browser automation and real users may focus the input before the home query
              // returns. Treat the path as user-owned from focus so query results cannot insert a prefix during fill/type.
              pathInteractionRevision.current += 1
            }}
            onChange={event => {
              pathInteractionRevision.current += 1
            ...
            onBlur={event => confirmPathInput(event.currentTarget.value)}
            onKeyDown={event => {
              if (isImeEnterEvent(event)) return
              if (event.key === 'Enter') {
                event.preventDefault()
                confirmPathInput(event.currentTarget.value)
              }
            }}
            className={
              dark
      ...
      {mode === 'create' && (
        ...
        >
          <FolderPlus
            className={
              dark ? 'h-4 w-4 shrink-0 text-text-secondary' : 'h-4 w-4 shrink-0 text-[#606368]'
            }
          />
          <input
      ...
      {error && (
        <p
          ...
          className={
            dark
              ? 'border-b border-border px-3 py-2 text-xs text-red-500'
      ...
      >
        {!dark && !pickerDisabled && currentPath && currentPath !== '/' && (
          <button
            ...
            className={
              dark
                ? 'flex h-10 w-full items-center gap-3 rounded-lg px-2 text-left text-base text-text-secondary hover:bg-muted'
        ...
        {loadingDirectories && (
          <p
            className={
              dark ? 'px-2 py-3 text-sm text-text-secondary' : 'px-2 py-3 text-sm text-[#8a8f98]'
            }
          >
        ...
        {!loadingDirectories &&
          filteredDirectories.map(directory => {
            ...
            return (
              ...
              >
                <Folder
                  className={dark ? 'h-4 w-4 shrink-0 text-text-secondary' : 'h-4 w-4 shrink-0'}
                />
        ...
        {!pickerDisabled && !loadingDirectories && !error && filteredDirectories.length === 0 && (
          <p
            className={
              dark
                ? 'px-2 py-8 text-center text-sm text-text-secondary'
      ...
      >
        <button
          ...
          disabled={submitting}
          onMouseDown={event => {
            // Prevent path input blur and directory refresh on button press, which could move the button before release.
            event.preventDefault()
          }}
          onClick={onCancel}
          className={
        ...
        <button
          ...
          onMouseDown={event => {
            // Keep button geometry stable so one real mouse click lands press and release on the same button.
            event.preventDefault()
          }}
