...
describe('App center route', () => {
  ...
  test('loads Agent from the connected cloud address', async () => {
    ...
    expect(await screen.findByTitle('智能体')).toHaveAttribute('src', 'https://app.example.com')
  })
  ...
  test('renders copyable debug instance rows', async () => {
    ...
    vi.stubEnv('VITE_KCODER_STUDIO_DEV_TITLE', 'Runtime task')
    vi.stubEnv('VITE_KCODER_STUDIO_DEV_PORT', '1420')
    vi.stubEnv('VITE_KCODER_STUDIO_DEV_WORKTREE', '/Users/me/Wegent')
    vi.stubEnv('VITE_KCODER_STUDIO_PARENT_TITLE', 'Parent task')

    render(<App />)

    await waitForStartupScreenToClose()
    expect(screen.getByTestId('kcoder-dev-instance-trigger')).toHaveClass(
      'h-8',
      'w-8',
      'rounded-full'
    )
    fireEvent.click(screen.getByTestId('kcoder-dev-instance-trigger'))
    expect(screen.getByTestId('kcoder-dev-instance-badge')).toHaveTextContent('Runtime task')
    ...
    fireEvent.click(screen.getByTestId('collapse-kcoder-dev-instance-button'))
    expect(screen.getByTestId('kcoder-dev-instance-trigger')).toHaveClass(
      'h-8',
      'w-8',
      'rounded-full'
    )
    expect(screen.getByTestId('kcoder-dev-instance-badge')).toHaveTextContent('Port')

    const badge = screen.getByTestId('kcoder-dev-instance-badge')
    ...
    const trigger = screen.getByTestId('kcoder-dev-instance-trigger')
    ...
    fireEvent.click(screen.getByTestId('kcoder-dev-instance-trigger'))
    expect(screen.getByTestId('kcoder-dev-instance-trigger')).not.toHaveClass('rounded-full')
  })
  ...
  test('uses the fixed titlebar switcher and collapses the app center sidebar', async () => {
    ...
    const weworkTab = screen.getByTestId('chrome-tab-studio')
    expect(screen.queryByTestId('chrome-tab-todo')).not.toBeInTheDocument()
    expect(screen.queryByTestId('chrome-tab-apps')).not.toBeInTheDocument()
