...
import { isNativeTauriHost, isTauriRuntime } from '@/lib/runtime-environment'
...
const LOCAL_MODEL_SEND_CIRCUIT_BREAKER_ERROR = 'KCODER_STUDIO_E2E_LOCAL_MODEL_SEND_CIRCUIT_OPEN'
...
declare global {
  interface Window {
    __KCODER_STUDIO_E2E__?: WeworkAutomationBridge
  }
}

export function isWeworkAutomationEnabled(): boolean {
  return (
    Boolean(gatewayVerificationConfig()) ||
    import.meta.env.MODE === 'e2e' ||
    import.meta.env.VITE_KCODER_STUDIO_E2E === 'true'
  )
}

export function shouldUseNativeProjectDirectoryPicker(): boolean {
  return (
    (isNativeTauriHost() || typeof desktopHost()?.pickWorkspacePaths === 'function') &&
    (!isWeworkAutomationEnabled() ||
      import.meta.env.VITE_KCODER_STUDIO_E2E_NATIVE_DIRECTORY_PICKER === 'true')
  )
}

function desktopControlUrl(): string | null {
  const verification = gatewayVerificationConfig()
  if (verification) return verification.controlUrl
  const value = import.meta.env.VITE_KCODER_STUDIO_DESKTOP_E2E_CONTROL_URL?.trim()
  return value ? value.replace(/\/+$/, '') : null
}

function desktopControlHeaders(): HeadersInit | undefined {
  const verification = gatewayVerificationConfig()
  if (verification) return { Authorization: `Bearer ${verification.token}` }
  const token = import.meta.env.VITE_KCODER_STUDIO_DESKTOP_E2E_CONTROL_TOKEN?.trim()
...
function seedDesktopE2ECloudConnection() {
  const backendUrl = import.meta.env.VITE_KCODER_STUDIO_E2E_CLOUD_BACKEND_URL?.trim()
  if (!backendUrl) return
  const modelServerUrl =
    import.meta.env.VITE_KCODER_STUDIO_E2E_MODEL_SERVER_URL?.trim() || backendUrl
  const localModelsCatalogReady =
    import.meta.env.VITE_KCODER_STUDIO_E2E_LOCAL_MODELS_CATALOG_READY === 'true'
  const token =
    import.meta.env.VITE_KCODER_STUDIO_E2E_CLOUD_TOKEN?.trim() || 'studio-desktop-e2e-cloud-token'
  ...
  const localModels =
    import.meta.env.VITE_KCODER_STUDIO_E2E_SEED_LOCAL_MODELS === 'true'
      ? [
          {
...
export function installStudioAutomationBridge() {
  if (!isWeworkAutomationEnabled() || typeof window === 'undefined') {
    return
  }

  if (!gatewayVerificationConfig()) seedDesktopE2ECloudConnection()
  window.__KCODER_STUDIO_E2E__ = createBridge()
  installDesktopControlClient()
}

function findDesktopControlElements(selector: string): HTMLElement[] {
  return queryDesktopControls(selector)
}
...
async function executeDesktopControlCommand(command: DesktopControlCommand): Promise<string> {
  if (
    gatewayVerificationConfig() &&
    ['capture', 'closeMainWindowToTray', 'focusMainWindow', 'performanceSnapshot'].includes(
      command.action
    )
  ) {
    throw new Error('Native automation commands are disabled in Gateway verification')
  }
  switch (command.action) {
    case 'getActiveToolsCatalog':
      return JSON.stringify(await getActiveToolsCatalog())
    case 'capture':
    ...
    case 'getAttribute': {
      ...
      if (!attribute || !/^[a-zA-Z][a-zA-Z0-9_-]*$/.test(attribute)) {
...
async function runDesktopControlClient(url: string): Promise<void> {
  const verification = gatewayVerificationConfig()
  const nativeIsolation = verification ? await verification.nativeIsolation : undefined
  if (verification && (nativeIsolation?.length !== 2 || nativeIsolation.some(denied => !denied))) {
    throw new Error('Gateway verification requires native app and plugin IPC to be denied')
  }
  ...
  const readyResponse = await fetch(`${url}/ready`, {
    ...
    body: JSON.stringify({
      ...
      ...(verification
        ? {
            nativeAppCommandDenied: nativeIsolation?.[0],
            nativePluginCommandDenied: nativeIsolation?.[1],
          }
        : {}),
    }),
  })
  ...
  while (true) {
    try {
      ...
      console.error('[KCoder Studio] Desktop E2E control client failed:', error)
      await new Promise(resolve => window.setTimeout(resolve, DESKTOP_CONTROL_RETRY_DELAY_MS))
      commandRequest = pollForCommand()
    }
  }
