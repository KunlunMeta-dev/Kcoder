...
export const LOCAL_WORKSPACE_OPEN_REQUESTED_EVENT = 'kcoder-open-local-workspace-requested'
...
export function installLocalWorkspaceOpenListener(
  ...
  const drainRequests = () => {
    ...
    drainPromise = takePendingLocalWorkspaceOpenRequests()
      ...
      .catch(error => {
        ...
        console.error('[KCoder Studio] Failed to open local workspace from CLI:', error)
        onError?.(message)
      })
      .finally(() => {
        drainPromise = null
  ...
  return listen(LOCAL_WORKSPACE_OPEN_REQUESTED_EVENT, () => {
    ...
    console.error('[KCoder Studio] Failed to install local workspace open listener', error)
    onError?.(error instanceof Error ? error.message : 'Failed to install KCoder CLI listener')
    return () => {}
  })
}
