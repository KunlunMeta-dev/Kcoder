...
export const KCODER_STUDIO_TRAY_OPEN_SETTINGS_EVENT = 'kcoder-tray-open-settings'
export const KCODER_STUDIO_TRAY_OPEN_TASK_EVENT = 'kcoder-tray-open-task'
...
export function syncTrayMenuState(
  ...
  void invoke(SET_TRAY_MENU_STATE_COMMAND, {
    ...
    console.error('[KCoder Studio] Failed to sync tray menu state', error)
  })
}
...
export function installTraySettingsNavigation() {
  ...
  if (!traySettingsNavigationListener) {
    traySettingsNavigationListener = listen(KCODER_STUDIO_TRAY_OPEN_SETTINGS_EVENT, () => {
      navigateTo('/settings')
    }).catch(error => {
      traySettingsNavigationListener = null
      console.error('[KCoder Studio] Failed to install tray settings navigation listener', error)
      return () => {}
    })
  }

  if (!trayTaskNavigationListener) {
    trayTaskNavigationListener = listen<{ id: string }>(KCODER_STUDIO_TRAY_OPEN_TASK_EVENT, event => {
      ...
      console.error('[KCoder Studio] Failed to install tray task navigation listener', error)
      return () => {}
    })
  }
