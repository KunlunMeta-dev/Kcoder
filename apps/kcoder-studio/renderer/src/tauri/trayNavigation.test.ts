...
describe('trayNavigation', () => {
  ...
  test('opens settings when the tray settings event is received', async () => {
    ...
    const { installTraySettingsNavigation, KCODER_STUDIO_TRAY_OPEN_SETTINGS_EVENT } =
    ...
    expect(listenMock).toHaveBeenCalledWith(KCODER_STUDIO_TRAY_OPEN_SETTINGS_EVENT, expect.any(Function))

    handlers.get(KCODER_STUDIO_TRAY_OPEN_SETTINGS_EVENT)?.()
  ...
  test('opens a runtime task when the tray task event is received', async () => {
    ...
    const { installTraySettingsNavigation, KCODER_STUDIO_TRAY_OPEN_TASK_EVENT } =
      await import('./trayNavigation')
    const { createTrayTaskMenuId } = await import('./trayTaskMenuId')

    installTraySettingsNavigation()

    handlers.get(KCODER_STUDIO_TRAY_OPEN_TASK_EVENT)?.({
      payload: {
