...
export const APPSHOT_CAPTURED_EVENT = 'kcoder-appshot-captured'
...
export async function subscribeToAppshots(
  ...
  const acknowledge = async (id: string) => {
    try {
      ...
      console.error('[KCoder Studio] Failed to acknowledge Appshot:', error)
    }
  }
  ...
  const unlistenCaptured = await listen<AppshotPayload>(APPSHOT_CAPTURED_EVENT, event => {
    void deliver(event.payload).catch(error => {
      console.error('[KCoder Studio] Failed to deliver Appshot:', error)
    })
  })
