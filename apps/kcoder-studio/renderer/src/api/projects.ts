...
// Protocol handshake value for the cloud service (matches the socket auth `client_origin`); not a product brand.
const KCODER_STUDIO_CLIENT_ORIGIN = 'wework'

function withClientOrigin(path: string): string {
  const separator = path.includes('?') ? '&' : '?'
  return `${path}${separator}client_origin=${KCODER_STUDIO_CLIENT_ORIGIN}`
}
