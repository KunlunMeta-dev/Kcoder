...
import { isWorkspaceScanCancelled } from '@/kcoder/workspaceScanError'
import {
...
import { KCODER_STUDIO_MIN_EXECUTOR_VERSION } from '@/lib/device-capabilities'
...
} from '@/kcoder/gatewayServiceBridge'
import { runtimeMethodForCurrentHost, runtimeNameForCurrentHost } from '@/kcoder/legacyRuntimeAbi'
...
export const LOCAL_WORKBENCH_TEAM = {
  id: 0,
  name: 'local-wework',
  displayName: 'Local KCoder',
  is_active: true,
  default_for_modes: ['studio'],
...
function localCodexModel(model: CodexOfficialModel, codexAuthConfigured: boolean): UnifiedModel {
  ...
  return {
    ...
    config: {
      ...
      codexProviderCurrent: model.providerCurrent,
      codexModelDefault: model.isDefault,
      ...(model.configuration ? { modelConfiguration: model.configuration, activeModelConfiguration: model.activeConfiguration, modelExecutionScope: model.executionScope, contextWindowTokens: model.configuration.contextWindowTokens, maxOutputTokens: model.configuration.maxOutputTokens } : {}),
      ...(model.providerError ? { unavailableReason: model.providerError } : {}),
      ui: {
        family: modelFamily,
    ...
    isActive: model.providerAvailable !== false,
...
function localRuntimeModels(
  ...
  codexAuthConfigured = false,
  includeLocalModelInterfaces = true
): UnifiedModel[] {
  const officialModels =
    codexOfficialError || codexOfficialModels.length === 0
      ? [unavailableCodexModel(codexOfficialError || 'The runtime returned no available models')]
      : codexOfficialModels.map(model => localCodexModel(model, codexAuthConfigured))

  return [
    ...officialModels,
    ...(includeLocalModelInterfaces
      ? listLocalModelConfigs()
...
interface LocalAppServicesDeps {
  ...
  request?: <T>(method: string, params?: Record<string, unknown>, options?: { signal?: AbortSignal }) => Promise<T>
...
function localDeviceFromStatus(status: LocalExecutorStatus): DeviceInfo {
  ...
  const device = {
    ...
    slot_max: 5,
    executor_version:
      (status as LocalExecutorStatus & { version?: string }).version ??
...
function normalizeRuntimeTaskSummaries(
  ...
  if (tasks.length > 0 && normalizedTasks.length === 0) {
    console.warn('[KCoder Studio] Dropped runtime tasks without taskId', {
...
function builtInCodexModelId(modelId?: string): string | undefined {
  if (modelId === CODEX_OFFICIAL_UNAVAILABLE_MODEL_NAME) {
    throw new Error('The runtime model list is unavailable')
  }
  return codexOfficialModelIdFromModelName(modelId) ?? undefined
}
...
function localRuntimeModelConfig(
  ...
  const selectedModelId = builtInCodexModelId(modelName)
  return {
    model: 'openai',
    // Omitting a selection preserves the target runtime's current model.
...
function applyRuntimeModelOptions(
  ...
  if (includeLocalProxy) modelConfig = applyLocalProxyConfig(modelConfig)
...
function localRuntimeAttachments(
  ...
  attachments.forEach(attachment => {
    ...
    runtimeAttachments.push({
      ...
      ...(attachment.runtime_device_id ? { runtime_device_id: attachment.runtime_device_id } : {}),
      ...(attachment.runtime_thread_id ? { runtime_thread_id: attachment.runtime_thread_id } : {}),
      ...(attachment.runtime_workspace_path
        ? { runtime_workspace_path: attachment.runtime_workspace_path }
        : {}),
...
function buildLocalRuntimeExecutionRequest(
  ...
  return {
    ...
    model_config: {
      ...modelConfig,
      ...(input.modelOptions?.kcoderPermissionMode &&
      input.modelOptions.kcoderPermissionMode !== 'default'
        ? { permission_mode: input.modelOptions.kcoderPermissionMode }
        : {}),
    },
...
async function createLocalRuntimeTaskPayload(
  ...
  const runtimeWorkspace = await prepareLocalRuntimeWorkspace(
...
function createLocalRuntimeSendPayload(
  ...
  if (!taskId) {
    console.warn('[KCoder Studio] Local runtime send missing taskId', {
      deviceId: localDeviceId,
      workspacePath,
...
function normalizeRuntimeWorkDeviceId(
  ...
  const normalizeWorkspace = (workspace: RuntimeDeviceWorkspace): RuntimeDeviceWorkspace => {
    ...
    if (!Array.isArray(workspaceRecord.tasks)) {
      console.warn('[KCoder Studio] Runtime workspace missing tasks', {
        deviceId: localDeviceId,
        workspacePath,
...
function kcoderDeviceIdFromData(
  ...
  const executionRequest = recordValue(value.executionRequest)
  const projectKey =
    stringValue(value.runtimeProjectKey) ?? stringValue(executionRequest.runtime_project_key)
  return (
    stringValue(value.deviceId) ??
    stringValue(value.device_id) ??
    stringValue(address.deviceId) ??
    stringValue(address.device_id) ??
    stringValue(execution.deviceId) ??
    stringValue(execution.device_id) ??
    (projectKey?.startsWith('kcoder:') ? projectKey.slice('kcoder:'.length) : null) ??
    fallback
  )
}

function adaptKCoderRuntimeWorkListResponse(
  response: unknown,
  defaultDeviceId: string
): RuntimeWorkListResponse {
  const rawResponse = recordValue(response)
  const rawWorkspaces = Array.isArray(rawResponse.workspaces) ? rawResponse.workspaces : []
  const targetsByProjectKey = new Map<string, { deviceId: string; name: string | null }>()
  const targetsByWorkspacePath = new Map<string, { deviceId: string; name: string | null }>()
  rawWorkspaces.forEach(value => {
    const workspace = recordValue(value)
    const deviceId = stringValue(workspace.deviceId) ?? stringValue(workspace.device_id)
    if (!deviceId) return
    const target = {
      deviceId,
      name: stringValue(workspace.deviceName) ?? stringValue(workspace.label),
    }
    const projectKey = stringValue(workspace.projectKey) ?? stringValue(workspace.project_key)
    const workspacePath =
      stringValue(workspace.workspacePath) ?? stringValue(workspace.workspace_path)
    if (projectKey) targetsByProjectKey.set(projectKey, target)
    if (workspacePath) targetsByWorkspacePath.set(workspacePath, target)
  })
  const runtimeWork = adaptRuntimeWorkListResponse(response, defaultDeviceId)
  return {
    ...runtimeWork,
    projects: runtimeWork.projects.map(project => {
      const target = targetsByProjectKey.get(project.project.key)
      const deviceWorkspaces = project.deviceWorkspaces.map(workspace => ({
        ...workspace,
        ...(target
          ? {
              deviceId: target.deviceId,
              deviceName: target.name ?? target.deviceId,
      ...
      return {
        ...
        project: {
          ...project.project,
          // The selected gateway target is passed as defaultDeviceId and can change across a
          // reload. Upstream derives legacy project ids from that value, which made the stored
          // lastProjectId point at a different project after switching servers. Namespace ids to
          // the gateway itself so every target keeps one stable UI identity.
          id: stableLocalId(`kcoder-gateway\0${project.project.key}`),
          stateDeviceId: deviceWorkspaces[0]?.deviceId ?? project.project.stateDeviceId,
        },
        deviceWorkspaces,
      }
    }),
    chats: runtimeWork.chats.map(workspace => {
      const target = targetsByWorkspacePath.get(workspace.workspacePath)
      return {
        ...workspace,
        ...(target
          ? {
...
function adaptRuntimeWorkListResponse(
  ...
  for (const rawWorkspace of workspaces) {
    ...
    if (!Array.isArray(workspace.tasks)) {
      console.warn('[KCoder Studio] Local runtime workspace missing tasks', {
        deviceId: localDeviceId,
        workspacePath,
    ...
    const deviceWorkspace: RuntimeDeviceWorkspace = {
      ...(typeof workspace.threadsComplete === 'boolean'
        ? { threadsComplete: workspace.threadsComplete }
        : {}),
      ...(Number.isSafeInteger(workspace.threadListIssueCount) &&
      Number(workspace.threadListIssueCount) >= 0
        ? { threadListIssueCount: Number(workspace.threadListIssueCount) }
        : {}),
      ...
      available: workspaceDeviceId === localDeviceId && workspace.available !== false,
      error: stringValue(workspace.error),
      workspacePath,
      workspaceKind,
      worktreeId,
      label,
      ...(workspace.labelKey === 'currentComputer' ? { labelKey: 'currentComputer' as const } : {}),
      ...(workspace.deviceNameKey === 'currentComputer' ? { deviceNameKey: 'currentComputer' as const } : {}),
      workspaceSource,
      remoteHostId,
      mapped: true,
      tasks,
    ...
    const projectWork: RuntimeWorkListResponse['projects'][number] = {
      project: {
        ...
        name: label,
        ...(workspace.labelKey === 'currentComputer' ? { nameKey: 'currentComputer' as const } : {}),
        kind: projectKind,
        ...
        appearance: (workspace.projectAppearance ?? workspace.project_appearance ?? null) as
          RuntimeWorkListResponse['projects'][number]['project']['appearance'] | null,
      },
...
export function createRuntimeWorkApiFromIpc(
  ...
  const includeLocalProxy = options.includeLocalProxy !== false
  ...
  const requestWithLocalDevice = async <TResponse, TRequest extends object>(
    ...
    try {
      if (debugTranscript) {
        console.debug(`[KCoder Studio] ${transportLabel} runtime IPC transcript request`, {
      ...
      if (debugTranscript) {
        console.debug(`[KCoder Studio] ${transportLabel} runtime IPC transcript response`, {
          address: runtimeAddressDebug(normalizedData),
      ...
      if (method === 'runtime.tasks.transcript') {
        console.error(`[KCoder Studio] ${transportLabel} runtime IPC transcript failed`, {
          address: runtimeAddressDebug(normalizedData),
          elapsedMs: Math.round(nowMs() - startedAt),
          error,
        })
  ...
  return {
    async listRuntimeWork(
      ...
      const startedAt = nowMs()
      try {
        return await readWorkspaceList(
          params => request('runtime.tasks.list', params, localDeviceId),
          response => adaptListResponse(response, localDeviceId),
          isKCoderGatewayPage() && transportLabel === 'Local' ? onProgress : undefined
        )
      } catch (error) {
        if (!isWorkspaceScanCancelled(error)) {
          console.error(`[KCoder Studio] ${transportLabel} runtime IPC list failed`, {
            elapsedMs: Math.round(nowMs() - startedAt),
            error,
          })
        }
    ...
    async sendRuntimeMessage(data: RuntimeSendRequest): Promise<RuntimeSendResponse> {
      ...
      if (!payload.executionRequest) {
        console.warn('[KCoder Studio] Local runtime send payload missing executionRequest', {
      ...
      console.debug('[KCoder Studio] Local runtime send payload', {
        taskId: payload.taskId,
        address: runtimeAddressDebug(payload),
        payloadKeys: Object.keys(payload).sort(),
      })
    ...
    async rollbackRuntimeTask(data: RuntimeRollbackRequest): Promise<RuntimeSendResponse> {
      ...
      if (!payload.executionRequest) {
        console.warn('[KCoder Studio] Local runtime rollback payload missing executionRequest', {
          taskId: payload.taskId,
          address: runtimeAddressDebug(payload),
          payloadKeys: Object.keys(payload).sort(),
        })
      ...
      console.debug('[KCoder Studio] Local runtime rollback payload', {
        taskId: payload.taskId,
        address: runtimeAddressDebug(payload),
        payloadKeys: Object.keys(payload).sort(),
      })
    ...
    async compactRuntimeTask(data: RuntimeCompactRequest): Promise<RuntimeSendResponse> {
      ...
      const normalizedAddress = normalizeDeviceRecord({ address: data.address }, localDeviceId)
    ...
    async steerRuntimeSubagent(
      data: RuntimeSubagentSteerRequest
    ...
    async readSubagentArtifact(
      data: RuntimeSubagentArtifactRequest
    ...
    async interruptAndSendRuntimeMessage(
      ...
      if (!payload.executionRequest) {
        console.warn('[KCoder Studio] Local runtime interrupt payload missing executionRequest', {
          taskId: payload.taskId,
          address: runtimeAddressDebug(payload),
          payloadKeys: Object.keys(payload).sort(),
        })
      ...
      console.debug('[KCoder Studio] Local runtime interrupt payload', {
        taskId: payload.taskId,
        address: runtimeAddressDebug(payload),
        payloadKeys: Object.keys(payload).sort(),
      })
    ...
    getRuntimeSessionModes(data: RuntimeSessionModesRequest): Promise<RuntimeSessionModesResponse> {
      return requestWithLocalDevice('runtime.session.modes', data)
    },
    ...
    previewWorktreeArchive(
      ...
      return requestWithLocalDevice('runtime.worktrees.archive.preview', data)
    },
    archiveWorktree(data: RuntimeWorktreeArchiveRequest): Promise<RuntimeWorktreeMutationResponse> {
      return requestWithLocalDevice('runtime.worktrees.archive', data)
    },
    restoreWorktree(
    ...
    forgetWorktree(data: RuntimeWorktreeForgetRequest): Promise<RuntimeWorktreeMutationResponse> {
      return requestWithLocalDevice('runtime.worktrees.forget', data)
    },
    ...
    shortenWaitRuntimeTask(data: RuntimeTaskAddress): Promise<RuntimeTaskShortenWaitResponse> {
      return requestWithLocalDevice('runtime.tasks.shorten_wait', data)
    },
    async createRuntimeTask(data: RuntimeTaskCreateRequest): Promise<RuntimeTaskCreateResponse> {
      ...
      console.info('[KCoder Studio] Local runtime execution identity', {
    ...
    disposeTemporaryRuntimeTask(data: RuntimeTaskAddress): Promise<unknown> {
      return requestWithLocalDevice('runtime.tasks.dispose', data)
    },
  }
}
...
function debugLocalRuntimeCreatePayload(
  ...
  const executionRequest = recordValue(payload.executionRequest)
  console.debug('[KCoder Studio] Local runtime create payload', {
...
export function gatewayDeviceStatus(status: GatewayServer['status']): DeviceInfo['status'] {
...
export function createLocalAppServices(deps: LocalAppServicesDeps = {}): WorkbenchServices {
  ...
  const kcoderGateway = isKCoderGatewayPage()
  if (kcoderGateway && globalThis.localStorage?.getItem(LOCAL_MODEL_SETTINGS_STORAGE_KEY)) {
    clearLocalModelConfigs()
  }
  if (kcoderGateway && globalThis.localStorage?.getItem(LOCAL_PROXY_SETTINGS_STORAGE_KEY)) {
    clearLocalProxyConfig()
  }
  const gatewayServers = () => fetchGatewayServersWithHealth()
  ...
  const ensureStatus = async () => {
    if (!ensurePromise) {
      ensurePromise = ensure()
        .then(async status => {
          lastStatus = status
          if (kcoderGateway) return status
          ...
          if (shouldReconcile) {
            ...
            try {
              await request(runtimeMethodForCurrentHost('catalogCustomWrite'), {
              ...
              const restart = await request<{
                restarted?: boolean
              }>(runtimeMethodForCurrentHost('appServerRestart'), { ifIdle: true })
  ...
  const deviceApi: WorkbenchServices['deviceApi'] = {
    async listDevices(options) {
      try {
        ...
        if (!kcoderGateway) return [localDeviceFromStatus(status)]
        const selectedDeviceId = localDeviceIdFromStatus(status)
        const version =
          (status as LocalExecutorStatus & { version?: string }).version ??
          KCODER_STUDIO_MIN_EXECUTOR_VERSION
        const selectedOnly = options?.health === 'selected'
        const servers = await (selectedOnly ? fetchGatewayServers() : gatewayServers())
        return servers.map(server => {
          const deviceStatus = selectedOnly
            ? server.id === selectedDeviceId && status.ready && status.running
              ? 'online'
              : 'offline'
            : gatewayDeviceStatus(server.status)
          return {
            id: stableLocalId(server.id),
            device_id: server.id,
            name: server.label,
            nameKey: server.labelKey,
            status: deviceStatus,
            is_default: server.id === selectedDeviceId,
            // Even a same-host KCoder target is remote from the browser renderer: all terminal I/O
            // crosses the authenticated gateway instead of a Tauri/local-IPC bridge. Classifying it
            // as local makes the upstream workspace panel stop before calling startDeviceTerminal.
            ...
            capabilities: [
              ...
              'device-commands',
              'kcoder-gateway',
              ...(server.transport === 'local' ? ['kcoder-gateway-local'] : []),
              ...(server.capabilities?.terminalSessions !== false ? ['runtime-terminal'] : []),
              ...(server.capabilities?.browserSessions !== false ? ['runtime-browser'] : []),
              ...(server.capabilities?.workspaceFiles !== false ? ['runtime-workspace-files'] : []),
              ...(server.capabilities?.fileChangesRevert !== false
                ? ['runtime-file-changes-revert']
                : []),
            ],
            runtime_routes: [
              {
                kind: 'remote-relay' as const,
                device_id: server.id,
                runtime_device_id: server.id,
                device_type: 'remote',
                name: server.label,
                status: deviceStatus,
              },
            ...
            runtime_instance_id: `kcoder-gateway:${server.id}`,
            runtime_transfer_host: null,
          }
        })
      } catch (error) {
    ...
    async createWorkspaceTextFile(deviceId, parentPath, name, content = '') {
      const response = await executeCommand(deviceId, {
        command_key: 'workspace_create_text_file',
        path: parentPath,
        args: [name],
        stdin: content,
        timeout_seconds: 15,
      ...
      assertCommandSuccess(response, 'Failed to create workspace file')
      return normalizeWorkspaceTextFile(
        response.stdout,
        `${parentPath.replace(/\/+$/, '')}/${name}`
      )
    },
    async createWorkspaceDirectory(deviceId, parentPath, name) {
      const response = await executeCommand(deviceId, {
      ...
      assertCommandSuccess(response, 'Failed to create workspace directory')
      return normalizeWorkspaceEntry(response.stdout, parentPath, parentPath)
    },
    async renameWorkspaceEntry(deviceId, parentPath, name, newName) {
      const response = await executeCommand(deviceId, {
        command_key: 'workspace_rename_entry',
        path: parentPath,
        args: [name, newName],
        timeout_seconds: 15,
        max_output_bytes: 4096,
      })
      assertCommandSuccess(response, 'Failed to rename workspace entry')
      return normalizeWorkspaceEntry(response.stdout, parentPath, parentPath)
    },
    async deleteWorkspaceEntry(deviceId, parentPath, name, recursive = false) {
      const response = await executeCommand(deviceId, {
        command_key: 'workspace_delete_entry',
        path: parentPath,
        args: [name, String(recursive)],
        timeout_seconds: 15,
        max_output_bytes: 4096,
      })
      assertCommandSuccess(response, 'Failed to delete workspace entry')
    },
  }
  const runtimeWorkApi = createRuntimeWorkApiFromIpc(
    ...
    getLocalDeviceId,
    {
      ...(kcoderGateway
        ? {
            resolveDeviceId: async (data?: Record<string, unknown>) =>
              kcoderDeviceIdFromData(data, await getLocalDeviceId()),
            normalizeDeviceRecord: <T extends Record<string, unknown>>(data: T) => data,
            adaptListResponse: adaptKCoderRuntimeWorkListResponse,
          }
        : {}),
      cloudModelGateway: deps.cloudModelGateway,
      user: deps.user,
      includeLocalProxy: !kcoderGateway,
    }
  ) as unknown as NonNullable<WorkbenchServices['runtimeWorkApi']>
  const deliveryApi = createLocalDeliveryApi((method, params) => request(method, params))
  const externalIssueApi = createExternalIssueApi((method, params) => request(method, params))
  ...
  return {
    ...
    modelApi: {
      listModels: async (target?: ModelCatalogTarget, options?: { signal?: AbortSignal }) => {
        options?.signal?.throwIfAborted()
        let codexOfficialModels: CodexOfficialModel[]
        let supportsModelSelectionMode = false
        ...
        try {
          ...
          const [codexOfficialResult, nextCodexAuthConfigured] = await Promise.all([
            requestLocalCodexOfficialModels(request, kcoderGateway ? target : undefined, options).then(
            ...
            kcoderGateway ? Promise.resolve(false) : loadLocalCodexAuthConfigured(request),
          ])
          codexOfficialModels = codexOfficialResult.value?.models ?? []
          supportsModelSelectionMode = kcoderGateway && codexOfficialResult.value?.supportsModelSelectionMode === true
          if (kcoderGateway && codexOfficialResult.error) throw new Error(codexOfficialResult.error)
          codexOfficialError = codexOfficialResult.error
          codexAuthConfigured = nextCodexAuthConfigured
        } catch (error) {
          if (kcoderGateway) throw error
          codexOfficialModels = []
          codexOfficialError = error instanceof Error ? error.message : String(error)
          codexAuthConfigured = false
        }
        return {
          data: localRuntimeModels(
            codexOfficialModels,
            codexOfficialError,
            codexAuthConfigured,
            !kcoderGateway
          ).map(model => supportsModelSelectionMode
            ? { ...model, config: { ...model.config, supportsModelSelectionMode: true } }
            : model),
        }
      },
    },
    ...
    ...(kcoderGateway
      ? {
          workspaceSessionApi: {
            startDeviceTerminal: startGatewayTerminal,
            restoreDeviceTerminal: restoreGatewayTerminal,
            createRemoteTerminalClient: createGatewayTerminalClient,
            startProjectTerminal: async () => {
              throw new Error('KCoder 项目终端需要明确的服务器目标')
            },
            startDeviceCodeServer: async () => {
              throw new Error('KCoder 网关尚未实现 Code Server')
            },
            startProjectCodeServer: async () => {
              throw new Error('KCoder 网关尚未实现 Code Server')
            },
          },
        }
