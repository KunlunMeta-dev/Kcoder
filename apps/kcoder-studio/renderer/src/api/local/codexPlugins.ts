...
import { createKCoderPluginApi } from '@/kcoder/gatewayPluginApi'
...
export interface PluginTrustState {
  entries: Array<{
    path: string
    decision: 'trust' | 'revoke' | 'never'
    effective: 'trusted' | 'never' | 'untrusted'
    source: 'default_home' | 'explicit'
  }>
  bypassActive: boolean
}

export interface LocalCodexPluginApi {
  listTrust?(): Promise<PluginTrustState>
  setTrust?(path: string, action: 'trust' | 'revoke' | 'never'): Promise<PluginTrustState>
  readPluginIcon?(id: string, dark: boolean): Promise<string | null>
  ...
  installAvailablePlugin(
    ...
    options?: { installAttemptId: string }
  ): Promise<InstalledPlugin>
  cancelInstallAttempt?(installAttemptId: string): Promise<{ cancellationRequested: boolean }>
...
export function createLocalCodexPluginApi(target?: {
  isScopeCurrent?: () => boolean
  deviceId?: string
  workspacePath?: string
}): LocalCodexPluginApi {
  const base = createNativePluginApi()
  return runtimeNameForCurrentHost() === 'kcoder' ? createKCoderPluginApi(base, target) : base
}

function createNativePluginApi(): LocalCodexPluginApi {
