...
describe('createHybridWorkbenchServices', () => {
  beforeEach(() => {
    ...
    mocks.localListTeams.mockResolvedValue([
      { id: 0, name: 'local-wework', is_active: true, default_for_modes: ['studio'] },
    ])
    ...
    mocks.cloudListTeams.mockResolvedValue([
      { id: 1, name: 'cloud-wework', is_active: true, default_for_modes: ['studio'] },
    ])
  ...
  it('loads cloud models in the background without delaying local models', async () => {
    ...
    expect(refreshed.data.map(model => model.name)).toEqual(['gpt-5.5'])
    expect(info).toHaveBeenCalledWith('[KCoder Studio] Cloud model catalog loaded', {
      count: 1,
      models: [
        {
          name: 'codex-gpt-5.5',
