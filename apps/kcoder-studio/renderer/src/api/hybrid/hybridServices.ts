...
export function createHybridWorkbenchServices(
  ...
  const loadCloudModelsInBackground = () => {
    ...
    cloudModelsRequest = Promise.resolve()
      ...
      .then(response => {
        ...
        console.info('[KCoder Studio] Cloud model catalog loaded', modelCatalogLog)
        void writeInfoLog(
          `[KCoder Studio] Cloud model catalog loaded ${JSON.stringify(modelCatalogLog)}`
        ).catch(() => undefined)
      ...
      .catch(error => {
        console.warn('[KCoder Studio] Failed to refresh cloud models in background', error)
      })
      .finally(() => {
  ...
  const archiveAllCloudConversationsInBackground = () => {
    void Promise.resolve()
      ...
      .catch(error => {
        console.warn('[KCoder Studio] Failed to archive cloud conversations in background', error)
      })
  }
  ...
  const searchCloudRuntimeWorkInBackground = (data: RuntimeWorkSearchRequest) => {
    ...
    const pending = searchCloudRuntimeWork(request)
      ...
      .catch(error => {
        console.warn('[KCoder Studio] Failed to refresh cloud search results in background', error)
      })
      .finally(() => {
  ...
  const listCloudArchivesInBackground = (data: ArchivedConversationsListRequest = {}) => {
    ...
    const pending = Promise.resolve()
      ...
      .catch(error => {
        console.warn('[KCoder Studio] Failed to refresh cloud archives in background', error)
      })
      .finally(() => {
  ...
  const hybridDeviceApi: WorkbenchServices['deviceApi'] = {
    ...
    createWorkspaceTextFile(deviceId, parentPath, name, content) {
      if (!isLocalDeviceId(deviceId))
        throw new Error('Workspace file creation is only available for local devices')
      return localServices.deviceApi.createWorkspaceTextFile(deviceId, parentPath, name, content)
    },
    createWorkspaceDirectory(deviceId, parentPath, name) {
      if (!isLocalDeviceId(deviceId))
        throw new Error('Workspace directory creation is only available for local devices')
      return localServices.deviceApi.createWorkspaceDirectory(deviceId, parentPath, name)
    },
    renameWorkspaceEntry(deviceId, parentPath, name, newName) {
      if (!isLocalDeviceId(deviceId))
        throw new Error('Workspace rename is only available for local devices')
      return localServices.deviceApi.renameWorkspaceEntry(deviceId, parentPath, name, newName)
    },
    deleteWorkspaceEntry(deviceId, parentPath, name, recursive) {
      if (!isLocalDeviceId(deviceId))
        throw new Error('Workspace deletion is only available for local devices')
      return localServices.deviceApi.deleteWorkspaceEntry(deviceId, parentPath, name, recursive)
    },
  ...
  const hybridRuntimeWorkApi: NonNullable<WorkbenchServices['runtimeWorkApi']> = {
    ...
    async getRuntimeTranscript(data: RuntimeTranscriptRequest) {
      ...
      try {
        ...
        console.error('[KCoder Studio] Hybrid runtime transcript failed', {
          route,
    ...
    steerRuntimeSubagent(data) {
      return routeByAddress(data.address).steerRuntimeSubagent(data)
    },
    readSubagentArtifact(data) {
      return routeByAddress(data.address).readSubagentArtifact(data)
    },
    getRuntimeGoal(data) {
      return routeByAddress(data.address).getRuntimeGoal(data)
    },
    getRuntimeSessionModes(data) {
      return (
        data.address ? routeByAddress(data.address) : runtimeApi(data.deviceId)
      ).getRuntimeSessionModes(data)
    },
    setRuntimeGoal(data) {
    ...
    previewWorktreeArchive(data) {
      return runtimeApi(data.deviceId).previewWorktreeArchive(data)
    },
    archiveWorktree(data) {
      return runtimeApi(data.deviceId).archiveWorktree(data)
    },
    restoreWorktree(data) {
      return runtimeApi(data.deviceId).restoreWorktree(data)
    },
    forgetWorktree(data) {
      return runtimeApi(data.deviceId).forgetWorktree(data)
    },
    ...
    shortenWaitRuntimeTask(address: RuntimeTaskAddress) {
      return routeByAddress(address).shortenWaitRuntimeTask(address)
    },
  ...
  return {
    ...
    modelApi: {
      async listModels(target?: ModelCatalogTarget, options?: { signal?: AbortSignal }): Promise<UnifiedModelListResponse> {
        const localModels = await localServices.modelApi.listModels(target, options)
