...
export interface DeviceListOptions {
  // Selected-target mode is for initial Gateway bootstrap; full health remains the default.
  health?: 'selected' | 'full'
}
...
export function createDeviceApi(client: HttpClient) {
  async function fetchDevices(_options?: DeviceListOptions): Promise<DeviceInfo[]> {
    // Backend-relay devices have no client-side Gateway health probe to defer.
    void _options
  ...
  return {
    ...
    async createWorkspaceTextFile(
      ...
      const normalizedParent = normalizeAbsoluteWorkspacePath(
        parentPath,
        'Workspace path must be absolute'
      )
      const response = await client.post<DeviceCommandResponse>(
      ...
      if (!response.success)
        throw new Error(response.error || response.stderr || 'Failed to create workspace file')
      return normalizeWorkspaceTextFile(
        response.stdout,
        `${normalizedParent.replace(/\/+$/, '')}/${name}`
      )
    },

    async createWorkspaceDirectory(deviceId: string, parentPath: string, name: string) {
      const normalizedParent = normalizeAbsoluteWorkspacePath(
        parentPath,
        'Workspace path must be absolute'
      )
      ...
      if (!response.success)
        throw new Error(response.error || response.stderr || 'Failed to create workspace directory')
      return normalizeWorkspaceEntry(response.stdout, normalizedParent, normalizedParent)
    },
    ...
    async renameWorkspaceEntry(
      ...
      if (!response.success)
        throw new Error(response.error || response.stderr || 'Failed to rename workspace entry')
      return normalizeWorkspaceEntry(response.stdout, normalizedParent, normalizedParent)
    },
    ...
    async deleteWorkspaceEntry(
      ...
      if (!response.success)
        throw new Error(response.error || response.stderr || 'Failed to delete workspace entry')
    },
