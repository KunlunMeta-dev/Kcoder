...
interface RuntimeChatStreamSubscription {
...
export function createRuntimeChatStream(deps: RuntimeChatStreamDeps) {
  const subscriptions = new Map<number, RuntimeChatStreamSubscription>()
  const pendingEvents: LocalExecutorEvent[] = []
  const maxPendingEvents = 512
  const incompleteScopes = new Set<string>()
  let allScopesIncomplete = false
  const scopeKey = (taskId: string, deviceId: unknown) => JSON.stringify([deviceId ?? null, taskId])
  const eventScopeKey = (event: LocalExecutorEvent) => {
    const payload = asRecord(event.payload)
    return scopeKey(stringField(payload, 'taskId') ?? '', stringField(payload, 'deviceId'))
  }
  let nativeCleanup: (() => void) | null = null
  let nativeSubscribePromise: Promise<void> | null = null
  function deliverEvent(
    subscriptionId: number,
    subscription: RuntimeChatStreamSubscription,
  ...
  function ensureNativeListener(): void {
    ...
    nativeSubscribePromise = Promise.resolve(
      deps.subscribe(event => {
        if (event.event === 'executor.account_context_invalidated') {
          const targetId = stringField(asRecord(event.payload), 'deviceId')
          if (!targetId) return
          for (let index = pendingEvents.length - 1; index >= 0; index--) {
            if (stringField(asRecord(pendingEvents[index].payload), 'deviceId') === targetId) pendingEvents.splice(index, 1)
          }
          for (const key of incompleteScopes) if (JSON.parse(key)[0] === targetId) incompleteScopes.delete(key)
          for (const subscription of subscriptions.values()) {
            if (subscription.handlers.scope?.deviceId === targetId) subscription.state = createResponseApiStreamState()
          }
          clearRuntimeTargetPlans(targetId)
          return
        }

        if (event.event === 'executor.runtime_replaced') {
          // Buffering covers only the first-subscription race for the current transport.
          // After runtime replacement, deltas/completion from the old transport cannot replay into the recovered pane.
          pendingEvents.length = 0
          incompleteScopes.clear()
          allScopesIncomplete = false
        }
        if (import.meta.env.DEV && event.event === 'runtime.plan.updated') {
          console.warn('[KCoder Studio] Runtime task plan event received', {
        ...
        if (isRuntimeChatStreamDebugEnabled() && event.event === 'response.subagent.activity') {
          const payload = asRecord(event.payload)
          console.debug('[KCoder Studio] Runtime subagent event routing', {
            taskId: stringField(payload, 'taskId'),
            deviceId: stringField(payload, 'deviceId'),
            agentPath: stringField(asRecord(payload.data), 'agent_path'),
            matchedSubscriptions: subscriptionEntries.filter(([, subscription]) =>
        ...
        let exactScopedMatchCount = 0
        const eventTaskId = stringField(asRecord(event.payload), 'taskId')
        for (const [subscriptionId, subscription] of subscriptionEntries) {
          if (event.event === 'executor.runtime_replaced') {
            deliverEvent(subscriptionId, subscription, event)
            continue
          }
          ...
          if (!inScope && import.meta.env.DEV) {
            console.warn(
              '[KCoder Studio] Runtime task plan event forwarded outside subscription scope',
              {
                subscriptionId,
              }
            )
          ...
          if (
            eventTaskId &&
            subscription.handlers.scope?.taskId === eventTaskId &&
            isLocalExecutorEventInScope(event, subscription.handlers.scope)
          ) {
            exactScopedMatchCount += 1
          }
          deliverEvent(subscriptionId, subscription, event)
        }
        if (exactScopedMatchCount === 0 && isMappedResponseApiEvent(event.event) && eventTaskId) {
          const key = eventScopeKey(event)
          if (!allScopesIncomplete && !incompleteScopes.has(key)) {
            pendingEvents.push(event)
            if (pendingEvents.length > maxPendingEvents) {
              const lostKey = eventScopeKey(pendingEvents[0])
              // Do not replay a suffix as if it were a complete stream. Keep a
              // bounded dirty marker until a history-capable pane subscribes.
              incompleteScopes.add(lostKey)
              for (let index = pendingEvents.length - 1; index >= 0; index--) {
                if (eventScopeKey(pendingEvents[index]) === lostKey) pendingEvents.splice(index, 1)
              }
              if (incompleteScopes.size > maxPendingEvents) {
                incompleteScopes.clear()
                pendingEvents.length = 0
                allScopesIncomplete = true
              }
            }
          }
        }
      ...
      .catch(error => {
        nativeSubscribePromise = null
        console.error('[KCoder Studio] Runtime chat stream native listener failed', {
  ...
  return {
    ...
    subscribe(handlers: ChatStreamHandlers): () => void {
      ...
      const key = handlers.scope?.taskId
        ? scopeKey(handlers.scope.taskId, handlers.scope.deviceId)
        : null
      if (key && handlers.onStreamGap && (allScopesIncomplete || incompleteScopes.has(key))) {
        incompleteScopes.delete(key)
        handlers.onStreamGap()
      }
      for (let index = 0; index < pendingEvents.length;) {
        const event = pendingEvents[index]
        if (!isLocalExecutorEventInScope(event, handlers.scope)) {
          index += 1
          continue
        }
...
function logRuntimeChatTerminalEvent(
  ...
  const payload = asRecord(event.payload)
  console.info('[KCoder Studio] Runtime chat stream terminal event received', {
    event: event.event,
...
function logRuntimeChatStreamSubscription(
  ...
  console.debug('[KCoder Studio] Runtime chat stream subscription', {
    action,
    subscriptionId,
    ...details,
  })
...
function logRuntimeChatStreamNativeSubscription(
  ...
  if (!isRuntimeChatStreamDebugEnabled()) return
  console.debug('[KCoder Studio] Runtime chat stream native subscription', {
    action,
    activeSubscriptions: activeRuntimeChatStreamSubscriptions,
    ...details,
  })
...
function logRuntimeChatStreamEvent(
  ...
  console.debug('[KCoder Studio] Runtime chat stream event', {
    subscriptionId,
    activeSubscriptions: activeRuntimeChatStreamSubscriptions,
    event: event.event,
