...
describe('createRuntimeWorkApi', () => {
  ...
  test('opens a runtime workspace without creating a task', async () => {
    const post = vi.fn().mockResolvedValue({
      accepted: true,
      deviceId: 'device-1',
      workspacePath: '/Users/dev/Documents/hello-0',
      runtime: 'codex',
    })
    const api = createRuntimeWorkApi({ post } as unknown as HttpClient)
  ...
  test('routes the safe managed-worktree lifecycle without dropping CAS fields', async () => {
    const post = vi.fn().mockResolvedValue({ success: true })
    const api = createRuntimeWorkApi({ post } as unknown as HttpClient)
    await api.previewWorktreeArchive({ deviceId: 'device-1', path: '/managed/task/repo' })
    await api.archiveWorktree({
      deviceId: 'device-1',
      path: '/managed/task/repo',
      expectedRevision: 3,
    ...
    expect(post.mock.calls).toEqual([
      [
        '/runtime-work/worktrees/archive/preview',
        { deviceId: 'device-1', path: '/managed/task/repo' },
      ],
      [
        '/runtime-work/worktrees/archive',
        {
          deviceId: 'device-1',
          path: '/managed/task/repo',
          expectedRevision: 3,
      ...
      [
        '/runtime-work/worktrees/restore',
        { deviceId: 'device-1', path: '/managed/task/repo', expectedRevision: 5 },
      ],
      [
        '/runtime-work/worktrees/forget',
        {
          deviceId: 'device-1',
          path: '/managed/task/repo',
          expectedRevision: 7,
