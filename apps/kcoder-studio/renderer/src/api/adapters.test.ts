...
describe('REST adapters', () => {
  ...
  test('creates Git workspace projects from the cloud client origin', async () => {
    const client = mockClient()
  ...
  test('picks the configured wework default team', async () => {
    ...
    vi.mocked(client.get).mockResolvedValueOnce({
      ...
      items: [
        ...
        { id: 3, name: 'studio', default_for_modes: ['studio'], is_active: true },
      ],
    })
  ...
  test('does not fallback to code or chat when the wework default team is missing', async () => {
    ...
    await expect(createTeamApi(client).getDefaultWorkbenchTeam()).rejects.toThrow(
      'KCoder default team is not configured'
    )
  })
    ...
    const skills = [
      {
        ...
        path: '/Users/dev/.codex/skills/zeta/SKILL.md',
        source: 'codex',
      },
      {
        name: 'Dws',
        description: 'DingTalk skill from Claude',
        path: '/Users/dev/.claude/skills/dws/SKILL.md',
        source: 'claude',
      },
      {
        name: 'dws',
        description: 'DingTalk skill from Codex',
        path: '/Users/dev/.codex/skills/dws/SKILL.md',
        source: 'codex',
      },
      {
        name: 'alpha',
        description: 'Alpha skill',
        path: '/Users/dev/.codex/skills/alpha/SKILL.md',
        source: 'codex',
      },
    ]
    vi.mocked(client.post).mockResolvedValueOnce({
  ...
  test('configures shared local skill directories through the device command API', async () => {
    ...
    const result = {
      success: true,
      status: 'configured',
      shared_path: '/Users/dev/.agents/skills',
      shared_created: true,
      legacy_paths: ['/Users/dev/.codex/skills', '/Users/dev/.claude/skills'],
      moved_count: 2,
      moved: [],
      links: [],
    }
