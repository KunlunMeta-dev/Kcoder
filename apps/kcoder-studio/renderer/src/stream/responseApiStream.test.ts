...
describe('emitResponseApiEvent', () => {
  test('preserves local runtime classification through the failed-message boundary', () => {
    const onChatError = vi.fn()
    emitResponseApiEvent({ onChatError }, 'response.failed', {
      taskId: 'task-1', subtaskId: 'failed-turn', type: 'local_runtime_error',
      data: { message: 'Storage initialization failed' },
    }, createResponseApiStreamState())
    expect(onChatError).toHaveBeenCalledWith(expect.objectContaining({
      type: 'local_runtime_error', error: 'Storage initialization failed',
  ...
  test('emits subagent activity payloads', () => {
    ...
    expect(onSubagentActivity).toHaveBeenCalledWith({
      ...
      status: 'running',
      steerStatus: 'applied',
      steerMessageId: 'msg-1',
      clientMessageId: 'client-1',
      occurredAtMs: 12345,
    })
  })

  test('preserves a background subagent running lifecycle in a completed tool call', () => {
      ...
      'response.output_item.done',
      {
        ...
        data: {
          item: {
            ...
            name: 'spawn_agent',
            status: 'completed',
            output: { agent_id: 'agent-1', status: 'running' },
          },
        },
      },
      createResponseApiStreamState()
