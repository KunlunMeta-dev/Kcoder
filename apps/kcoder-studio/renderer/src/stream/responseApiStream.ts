...
export function clearRuntimeTargetPlans(targetId: string): void {
  for (const key of runtimeTaskPlans.keys()) if (key.startsWith(`${targetId}:`)) runtimeTaskPlans.delete(key)
}
...
function warnDroppedResponseBlock(
  ...
  console.warn('[KCoder Studio] Dropped response block event', {
    event,
    reason,
    taskId: base.taskId,
...
function warnDroppedResponseDelta(
  ...
  console.warn('[KCoder Studio] Dropped response delta event', {
    event,
    reason,
    taskId: base.taskId,
    subtaskId: base.subtaskId,
...
function emitSubagentActivity(
  ...
  handlers.onSubagentActivity?.({
    ...
    retainedRuns: optionalNumberField(data, 'retained_runs'),
    retainedRunLimit: optionalNumberField(data, 'retained_run_limit'),
    capacityWarning: typeof data.capacity_warning === 'boolean' ? data.capacity_warning : undefined,
    steerStatus: stringField(data, 'steer_status') ?? stringField(data, 'steerStatus'),
    steerMessageId: stringField(data, 'message_id') ?? stringField(data, 'messageId'),
    clientMessageId: stringField(data, 'client_message_id') ?? stringField(data, 'clientMessageId'),
    occurredAtMs:
...
export function emitResponseApiEvent(
  ...
  if (eventName === 'runtime.plan.updated') {
    ...
    if (import.meta.env.DEV) {
      console.warn('[KCoder Studio] Runtime task plan parsed', {
  ...
  if (
    ...
    handlers.onChatError?.({
      ...
      providerFailure: decodeProviderFailure(data.provider_failure),
