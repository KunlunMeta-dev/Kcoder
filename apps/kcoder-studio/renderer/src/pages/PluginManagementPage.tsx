...
import { KCoderPluginManagementWorkspace } from '@/kcoder/KCoderPluginManagementWorkspace'
...
export function PluginManagementPage() {
  ...
  const ManagementWorkspace =
    runtimeNameForCurrentHost() === 'kcoder'
      ? KCoderPluginManagementWorkspace
      : PluginManagementWorkspace
  const pluginTarget =
    runtimeNameForCurrentHost() === 'kcoder' ? workbenchModelTarget(state) : undefined
  const [settingsOpen, setSettingsOpen] = useState(false)
  const mobileSettingsLayout = useSettingsLayout(isMobile, settingsOpen)
  ...
  return (
    <div className="flex h-full overflow-hidden bg-background text-text-primary">
      ...
      <ManagementWorkspace
        key={JSON.stringify(pluginTarget)}
        targetDeviceId={pluginTarget?.deviceId}
        targetWorkspacePath={pluginTarget?.workspacePath}
