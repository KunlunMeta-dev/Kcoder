...
const CLASSIFICATION_RULES: Array<[ChatErrorType, Array<string | RegExp>]> = [
  // Compatibility for older app-servers without structured runtime failures.
  ['local_runtime_error', ['planstore requires secure handle-relative directory i/o']],
  [
    'context_length_exceeded',
    [
      'prompt is too long',
  ...
  ['quota_exceeded', QUOTA_EXHAUSTION_PATTERNS],
...
export function parseChatError(error: string, backendType?: string | null, providerFailure?: unknown): ParsedChatError {
  const facts = decodeProviderFailure(providerFailure)
  if (facts) {
    const type = facts.category
    return { type, titleKey: `assistant_error.types.${type}.title`, descriptionKey: `assistant_error.types.${type}.description` }
  }
  // HTTP status takes precedence over broad provider wording and stale UI labels.
  // Match status syntax, not incidental digits in request IDs or model names.
  const status = error.match(
    /(?:^\s*|\bstatus(?:_code| code)?["']?\s*[:=]\s*|\bHTTP(?:\/[\d.]+)?\s+|\bAPI(?:\s+Error)?\s*[:(]\s*)(401|403|429)\b/i
  )?.[1]
  const backend = normalizeBackendType(backendType)
  const structured = normalizeBackendType(extractStructuredErrorType(error))
  const statusType: ChatErrorType | undefined =
    status === '401'
      ? 'authentication_error'
      : status === '403'
        ? 'forbidden'
        : status === '429'
          ? backend === 'quota_exceeded' ||
            structured === 'quota_exceeded' ||
            QUOTA_EXHAUSTION_PATTERNS.some(pattern => error.toLowerCase().includes(pattern))
            ? 'quota_exceeded'
            : 'rate_limit'
          : undefined
  const type = statusType ?? backend ?? structured ?? classifyByMessage(error)
