...
describe('model-ui', () => {
  ...
  test('filters reasoning and speed controls out of the selector surface', () => {
  ...
  test('drops stored reasoning values from normalized options and display labels', () => {
    const model: UnifiedModel = {
      name: 'gpt-5.5',
      type: 'runtime',
      displayName: 'gpt-5.5',
    ...
    expect(getControlsForModel(model).find(control => control.id === 'reasoning')).toBeUndefined()
    expect(normalizeModelOptions(model, { reasoning: 'extra_high' })).toEqual({})
    expect(getSelectedModelDisplayLabel(model, { reasoning: 'extra_high' })).toBe('gpt-5.5')
  })

  test('renders region prefixes with English labels', () => {
    const intranetModel: UnifiedModel = {
      name: 'glm-5',
      type: 'public',
      displayName: '内网:GLM5',
    }
    const publicModel: UnifiedModel = {
      name: 'glm-5.1',
      type: 'public',
      displayName: '公网:GLM-5.1',
    }
    const overseasModel: UnifiedModel = {
      name: 'claude-opus',
      type: 'public',
      displayName: '海外:Claude-Opus4.6',
    }

    expect(getModelDisplayLabel(intranetModel)).toBe('Intranet:GLM5')
    expect(getModelDisplayLabel(publicModel)).toBe('Public:GLM-5.1')
    expect(getModelDisplayLabel(overseasModel)).toBe('Overseas:Claude-Opus4.6')
  })
})

describe('declared per-model reasoning policy', () => {
  const model = (mode: string, efforts = ['none', 'high']): UnifiedModel => ({
    name: 'unknown-vendor-model', type: 'public', config: { modelConfiguration: { reasoningPolicy: { mode, efforts } } },
  })
  test('shows only explicitly declared efforts, never a guessed model-family set', () => {
    const reasoning = getControlsForModel(model('optional')).find(control => control.id === 'reasoning')
    expect(reasoning?.options.map(option => option.value)).toEqual(['default', 'none', 'high'])
    expect(reasoning?.defaultValue).toBe('default')
  })
  test.each(['hidden', 'always_on', 'always_off'])('does not offer overrides for %s', mode => {
    expect(getControlsForModel(model(mode)).some(control => control.id === 'reasoning')).toBe(false)
  })
})
