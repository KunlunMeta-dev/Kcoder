...
describe('windows file workspace targets', () => {
  test('creates a local target for verbatim windows file paths', () => {
    expect(
      createLocalFileWorkspaceTarget(
      ...
      deviceId: 'device-local-real',
      path: 'C:/Users/kunlunmeta/projects/gpt-model-service/src',
      source: 'runtime',
      workspaceSource: 'local',
    })
  })

  test('creates a local target for plain and posix-spelled windows paths', () => {
    expect(
      createLocalFileWorkspaceTarget(String.raw`C:\Users\me\project\file.ts`, [localDevice])?.path
    ).toBe('C:/Users/me/project')
    expect(
      createLocalFileWorkspaceTarget('//?/C:/Users/me/project/file.ts', [localDevice])?.path
    ).toBe('C:/Users/me/project')
  })

  test('creates a verbatim windows attachment target only for attachment paths', () => {
    expect(
      createLocalAttachmentWorkspaceTarget(
        String.raw`\\?\C:\Users\me\.wegent-executor\workspace\attachments\draft\42\result.png`,
        [localDevice]
      )
    ).toMatchObject({
      deviceId: 'device-local-real',
      path: 'C:/Users/me/.wegent-executor/workspace/attachments/draft/42',
      workspaceSource: 'local',
    })
    expect(
      createLocalAttachmentWorkspaceTarget(String.raw`C:\Users\me\project\x.png`, [localDevice])
    ).toBeNull()
  })
})
