import { isWindowsNamespacePath, stripWindowsNamespacePrefix } from './workspace-file-path'
...
export function normalizeDevicePath(path: string): string {
  const rewritten = stripWindowsNamespacePrefix(path)
  if (rewritten === null && isWindowsNamespacePath(path)) {
    // Namespaces that cannot be rewritten (literal trailing dots or spaces)
    // keep their spelling; slashing them would fabricate a different path.
    return path
  }
  const trimmed = (rewritten ?? path).trim().replace(/\\/g, '/')
  if (!trimmed) return ''
