...
declare global {
  interface Window {
    __KCODER_STUDIO_PERF__?: PerformanceDiagnosticsController
  }
...
export function isPerformanceDiagnosticsEnabled(): boolean {
  ...
  return (
    ...
    import.meta.env.VITE_KCODER_STUDIO_PERF_DEBUG === '1'
  )
}
...
export function installPerformanceDiagnostics(): PerformanceDiagnosticsController | null {
  ...
  controller = {
    ...
    stop: () => {
      ...
      if (window.__KCODER_STUDIO_PERF__ === controller) {
        delete window.__KCODER_STUDIO_PERF__
      }
      controller = null
    },
    getEvents: () => [...events],
  }
  window.__KCODER_STUDIO_PERF__ = controller
  ...
  console.info(
    '[Wework perf] diagnostics enabled. Use window.__KCODER_STUDIO_PERF__.snapshot() to inspect current state.'
  )
  return controller
}
