...
describe('project workspace selection', () => {
  ...
  test('orders local multi-root workspaces by the saved primary root', () => {
        ...
        projects: [
          {
            project: {
              ...
              roots: [
                { kind: 'local', path: '/repo/backend' },
                { kind: 'local', path: '/repo/frontend' },
              ],
            },
            deviceWorkspaces: [
              {
    ...
    expect(options[0].workspaces.map(workspace => workspace.id)).toEqual([102, 101])
  })
