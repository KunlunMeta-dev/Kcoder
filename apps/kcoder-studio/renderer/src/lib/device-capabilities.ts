...
export const KCODER_STUDIO_MIN_EXECUTOR_VERSION = '1.8.5'
...
export function isKCoderLocalGatewayDevice(
  device: Pick<DeviceInfo, 'capabilities'>
): boolean {
  return device.capabilities?.includes('kcoder-gateway-local') === true
}

export function isKCoderGatewayDevice(device: Pick<DeviceInfo, 'capabilities'>): boolean {
  return device.capabilities?.includes('kcoder-gateway') === true
}

/**
 * Goal availability before a task exists cannot depend on `device_type`:
 * gateway-routed KCoder servers (including SSH remotes) are classified as
 * 'remote', yet they are the only runtime that implements /goal.
 */
export function isGoalCapableWorkbenchDevice(
  device: Pick<DeviceInfo, 'capabilities' | 'device_type'> | undefined | null,
  ...
  return Boolean(
    device?.device_type === 'local' ||
      ...
      deviceId === 'local' ||
      (device && isKCoderGatewayDevice(device))
  )
}
...
export function isExecutorVersionCompatible(version?: string | null): boolean {
  if (!version) return false
  return isVersionAtLeast(version, KCODER_STUDIO_MIN_EXECUTOR_VERSION)
}

export function isDeviceBelowStudioVersion(device: DeviceLike): boolean {
  return !isExecutorVersionCompatible(device.executor_version)
}

export function isStudioCompatibleDevice(device: DeviceLike): boolean {
  return isClaudeCodeDevice(device) && isExecutorVersionCompatible(device.executor_version)
}

export function hasStudioUpdateAvailable(device: DeviceLike): boolean {
...
export function canUseForProjectCreation(device: DeviceLike): boolean {
  return (
    ...
    isExecutorVersionCompatible(device.executor_version)
  )
}
...
export function supportsCodeServerSessions(device: DeviceLike, deviceId?: string | null): boolean {
  if (device.capabilities?.includes('kcoder-gateway')) {
    return device.capabilities.includes('code-server')
  }
  return supportsCloudSessions(device, deviceId) || supportsRemoteSessions(device, deviceId)
}
