...
describe('runtime project helpers', () => {
  test('preserves explicit built-in name provenance for composer display', () => {
    const value = { project: { key: 'local', name: '当前计算机', nameKey: 'currentComputer' }, deviceWorkspaces: [], totalTasks: 0 } as RuntimeProjectWork
    expect(runtimeProjectToProject(value).nameKey).toBe('currentComputer')
  })
