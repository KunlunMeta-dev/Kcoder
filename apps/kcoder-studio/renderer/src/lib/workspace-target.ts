...
export function createLocalFileWorkspaceTarget(
  ...
  try {
    normalizedPath = normalizeAbsoluteWorkspacePath(filePath, 'invalid local file path')
  } catch {
    return null
  }
...
export function createLocalAttachmentWorkspaceTarget(
  ...
  let normalizedPath: string
  try {
    normalizedPath = normalizeAbsoluteWorkspacePath(filePath, 'invalid local attachment path')
  } catch {
    return null
  }
