...
export function isStudioSelectableStandaloneDevice(device: SelectableDevice): boolean {
...
export function sortStandaloneDevices<T extends SelectableDevice>(devices: T[]): T[] {
  return devices.filter(isClaudeCodeDevice).sort((left, right) => {
    ...
    const leftCompatible = isExecutorVersionCompatible(left.executor_version) ? 0 : 1
    const rightCompatible = isExecutorVersionCompatible(right.executor_version) ? 0 : 1
...
export function getPreferredStandaloneDeviceId(
  ...
  if (currentDevice && isStudioSelectableStandaloneDevice(currentDevice)) {
    return currentDevice.device_id
  }

  return sortStandaloneDevices(devices).find(isStudioSelectableStandaloneDevice)?.device_id ?? null
}
