import { readReasoningPolicy } from '../../../shared/modelConfiguration'
...
export const MODEL_FAMILY_CONFIGS: ModelFamilyConfig[] = [
  ...
  { id: 'codex-official', label: 'Codex Official', order: 20, controls: OPENAI_RESPONSES_CONTROLS },
  {
    id: 'codex-provider',
  ...
  { id: 'model-interface', label: 'Custom Models', order: 40, controls: OPENAI_RESPONSES_CONTROLS },
  {
    id: 'gpt',
    label: 'GPT',
    order: 50,
  ...
  { id: 'other', label: 'Other', order: 100, controls: [] },
]
...
export function getControlsForModel(model: UnifiedModel | null): ModelControlConfig[] {
  ...
  const controls = familyConfig.controls
    .filter(control => {
      // Reasoning effort and speed selection were removed from the Studio
      // UI; the engine keeps using the default reasoning_effort from
      // provider settings.
      if (control.id === 'reasoning' || control.id === 'speed') return false
  ...
  const policy = readReasoningPolicy(
    (model.config?.modelConfiguration as Record<string, unknown> | undefined)?.reasoningPolicy
  )
      ...
      options: [
        {
          value: 'default',
          label: 'Default',
          labelKey: 'providerSettings.reasoning_default',
          order: 0,
        },
        ...policy.efforts.map((effort, index) => ({
          value: effort,
          label: effort,
          labelKey: `providerSettings.reasoning_${effort}`,
          order: index + 1,
        })),
      ],
    })
