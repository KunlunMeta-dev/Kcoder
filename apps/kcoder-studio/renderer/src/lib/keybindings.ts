...
import { getKeyboardPlatform } from './keyboard-platform'
...
export const KCODER_STUDIO_OPEN_TERMINAL_EVENT = 'wework:open-terminal'
export const KCODER_STUDIO_STEP_FONT_SIZE_EVENT = 'wework:step-font-size'
export const KCODER_STUDIO_RESET_FONT_SIZE_EVENT = 'wework:reset-font-size'
...
const BASE_KEYBINDINGS: KeybindingCommand[] = [
  {
...
export function getDefaultKeybindings(): KeybindingCommand[] {
  const isMac = getKeyboardPlatform() === 'mac'
  return BASE_KEYBINDINGS.map(item => ({
    ...item,
    defaultKey: normalizeKeybinding(
      isMac ? item.defaultKey : item.defaultKey.replace('Command', 'Control')
    ),
  }))
}
...
export function mergeKeybindings(overrides: KeybindingOverride[]): Record<string, string | null> {
  const merged = new Map<string, string | null>(
    getDefaultKeybindings().map(item => [item.command, item.defaultKey])
  )
...
export function keybindingFromKeyboardEvent(event: KeyboardEvent): string {
  ...
  if (!key || ['Command', 'Control', 'Alt', 'Shift'].includes(key)) return ''
...
export function dispatchOpenTerminalShortcut() {
  ...
  window.dispatchEvent(new CustomEvent(KCODER_STUDIO_OPEN_TERMINAL_EVENT))
}
...
export function dispatchStepFontSizeShortcut(delta: -1 | 1) {
  window.dispatchEvent(new CustomEvent(KCODER_STUDIO_STEP_FONT_SIZE_EVENT, { detail: { delta } }))
}

export function dispatchResetFontSizeShortcut() {
  window.dispatchEvent(new CustomEvent(KCODER_STUDIO_RESET_FONT_SIZE_EVENT))
}
