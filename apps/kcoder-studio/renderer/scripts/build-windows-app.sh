...
TAURI_BUNDLES="${KCODER_STUDIO_TAURI_BUNDLES:-nsis}"
RELEASE_DEVTOOLS="${KCODER_STUDIO_RELEASE_DEVTOOLS:-}"
BRAND_CONFIG="${KCODER_STUDIO_BRAND_CONFIG:-}"

usage() {
  cat <<'EOF'
Usage: bash renderer/scripts/build-windows-app.sh [options]
...
Environment:
  ...
  KCODER_STUDIO_RELEASE_DEVTOOLS   Set to 1 to compile Tauri devtools into release builds.
  KCODER_STUDIO_BRAND_CONFIG       Default brand identity JSON.

Examples:
  bash renderer/scripts/build-windows-app.sh --profile dev
  bash renderer/scripts/build-windows-app.sh --target x86_64-pc-windows-msvc
  KCODER_STUDIO_RELEASE_DEVTOOLS=1 bash renderer/scripts/build-windows-app.sh
EOF
}
...
# to renderer/src-tauri/binaries/, so keep the two locations in sync.
...
if [ -n "$BRAND_CONFIG" ] || [ "$RELEASE_DEVTOOLS" = "1" ]; then
  ...
  wework_prepare_brand_config "$KCODER_STUDIO_DIR" "$BRAND_CONFIG" "${RELEASE_DEVTOOLS:-0}" "$CONFIG_OVERRIDE"
