#!/usr/bin/env bash
#
# Build (package) the KCoder Studio iOS app for a given environment.
#
# Usage:
...
# shellcheck source=lib/renderer-ios-env.sh
source "$SCRIPT_DIR/lib/renderer-ios-env.sh"
...
echo "Building KCoder Studio iOS app"
...
PBXPROJ="$KCODER_STUDIO_DIR/src-tauri/gen/apple/app.xcodeproj/project.pbxproj"
if [ -f "$PBXPROJ" ]; then
