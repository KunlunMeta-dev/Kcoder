...
Options:
  ...
  --drop-save-reply true    Drop one committed provider save reply in the owned test Gateway
  ...
  const server = createServer((request, response) => {
    void (async () => {
      ...
      if (request.method === 'POST' && url.pathname === '/command') {
        ...
        if (gatewayVerification && ['capture', 'resizeWindow', 'dragPointer'].includes(command.action)) {
          try {
            return actionResponse(
              ...
              command.action === 'capture'
                ...
                : command.action === 'resizeWindow'
                  ? await gatewayVerification.resizeWindow(command.selector, command.value)
                  : await gatewayVerification.dragPointer(command.selector, command.value),
              null
            )
          } catch (error) {
