...
# shellcheck source=lib/renderer-branding.sh
source "$SCRIPT_DIR/lib/renderer-branding.sh"
...
TAURI_BUNDLES="${KCODER_STUDIO_TAURI_BUNDLES:-}"
NO_SIGN="${KCODER_STUDIO_NO_SIGN:-}"
RELEASE_DEVTOOLS="${KCODER_STUDIO_RELEASE_DEVTOOLS:-1}"
BRAND_CONFIG="${KCODER_STUDIO_BRAND_CONFIG:-}"

usage() {
  cat <<'EOF'
Usage: bash renderer/scripts/build-mac-app.sh [options]
...
Environment:
  ...
  KCODER_STUDIO_TAURI_BUNDLES     Default bundle list when --bundles is not provided.
  KCODER_STUDIO_RELEASE_DEVTOOLS  Set to 0 to omit Web Inspector support.
  KCODER_STUDIO_BRAND_CONFIG      Default brand identity JSON.
  KCODER_STUDIO_NO_SIGN           Set to 1 to pass --no-sign.
...
Examples:
  bash renderer/scripts/build-mac-app.sh --profile dev --target aarch64-apple-darwin
  bash renderer/scripts/build-mac-app.sh --target aarch64-apple-darwin
  KCODER_STUDIO_RELEASE_DEVTOOLS=0 bash renderer/scripts/build-mac-app.sh --target aarch64-apple-darwin
EOF
}
...
if [ -n "$BRAND_CONFIG" ]; then
  CONFIG_OVERRIDE="$(mktemp "$KCODER_STUDIO_DIR/src-tauri/tauri.build.json.XXXXXX")"
  wework_prepare_brand_config "$KCODER_STUDIO_DIR" "$BRAND_CONFIG" "0" "$CONFIG_OVERRIDE"
  if [ -f "$CONFIG_OVERRIDE.namespace" ]; then
    export KCODER_STUDIO_EXECUTOR_NAMESPACE="$(<"$CONFIG_OVERRIDE.namespace")"
    rm -f "$CONFIG_OVERRIDE.namespace"
  fi
