...
#
# Responsibilities:
#   - Resolve project paths (SCRIPT_DIR, KCODER_STUDIO_DIR, ENV_DIR).
...
KCODER_STUDIO_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
KCODER_STUDIO_SCRIPTS_DIR="$(cd "$KCODER_STUDIO_LIB_DIR/.." && pwd)"
KCODER_STUDIO_DIR="$(cd "$KCODER_STUDIO_SCRIPTS_DIR/.." && pwd)"
KCODER_STUDIO_ENV_DIR="$KCODER_STUDIO_SCRIPTS_DIR/ios-env"
...
load_wework_env() {
  ...
  local env_file="$KCODER_STUDIO_ENV_DIR/$env_name.env"
  if [ ! -f "$env_file" ]; then
    echo "error: env file not found: $env_file" >&2
    echo "       available environments:" >&2
    local f
    for f in "$KCODER_STUDIO_ENV_DIR"/*.env; do
      [ -e "$f" ] || continue
  ...
  KCODER_STUDIO_RESOLVED_ENV_FILE="$env_file"
  return 0
}
...
print_wework_env_summary() {
  local env_name="$1"
  echo "  ENV=$env_name"
  echo "  ENV_FILE=${KCODER_STUDIO_RESOLVED_ENV_FILE:-<none>}"
