...
if [ "${KCODER_CARGO_TARGET_DIR_AUTO:-0}" = "1" ]; then
  unset CARGO_TARGET_DIR
  unset KCODER_CARGO_TARGET_DIR_AUTO
fi

configure_executor_dev_target_dir() {
  if [ "${KCODER_DISABLE_SHARED_CARGO_TARGET:-0}" = "1" ] || [ -n "${CARGO_TARGET_DIR:-}" ]; then
    configure_shared_cargo_target_dir "$PROJECT_DIR" "executor"
    return 0
  fi
...
if [ "${KCODER_CARGO_TARGET_DIR_AUTO:-0}" != "1" ] \
