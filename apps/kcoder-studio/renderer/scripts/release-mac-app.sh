...
# shellcheck source=lib/renderer-macos-signing.sh
source "$SCRIPT_DIR/lib/renderer-macos-signing.sh"

TARGET="local"
VERSION_OVERRIDE=""
RELEASE_NOTES="${RELEASE_NOTES:-New Wework release.}"
LOCAL_PROJECT_DIR="${LOCAL_PROJECT_DIR:-$KCODER_STUDIO_DIR/src-tauri/target/release/local-update-server}"
...
PROD_UPDATE_BASE_URL="${KCODER_STUDIO_UPDATE_BASE_URL:-}"
...
RELEASE_DEVTOOLS="${KCODER_STUDIO_RELEASE_DEVTOOLS:-1}"
...
Options:
  ...
  --update-base-url <url>    Production update service base URL. Overrides KCODER_STUDIO_UPDATE_BASE_URL.
...
Environment overrides:
  ...
  KCODER_STUDIO_UPDATE_BASE_URL, KCODER_STUDIO_UPDATE_PUBLISH_TOKEN, TAURI_SIGNING_PRIVATE_KEY,
  ...
  MACOS_BUILD_TARGET, KCODER_STUDIO_RELEASE_DEVTOOLS, VITE_WEGENT_BACKEND_URL,
...
bundle_root() {
  if [ -n "$MACOS_BUILD_TARGET" ]; then
    printf '%s\n' "$KCODER_STUDIO_DIR/src-tauri/target/$MACOS_BUILD_TARGET/release/bundle"
    return
  fi

  printf '%s\n' "$KCODER_STUDIO_DIR/src-tauri/target/release/bundle"
}
...
publish_to_update_service() {
  ...
  require_env KCODER_STUDIO_UPDATE_PUBLISH_TOKEN

  local curl_args=(
    -fsS
    -X POST
    -H "Authorization: Bearer ${KCODER_STUDIO_UPDATE_PUBLISH_TOKEN}"
    -F "version=${version}"
...
if [ "$TARGET" = "prod" ] && [ -z "$PROD_UPDATE_BASE_URL" ]; then
  echo "Missing required environment variable: KCODER_STUDIO_UPDATE_BASE_URL" >&2
  echo "Set it before publishing, for example:" >&2
  echo "  export KCODER_STUDIO_UPDATE_BASE_URL=https://example.com/wework/update" >&2
  exit 1
fi
...
if [ -z "${TAURI_SIGNING_PRIVATE_KEY:-}" ]; then
  if [ ! -f "$SIGNING_PRIVATE_KEY_PATH" ]; then
    ...
    echo "Generate one with: pnpm --filter kcoder-studio-renderer tauri signer generate -w $SIGNING_PRIVATE_KEY_PATH --ci" >&2
    exit 1
  fi
...
if [ "$TARGET" = "prod" ]; then
  ...
  dist_dir="$KCODER_STUDIO_DIR/src-tauri/target/release/wework-release-prod"
  rm -rf "$dist_dir"
fi
mkdir -p "$dist_dir"

config_override="$(mktemp "$KCODER_STUDIO_DIR/src-tauri/tauri.release.json.XXXXXX")"
cleanup() {
