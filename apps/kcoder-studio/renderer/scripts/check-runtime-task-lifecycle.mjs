...
const rawRunningAllowedFiles = new Set([
  ...
  // The gateway compatibility layer must read the server's raw running field before translating app-server wire data into runtime events.
  path.join(sourceRoot, 'kcoder', 'installGatewayRuntime.ts'),
])
