...
describe('reduceWorkbenchMessages', () => {
  ...
  test('keeps only a preview window for very long tool output', () => {
    ...
    const output = state[0].blocks?.[0]?.type === 'tool' ? state[0].blocks[0].toolOutput : ''
  ...
  test('merges a late live completion into the persisted message for the same turn', () => {
    const persisted: WorkbenchMessage[] = [
      {
    ...
    const completed = reduceWorkbenchMessages(persisted, {
      type: 'assistant_done',
      subtaskId: 'turn-1',
      turnId: 'turn-1',
      content: 'Complete response',
  ...
  test('only updates the last persisted assistant when a turn contains multiple messages', () => {
    const persisted: WorkbenchMessage[] = [
      {
        ...
        subtaskId: 'thread-1:history:1',
        turnId: 'turn-1',
        content: 'First response segment',
        status: 'done',
        createdAt: '2026-05-25T00:00:00.000Z',
      },
      {
        ...
        role: 'assistant',
        subtaskId: 'thread-1:history:2',
        turnId: 'turn-1',
        content: 'Second response segment',
        status: 'done',
    ...
    const completed = reduceWorkbenchMessages(persisted, {
      ...
      subtaskId: 'turn-1',
      turnId: 'turn-1',
      content: 'Authoritative final segment',
    })

    expect(completed).toHaveLength(2)
    expect(completed[0]?.content).toBe('First response segment')
  ...
  test('keeps tool arrival order when creation times are equal', () => {
    ...
    expect(withSecondTool[0].blocks?.map(block => block.id)).toEqual(['call-first', 'call-second'])
  })
  ...
  test('orders completed response blocks by creation time', () => {
    ...
    expect(done[0].blocks?.map(block => block.id)).toEqual(['call-earlier', 'call-later'])
  })
  ...
  test('resets the text offset after moving content before a tool block', () => {
    ...
    const secondPreamble = '本地分支落后于 main，CI 跑的提交是 `719f99694 fix(wework)`。'
  ...
  test('a scoped old cancellation never cancels a newer assistant when history changed its identity', () => {
    const old: WorkbenchMessage = { id: 'history-old', role: 'assistant', content: 'old partial', status: 'done',
      turnId: 'turn-1', subtaskId: 'history:1', createdAt: '2026-09-23T00:00:00Z' }
    const running = reduceWorkbenchMessages([old], { type: 'assistant_started', subtaskId: 'turn-2' })
    const cancelled = reduceWorkbenchMessages(running, { type: 'assistant_cancelled', subtaskId: 'turn-1' })
    const continued = reduceWorkbenchMessages(cancelled, { type: 'assistant_chunk', subtaskId: 'turn-2', content: 'new partial', offset: 0 })
    expect(continued.find(message => message.subtaskId === 'turn-2')).toMatchObject({ status: 'streaming', content: 'new partial' })
    expect(continued.find(message => message.id === 'history-old')).toMatchObject({ runtimeStatus: 'cancelled', stoppedNotice: true })
  })

  test('a scoped absent cancellation cannot fall back to every streaming assistant', () => {
    const running = reduceWorkbenchMessages([], { type: 'assistant_started', subtaskId: 'turn-2' })
    const cancelled = reduceWorkbenchMessages(running, { type: 'assistant_cancelled', messageId: 'old-no-longer-visible', subtaskId: 'turn-1' })
    expect(cancelled.find(message => message.subtaskId === 'turn-2')?.status).toBe('streaming')
    expect(cancelled.find(message => message.subtaskId === 'turn-1')?.stoppedNotice).toBe(true)
  })
  ...
  test('applies a late terminal tool update after the assistant message has settled', () => {
    const withTool = reduceWorkbenchMessages([], {
      type: 'block_created',
      subtaskId: '9',
      block: {
        id: 'call-agent-1',
        ...
        status: 'done',
        toolOutput: { agent_id: 'agent-1', status: 'running' },
        createdAt: 1770000000000,
      },
    })
    const settled = reduceWorkbenchMessages(withTool, {
      type: 'assistant_done',
      subtaskId: '9',
      content: '',
    })

    const interrupted = reduceWorkbenchMessages(settled, {
      type: 'block_updated',
      subtaskId: '9',
      blockId: 'call-agent-1',
      updates: {
        status: 'error',
        toolOutput: { agent_id: 'agent-1', status: 'interrupted' },
      },
    })
  ...
  test('locates a late tool update by block id after transcript reset changed the subtask id', () => {
    const historical = reduceWorkbenchMessages([], {
      type: 'reset',
      messages: [
        {
          id: 'history-message-1',
          role: 'assistant',
          subtaskId: 'thread-1:history:0',
          content: '',
          status: 'done',
          ...
          blocks: [
            {
              ...
              toolName: 'spawn_agent',
              status: 'done',
              toolOutput: { agent_id: 'agent-1', status: 'completed' },
              createdAt: 1770000000000,
            },
          ],
        },
    ...
    const interrupted = reduceWorkbenchMessages(historical, {
      type: 'block_updated',
      subtaskId: 'real-turn-1',
      blockId: 'call-agent-1',
      updates: {
        status: 'error',
        toolOutput: { agent_id: 'agent-1', status: 'interrupted' },
    ...
    expect(interrupted[0].blocks).toMatchObject([
      {
        id: 'call-agent-1',
        status: 'error',
        toolOutput: { agent_id: 'agent-1', status: 'interrupted' },
