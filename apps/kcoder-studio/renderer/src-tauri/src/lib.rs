...
#[cfg(desktop)]
const TRAY_OPEN_SETTINGS_EVENT: &str = "kcoder-tray-open-settings";
#[cfg(desktop)]
const TRAY_OPEN_TASK_EVENT: &str = "kcoder-tray-open-task";
#[cfg(desktop)]
const LOCAL_WORKSPACE_OPEN_REQUESTED_EVENT: &str = "kcoder-open-local-workspace-requested";
#[cfg(desktop)]
const CLOSE_TO_TRAY_HINT_REQUESTED_EVENT: &str = "kcoder-close-to-tray-hint-requested";
#[cfg(desktop)]
...
#[cfg(desktop)]
const FRONTEND_RESUME_PROBE_FUNCTION: &str = "__KCODER_STUDIO_NATIVE_RESUME_PROBE__";
#[cfg(desktop)]
...
#[cfg(desktop)]
const WEBVIEW_DEVTOOLS_ENV: &str = "KCODER_STUDIO_WEBVIEW_DEVTOOLS";
#[cfg(desktop)]
const APP_PREFERENCES_FILE_NAME: &str = "app-preferences.json";
#[cfg(all(desktop, target_os = "macos"))]
const KCODER_STUDIO_CLI_INSTALL_DIR: &str = ".local/bin";
#[cfg(all(desktop, target_os = "macos"))]
const KCODER_STUDIO_CLI_INSTALL_NAME: &str = "kcoder-studio";
#[cfg(all(desktop, target_os = "macos"))]
const KCODER_STUDIO_CLI_MANAGED_MARKER: &str = "# Wework CLI launcher";
...
#[cfg(all(desktop, target_os = "macos"))]
fn studio_cli_launcher_content(
    executable_path: &std::path::Path,
...
#[cfg(all(desktop, target_os = "macos"))]
fn can_replace_studio_cli_path(path: &std::path::Path) -> Result<bool, String> {
...
#[cfg(all(desktop, target_os = "macos"))]
fn install_studio_cli_link(app: &tauri::AppHandle) -> Result<std::path::PathBuf, String> {
    let home_dir = app
        .path()
        .home_dir()
        .map_err(|error| format!("Failed to locate home directory: {error}"))?;
...
#[cfg(all(desktop, not(target_os = "macos")))]
fn install_studio_cli_link(_app: &tauri::AppHandle) -> Result<std::path::PathBuf, String> {
...
#[tauri::command]
fn install_studio_cli(app: tauri::AppHandle) -> Result<String, String> {
...
mod tests {
    ...
    #[cfg(all(desktop, target_os = "macos"))]
    use super::{
        can_replace_studio_cli_path, install_studio_cli_impl, studio_cli_launcher_content,
    };
    use super::{
    ...
    fn test_temp_dir(name: &str) -> std::path::PathBuf {
        let path =
            std::env::temp_dir().join(format!("studio-cli-test-{}-{name}", std::process::id()));
    ...
    fn bakes_configured_executor_sidecar_into_cli_launcher() {
        ...
        assert!(content
            .contains("KCODER_STUDIO_EXECUTOR_SIDECAR='/repo/renderer/scripts/dev-executor-sidecar.sh'"));
        assert!(content.contains("export KCODER_STUDIO_EXECUTOR_SIDECAR"));
    ...
    fn installs_wework_cli_launcher_and_replaces_managed_files() {
        ...
        let installed_path = install_studio_cli_impl(&temp_dir, &executable_path)
    ...
    fn refuses_to_replace_unmanaged_wework_cli_file() {
        ...
        assert!(!can_replace_studio_cli_path(&installed_path)
