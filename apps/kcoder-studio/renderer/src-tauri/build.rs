...
const SIDECAR_ENV: &str = "KCODER_STUDIO_EXECUTOR_SIDECAR";
const EXECUTOR_NAMESPACE_ENV: &str = "KCODER_STUDIO_EXECUTOR_NAMESPACE";
...
fn prepare_local_executor_sidecar() {
    ...
    if is_checked_in_debug_stub(&sidecar_path) {
        make_executable(&sidecar_path);
        remove_debug_marker(&marker_path);
        return;
    }

    // An old file without a provenance marker may be only a source-launch wrapper,
    // not a usable sidecar. Debug builds replace it with a deterministic protocol
    // stub, while release builds fail closed above.
    if !sidecar_path.exists() || !marker_path.exists() {
        build_debug_stub(&sidecar_path, &marker_path, &target);
    }
}

fn is_checked_in_debug_stub(path: &Path) -> bool {
    fs::read_to_string(path)
        .is_ok_and(|source| source.lines().any(|line| line == "# KCODER_TAURI_DEBUG_STUB_V1"))
}
...
fn files_have_same_contents(source: &Path, destination: &Path) -> bool {
    ...
    match (fs::read(source), fs::read(destination)) {
        (Ok(source_contents), Ok(destination_contents)) => source_contents == destination_contents,
        _ => false,
    }
}
...
fn main() {
    ...
    for line in stdin.lock().lines() {
        ...
        println!(
            r#"{{"type":"response","id":"{}","ok":false,"error":{{"code":"SIDECAR_NOT_BUILT","message":"Local executor sidecar is not built. Run executor/local.sh build or set KCODER_STUDIO_EXECUTOR_SIDECAR before launching WeWork local-first."}}}}"#,
            id
        );
