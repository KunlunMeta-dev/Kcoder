...
const TOOL_REGRESSION_PROMPT = 'KCODER_STUDIO_DESKTOP_E2E_TOOL_TEXT_OFFSET'
const TOOL_PREAMBLE = '找到了关键错误。看一下失败前后的上下文：'
const TOOL_COMPLETION = '本地分支落后于 main，CI 跑的提交是 719f99694。'
const HIDDEN_REASONING = 'KCODER_STUDIO_DESKTOP_E2E_HIDDEN_REASONING_CONTENT'
const INITIAL_PROMPT = 'KCODER_STUDIO_DESKTOP_E2E_STREAMING_TEXT_INITIAL'
const INITIAL_COMPLETION = 'KCODER_STUDIO_DESKTOP_E2E_STREAMING_TEXT_INITIAL_COMPLETE'
const PROMPT = 'KCODER_STUDIO_DESKTOP_E2E_STREAMING_TEXT: keep the partial response active until released.'
const MARKER = 'KCODER_STUDIO_DESKTOP_E2E_STREAMING_TEXT_PARTIAL'
const VIEWPORT_MARKER = 'KCODER_STUDIO_DESKTOP_E2E_STREAMING_TEXT_VIEWPORT_ANCHOR'
const APPEND_MARKER = 'KCODER_STUDIO_DESKTOP_E2E_STREAMING_TEXT_APPENDED'
...
function assistantMessage(text) {
  return {
    ...
    item: {
      id: `studio-streaming-text-message-${Date.now()}`,
      type: 'message',
      role: 'assistant',
...
export function createDesktopScenario({
  ...
  const reasoningRelease = new Promise(resolve => {
  ...
  return {
    async handleHttp(request, response, url) {
      ...
      const responseId = `studio-streaming-text-${Date.now()}`
      ...
      if (requestContainsToolRegressionPrompt(body)) {
        if (toolRegressionStage === 'initial') {
          ...
          response.writeHead(200, { 'Content-Type': 'text/event-stream; charset=utf-8' })
          const reasoning = reasoningEvents('studio-hidden-reasoning', HIDDEN_REASONING)
          response.write(sse([responseCreated(responseId), ...reasoning.slice(0, 3)]))
          await reasoningRelease
          response.end(
            sse([
              ...reasoning.slice(3),
              assistantMessage(TOOL_PREAMBLE),
              ...functionCall('studio-tool-text-offset', tool.name, tool.arguments),
              responseCompleted(responseId),
            ])
          )
          return true
    ...
    async verify(control) {
      ...
      // This fixed protocol stream tests the folded activity indicator, not model quality.
      const spinner = `${ACTIVE_WORKBENCH_SELECTOR} [data-testid="assistant-thinking-spinner"]`
      const thinkingToggle = `${ACTIVE_WORKBENCH_SELECTOR} [data-testid="assistant-thinking-toggle"]`
      try {
        await control.command('waitFor', spinner, { visible: true, timeoutMs: uiTimeoutMs })
        assert.equal(
          Number(
            await control.command(
              'getElementCount',
              `${thinkingToggle}[aria-expanded="false"][aria-busy="true"]`
            )
          ),
          1
        )
        assert.match(
          await control.command('getStyle', spinner, { value: 'animation-name' }),
          /spin/
        )
        await control.command('click', thinkingToggle)
        await control.command('waitFor', '[data-testid="assistant-thinking-content"]', {
          text: HIDDEN_REASONING,
          visible: true,
          timeoutMs: uiTimeoutMs,
        })
        assert.equal(Number(await control.command('getElementCount', spinner)), 1)
        await control.command('click', thinkingToggle)
      } finally {
        releaseReasoning()
      }
      ...
      assert.equal(
        Number(await control.command('getElementCount', spinner)),
        0,
        'The reasoning spinner must stop when tools or assistant text take over'
      )
      await control.command('click', '[data-testid="new-chat-button"]')
      await control.command('waitFor', COMPOSER_SELECTOR, { timeoutMs: uiTimeoutMs })
      ...
      assert.ok(
        streamingTurnPreview.includes('KCODER_STUDIO_DESKTOP_E2E_STREAMING_TEXT'),
