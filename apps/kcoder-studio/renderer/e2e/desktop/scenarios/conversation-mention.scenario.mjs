...
const SOURCE_PROMPT = 'KCODER_STUDIO_CONVERSATION_REFERENCE_SOURCE: remember the launch code ORBIT-42.'
...
async function selectConversationOption(control, timeoutMs) {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    await control.command('fill', COMPOSER_SELECTOR, {
      value: '@KCODER_STUDIO_CONVERSATION_REFERENCE_SOURCE',
    })
...
export function createDesktopScenario({ captureScreenshot, uiTimeoutMs }) {
  ...
  return {
    ...
    async verify(control) {
      ...
      assert.ok(
        menuSnapshot.text.includes('KCODER_STUDIO_CONVERSATION_REFERENCE_SOURCE'),
