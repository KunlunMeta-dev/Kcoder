...
import { requiredE2EUrl } from '../utils/required-env'

const responseApiMockUrl = requiredE2EUrl('KCODER_STUDIO_RESPONSE_API_MOCK_URL')
...
test('serves browser-accessible OpenAI Responses API mock responses', async ({ page }) => {
        ...
        input: [
          {
            ...
            content: [
              {
                type: 'input_text',
                text: 'Remember CTX_KCODER_STUDIO_E2E.',
              },
            ],
          },
        ],
  ...
  expect(result.outputText).toContain('CTX_KCODER_STUDIO_E2E')
  const matchingRequests = (result.captured as CapturedRequest[]).filter(request =>
    JSON.stringify(request.body).includes('CTX_KCODER_STUDIO_E2E')
  )
...
test('surfaces local model send circuit breaker failures', async ({ page }) => {
  ...
  ).rejects.toThrow('KCODER_STUDIO_E2E_LOCAL_MODEL_SEND_CIRCUIT_OPEN')
})
...
test('streams OpenAI Responses API events from the mock server', async ({ page }) => {
  ...
  const streamText = await page.evaluate(async mockUrl => {
    const response = await fetch(`${mockUrl}/v1/responses`, {
      ...
      body: JSON.stringify({
        model: 'mock-response-model',
        input: 'Stream CTX_KCODER_STUDIO_STREAM.',
        stream: true,
        store: false,
      }),
    })
