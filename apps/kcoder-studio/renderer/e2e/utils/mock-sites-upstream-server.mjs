...
const PORT = Number.parseInt(process.env.KCODER_STUDIO_SITES_UPSTREAM_MOCK_PORT || '0', 10)
const DEFAULT_TOKEN = process.env.KCODER_STUDIO_SITES_UPSTREAM_MOCK_TOKEN || 'e2e-sites-token'
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  if (!address || typeof address === 'string') throw new Error('Sites mock address unavailable')
  console.log(
    JSON.stringify({ event: 'READY', service: 'sites', host: '127.0.0.1', port: address.port })
  )
})
