...
const PORT = Number.parseInt(process.env.KCODER_STUDIO_RESPONSE_API_MOCK_PORT || '0', 10)
const DEFAULT_RESPONSE_CONTENT =
  process.env.KCODER_STUDIO_RESPONSE_API_MOCK_CONTENT ||
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  if (!address || typeof address === 'string')
    throw new Error('Responses API mock address unavailable')
  console.log(
    JSON.stringify({ event: 'READY', service: 'responses', host: '127.0.0.1', port: address.port })
  )
})
