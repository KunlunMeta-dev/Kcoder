  ...
  return (
    <View testID="task-header" style={styles.header}>
      ...
      <View style={styles.headerCopy}>
        <Text testID="task-header-title" style={styles.headerTitle} numberOfLines={1}>
          {snapshot.title}
        </Text>
        <View style={styles.statusRow}>
          ...
          />
          <Text testID="task-header-status" style={styles.statusText} numberOfLines={1} ellipsizeMode="tail">
            {snapshot.running
              ? "KCoder 正在工作"
              : snapshot.connected
                ? snapshot.cwd
          ...
          ListFooterComponent={
            <>
              ...
              {snapshot.error || snapshot.sendAcceptanceUnknown ? (
                <RuntimeErrorBanner
                  key={snapshot.error}
                  error={snapshot.error ?? "发送是否已接受仍未知，请核对执行状态。"}
      ...
      >
        ...
        {!user && message.status === "failed" ? (
          <View accessibilityRole="alert" testID="message-attempt-failure">
            <FailureContinuation task={task} message={message} busy={continuationBusy} connected={connected} />
          </View>
        ) : null}
...
function RuntimeErrorBanner({ error, onReconnect, onReconcile }: { error: string; onReconnect?: () => Promise<void>; onReconcile?: () => Promise<void> }) {
  ...
  return (
    ...
    >
      ...
      {onReconcile ? <Pressable testID="runtime-reconcile-send" accessibilityRole="button" disabled={reconnecting}
        onPress={() => { setReconnecting(true); void onReconcile().catch(() => {}).finally(() => setReconnecting(false)); }}
        style={[styles.errorDetailsButton, { minHeight: 44 }]}>
        <Text style={styles.errorDetailsText}>{reconnecting ? '正在核对…' : '核对执行状态'}</Text>
      </Pressable> : null}
      {onReconnect ? <Pressable testID="runtime-reconnect" accessibilityRole="button" disabled={reconnecting}
        onPress={() => { setReconnecting(true); void onReconnect().catch(() => {}).finally(() => setReconnecting(false)); }}
        style={[styles.errorDetailsButton, { minHeight: 44 }]}>
        <Text style={styles.errorDetailsText}>{reconnecting ? '正在重新连接…' : '重试连接'}</Text>
      </Pressable> : null}
      {hasDetails ? (
        <Pressable
...
const styles = StyleSheet.create({
  ...
  statusText: { color: colors.textDim, fontSize: 10, maxWidth: 230, flexShrink: 1 },
