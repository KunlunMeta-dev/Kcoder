...
function normalizeMessage(message: ThreadMessage): ChatMessage | null {
  ...
  if (
    ...
    return null;
  return { ...message, content, ...blocks, attachments };
}
...
export class TaskRuntime {
  ...
  static async create(input: {
    ...
    try {
      ...
      runtime.patch({ activeTurnId: turn.turn?.id ?? null, running: true });
      return runtime;
    } catch (error) {
      try {
        await client.request("thread/delete", { threadId: thread.id });
      } catch {
        // Best-effort reclaim an empty resident thread when no turn was created; return the original send error to the user.
      }
      runtime.close();
      throw error;
    }
    ...
    try {
      ...
      const runtime = new TaskRuntime(
        null,
        {
          ...
          loadingOlder: false,
          running:
            (historyThread?.status ?? resumedThread?.status) === "running",
          connected: true,
          activeTurnId: null,
          interaction: null,
          error: null,
  ...
  async send(
    ...
    try {
      ...
      this.patch({ activeTurnId: result.turn?.id ?? null });
    } catch (error) {
      this.patch({
  ...
  private async reconnect(): Promise<void> {
    ...
    try {
      ...
      this.patch({
        ...
        loadingOlder: false,
        running: (historyThread?.status ?? resumedThread?.status) === "running",
        connected: true,
        activeTurnId: null,
        interaction: null,
        error: null,
  ...
  private handleRpc(message: RpcMessage): void {
    ...
    if (message.method === "turn/started") {
      this.patch({
        running: true,
        activeTurnId: turnIdFrom(message),
        error: null,
      });
      return;
    }
    ...
    if (message.method === "turn/completed") {
      ...
      if (fileChanges) {
        const currentTurnId = turnIdFrom(message);
        const messages = [...this.snapshot.messages];
        const index = messages.findIndex(
          (item) => item.role === "assistant" && item.turnId === currentTurnId,
        );
        if (index >= 0) {
          messages[index] = { ...messages[index], fileChanges };
          this.patch({ messages });
        }
      }
      this.pendingInteractions = [];
  ...
  private updateActivity(turnId: string, event: JsonRecord): void {
    ...
    if (eventType.startsWith("background_job_")) {
      ...
      else if (eventType === "background_job_cancelled")
        activity = {
          id,
          type: "cancelled",
