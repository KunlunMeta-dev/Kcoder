...
export class MobileRpcError extends Error {
  constructor(message: string, readonly code = -1, readonly reason: 'transport' | 'remote' | 'protocol' = 'transport') {
...
export class GatewayRpcClient {
  ...
  static connect(profile: GatewayProfile, server: KCoderServer, workspacePath?: string,
    channel: 'runtime' | 'browser' = 'runtime'): Promise<GatewayRpcClient> {
    return gatewayConnectionBudget.run(() => GatewayRpcClient.connectAdmitted(profile, server, workspacePath, channel));
  }
  ...
  private send(message: RpcMessage): void {
    if (!this.socket || this.socket.readyState !== WebSocket.OPEN) {
      throw new MobileRpcError("RPC 连接尚未就绪");
    }
  ...
  private handleMessage(raw: string): void {
    ...
    if (typeof message.id === "number" && !message.method) {
      ...
      if (request) {
        ...
        clearTimeout(request.timer);
        if (message.error) request.reject(new MobileRpcError(message.error.message || "RPC 请求失败", message.error.code ?? -1, "remote"));
