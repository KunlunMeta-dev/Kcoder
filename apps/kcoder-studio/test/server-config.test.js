...
test("uses one local server by default", async () => {
  ...
  assert.deepEqual(publicServer(servers[0]), {
    id: "local", label: "当前计算机", labelKey: "currentComputer", description: "本机 KCoder app-server", runtime: "kcoder", transport: "local", host: undefined,
...
test("Windows SSH uses overlapped stdio while native local and Unix launches retain pipe defaults", () => {
 ...
 test('default display name provenance survives persistence without relabelling custom names', async () => {
 const [target] = await loadServers({env: {}, ...defaults});
 assert.equal(publicServer(normalizeServer(storedServer(target), defaults)).labelKey, 'currentComputer');
 const custom = normalizeServer({id:'local',label:'当前计算机',transport:'local'}, defaults);
 assert.equal(publicServer(custom).labelKey, undefined);
 });
