...
export async function loadServers({
  ...
  if (!raw) {
    if (inlineServers) raw = JSON.parse(inlineServers);
      ...
      raw = [{ id: "local", label: "当前计算机", labelKey: "currentComputer", transport: "local" }];
    }
  }
...
export function normalizeServer(entry, defaults) {
  ...
  const labelKey = entry.labelKey === "currentComputer" && transport === "local"
  ...
  if (transport === "local") {
    ...
    return { id, label, ...(labelKey ? { labelKey } : {}), description, runtime, transport, command, cwd, settingsFile, profile, chromiumBin, chromiumNoSandbox };
  }
...
export function publicServer(server) {
  return {
    id: server.id,
    label: server.label,
    ...(server.labelKey ? { labelKey: server.labelKey } : {}),
